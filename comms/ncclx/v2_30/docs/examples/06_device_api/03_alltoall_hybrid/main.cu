...
__global__ void HybridAlltoAllKernel(
  ...
  // Remote ranks: world ranks below and above the LSA range (split loops for
  // clarity).
  ...
  // Wait only on the CTA whose signalIndex sees all GIN puts targeting this
  // rank.
...
void* hybridAlltoAll(
  ...
  printf(
      "  Rank %d created device communicator with hybrid support\n", my_rank);

  if (my_rank == 0) {
    printf(
        "Starting Hybrid AlltoAll with %zu elements per rank (%zu total elements, %zu MB)\n",
        count,
        total_elements,
  ...
  for (int src_rank = 0; src_rank < total_ranks; src_rank++) {
    for (size_t i = 0; i < count; i++) {
      ...
      if (h_recvbuff[recv_idx] != expected) {
        ...
        printf(
            "  Rank %d: Hybrid mismatch at [%d][%zu]: got %.0f, expected %.0f\n",
            my_rank,
            src_rank,
            i,
            h_recvbuff[recv_idx],
  ...
  if (my_rank == 0) {
    printf(
        "Hybrid AlltoAll result: %s\n", hybrid_success ? "PASSED" : "FAILED");
    if (hybrid_success) {
      printf(
          "✓ All %zu elements correctly exchanged using hybrid communication\n",
          total_elements);
    }
  }
