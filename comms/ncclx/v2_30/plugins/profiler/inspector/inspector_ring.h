...
// Generic fixed-size ring buffer (used for both coll and P2P completed
// entries). Defined here rather than in inspector.h to keep ring logic
// self-contained; inspector.h includes this header so inspectorCommInfo can
// embed the struct.
//
// Uses Sentinel Approach (never-full) - one sentinel slot is allocated beyond
// 'size' so that empty (head==tail) and full ((tail+1)%(size+1)==head) are
// always distinguishable without a separate count field. Empty : head == tail
 ...
 *   subscript notation is not available. This helper centralises the
 * byte-offset arithmetic (idx * entrySize) so no call site needs to spell it
 * out.
 *
 * Parameters:
 ...
 */
static inline void* ringSlot(
    const struct inspectorCompletedRing* ring,
    uint32_t idx) {
  return (char*)ring->entries + idx * ring->entrySize;
}

// Returns true when the ring contains at least one entry.
static inline bool inspectorRingNonEmpty(
    const struct inspectorCompletedRing* ring) {
 ...
 */
template <typename T>
inspectorResult_t inspectorRingDrain(
    struct inspectorCompletedRing* ring,
    std::vector<T>& scratch) {
  scratch.clear();
  if (!ring)
    return static_cast<inspectorResult_t>(2); // inspectorMemoryError
