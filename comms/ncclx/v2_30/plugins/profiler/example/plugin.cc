...
#define INFO(FLAGS, ...) \
  logFn(NCCL_LOG_INFO, (FLAGS), __func__, __LINE__, __VA_ARGS__)

__hidden double gettime(void) {
  using namespace std::chrono;
  auto now = steady_clock::now();
  return duration_cast<duration<double, std::micro>>(now.time_since_epoch())
      .count();
}
...
static inline struct context* getTaskEventCtx(struct taskEventBase* base) {
  ...
  if (base->type == ncclProfileColl)
    return ((struct collApi*)base->parent)->ctx;
  if (base->type == ncclProfileP2p)
    return ((struct p2pApi*)base->parent)->ctx;
  return nullptr;
}
...
static struct context* contextFromEventHandle(void* eHandle) {
  ...
  switch (type) {
    case ncclProfileGroupApi:
      return ((struct groupApi*)eHandle)->ctx;
    case ncclProfileCollApi:
      return ((struct collApi*)eHandle)->ctx;
    case ncclProfileP2pApi:
      return ((struct p2pApi*)eHandle)->ctx;
    ...
    case ncclProfileGroup:
      return ((struct group*)eHandle)->ctx;
    case ncclProfileColl:
    case ncclProfileP2p:
      return getTaskEventCtx((struct taskEventBase*)eHandle);
    case ncclProfileProxyCtrl:
      return ((struct proxyCtrl*)eHandle)->ctx;
    ...
    case ncclProfileProxyStep: {
      ...
      return (ev->parent && ev->parent->parent)
          ? getTaskEventCtx(ev->parent->parent)
          : nullptr;
    }
    ...
    case ncclProfileCeSync: {
      ...
      return (ev->parent && ev->parent->parent) ? ev->parent->parent->ctx
                                                : nullptr;
    }
...
static ncclResult_t allocateContextPools(struct context* ctx) {
  ctx->groupApiPool =
      (struct groupApi*)calloc(groupApiPoolSize, sizeof(*ctx->groupApiPool));
  if (!ctx->groupApiPool)
    goto fail;

  ctx->collApiPool =
      (struct collApi*)calloc(collApiPoolSize, sizeof(*ctx->collApiPool));
  if (!ctx->collApiPool)
    goto fail;

  ctx->p2pApiPool =
      (struct p2pApi*)calloc(p2pApiPoolSize, sizeof(*ctx->p2pApiPool));
  if (!ctx->p2pApiPool)
    goto fail;

  ctx->kernelLaunchPool = (struct kernelLaunch*)calloc(
      kernelLaunchPoolSize, sizeof(*ctx->kernelLaunchPool));
  if (!ctx->kernelLaunchPool)
    goto fail;

  ctx->groupPool =
      (struct group*)calloc(groupPoolSize, sizeof(*ctx->groupPool));
  if (!ctx->groupPool)
    goto fail;

  ctx->collPool =
      (struct collective*)calloc(collPoolSize, sizeof(*ctx->collPool));
  if (!ctx->collPool)
    goto fail;
  ...
  ctx->ceCollPool =
      (struct ceColl*)calloc(ceCollPoolSize, sizeof(*ctx->ceCollPool));
  if (!ctx->ceCollPool)
    goto fail;
  ...
  ctx->ceSyncPool =
      (struct ceSync*)calloc(ceSyncPoolSize, sizeof(*ctx->ceSyncPool));
  if (!ctx->ceSyncPool)
    goto fail;
  ...
  ctx->ceBatchPool =
      (struct ceBatch*)calloc(ceBatchPoolSize, sizeof(*ctx->ceBatchPool));
  if (!ctx->ceBatchPool)
    goto fail;
...
static ncclResult_t initGlobalProfiler(int* eActivationMask) {
  ...
  // Only start CE poller thread if CE events are enabled AND at least one CE
  // pool is allocated
  if ((*eActivationMask &
...
__hidden ncclResult_t exampleProfilerInit(
    void** context,
    uint64_t commId,
    int* eActivationMask,
    const char* commName,
...
static FILE*
openTraceFile(struct context* ctx, char* filename, size_t filenameSize) {
  const char* dump =
      profilerDumpFile ? profilerDumpFile : getenv("NCCL_PROFILE_DUMP_FILE");
  if (!dump)
    return NULL;

  snprintf(
      filename, filenameSize, "%s_%lu_%d.json", dump, ctx->commHash, ctx->rank);
...
static void printAllEvents(FILE* fh, struct context* ctx) {
  ...
  start = (ctx->groupApiPoolIndex - groupApiPoolSize >= 0)
      ? ctx->groupApiPoolIndex - groupApiPoolSize
      : 0;
  ...
  start = (ctx->proxyCtrlPoolIndex - proxyCtrlPoolSize >= 0)
      ? ctx->proxyCtrlPoolIndex - proxyCtrlPoolSize
      : 0;
  ...
  start = (ctx->ceCollPoolIndex - ctx->ceCollPoolSize >= 0)
      ? ctx->ceCollPoolIndex - ctx->ceCollPoolSize
      : 0;
...
static void finalizeGlobalProfiler(FILE* fh) {
  if (fh) {
    int start = (detachPoolIndex - detachPoolSize >= 0)
        ? detachPoolIndex - detachPoolSize
        : 0;
...
__hidden ncclResult_t exampleProfilerFinalize(void* context) {
  ...
  INFO(
      NCCL_DESTROY,
      "PROFILER/Plugin: finalize commName: %s commHash: %lu nranks: %d rank: %d traceFile: %s",
      ctx->commName ? ctx->commName : "",
      ctx->commHash,
      ctx->nranks,
      ctx->rank,
  ...
  for (int i = 0; i < ctx->ceCollPoolSize && i < ctx->ceCollPoolIndex; i++) {
    totalCount++;
    if (ctx->ceCollPool[i].stopCompleted)
      completedCount++;
  }
  INFO(
      NCCL_INIT,
      "PROFILER/Plugin: CeColl events - total=%d completed=%d",
      totalCount,
      completedCount);
...
__hidden ncclResult_t exampleProfilerStartEvent(
    void* context,
    void** eHandle,
  ...
  *eHandle = NULL;
  ...
  if (eDescr->type == ncclProfileGroupApi) {
    ...
    int groupApiId =
        __atomic_fetch_add(&ctx->groupApiPoolIndex, 1, __ATOMIC_RELAXED);
    if ((groupApiId -
         __atomic_load_n(&ctx->groupApiPoolBase, __ATOMIC_RELAXED)) <
        groupApiPoolSize) {
      ...
      while (!profilerQueueEmpty(&event->collApiEvents)) {
        struct collApi* collApiEvent =
            profilerQueueDequeue(&event->collApiEvents);
    ...
    *eHandle = event;
    ...
    if (eDescr->parentObj == NULL)
      return ncclSuccess;
    struct collApi* event;
    int collApiId =
        __atomic_fetch_add(&ctx->collApiPoolIndex, 1, __ATOMIC_RELAXED);
    if ((collApiId - __atomic_load_n(&ctx->collApiPoolBase, __ATOMIC_RELAXED)) <
        collApiPoolSize) {
    ...
    *eHandle = event;
    ...
    struct p2pApi* event;
    int p2pApiId =
        __atomic_fetch_add(&ctx->p2pApiPoolIndex, 1, __ATOMIC_RELAXED);
    if ((p2pApiId - __atomic_load_n(&ctx->p2pApiPoolBase, __ATOMIC_RELAXED)) <
        p2pApiPoolSize) {
    ...
    *eHandle = event;
    ...
    int kernelLaunchId =
        __atomic_fetch_add(&ctx->kernelLaunchPoolIndex, 1, __ATOMIC_RELAXED);
    if ((kernelLaunchId -
         __atomic_load_n(&ctx->kernelLaunchPoolBase, __ATOMIC_RELAXED)) <
        kernelLaunchPoolSize) {
    ...
    *eHandle = event;
    ...
    if ((groupId - __atomic_load_n(&ctx->groupPoolBase, __ATOMIC_RELAXED)) <
        groupPoolSize) {
    ...
    *eHandle = event;
    ...
    if ((collId - __atomic_load_n(&ctx->collPoolBase, __ATOMIC_RELAXED)) <
        collPoolSize) {
    ...
    *eHandle = event;
    ...
    if ((p2pId - __atomic_load_n(&ctx->p2pPoolBase, __ATOMIC_RELAXED)) <
        p2pPoolSize) {
    ...
    *eHandle = event;
    ...
    int proxyCtrlId =
        __atomic_fetch_add(&ctx->proxyCtrlPoolIndex, 1, __ATOMIC_RELAXED);
    struct proxyCtrl* event =
        &ctx->proxyCtrlPool[proxyCtrlId % proxyCtrlPoolSize];
    ...
    *eHandle = event;
    ...
    if (eventBase->type == ncclProfileColl) {
      ...
      struct proxyOp* event =
          &parent->op[channelId][parent->nProxyOps[channelId]++];
    ...
    *eHandle = event;
    ...
    if (type == NCCL_PROFILER_NET_TYPE_IB) {
      if (ver == 1) {
        ncclProfilerNetIbDescr_v1_t* descr =
            (ncclProfilerNetIbDescr_v1_t*)eDescr->netPlugin.data;
        struct netPlugin* event = parent->net +
            __atomic_fetch_add(&parent->nNetEvents, 1, __ATOMIC_RELAXED);
      ...
      if (ver == 1) {
        ncclProfilerNetSockDescr_v1_t* descr =
            (ncclProfilerNetSockDescr_v1_t*)eDescr->netPlugin.data;
        struct netPlugin* event = parent->net +
            __atomic_fetch_add(&parent->nNetEvents, 1, __ATOMIC_RELAXED);
        event->type = ncclProfileNetPlugin;
        event->pluginType = type;
...
__hidden ncclResult_t exampleProfilerRecordEventState(
    void* eHandle,
...
__attribute__((visibility("default"))) int exampleProfilerStart(
    int eActivationMask,
    const char* name) {
...
__hidden ncclResult_t exampleProfilerStartEvent_v6(
    void* context,
    void** eHandle,
  ...
  return exampleProfilerStartEvent(
      context, eHandle, (ncclProfilerEventDescr_t*)eDescr);
}
...
__hidden ncclResult_t exampleProfilerRecordEventState_v6(
    void* eHandle,
  ...
  return exampleProfilerRecordEventState(
      eHandle,
      (ncclProfilerEventState_t)eState,
      (ncclProfilerEventStateArgs_t*)eStateArgs);
}
