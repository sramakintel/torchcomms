...
#define CONFIG_FIELDS_WITH_REGBUFF \
  10 // Fields including both numPipeOps and regBuff
...
static int parseAlgorithm(const char* str) {
  ...
  if (strcmp(str, "collnet_direct") == 0)
    return NCCL_ALGO_COLLNET_DIRECT;
  if (strcmp(str, "collnet_chain") == 0)
    return NCCL_ALGO_COLLNET_CHAIN;
  if (strcmp(str, "nvls") == 0)
    return NCCL_ALGO_NVLS;
  if (strcmp(str, "nvls_tree") == 0)
...
static ncclResult_t loadConfig(TunerContext* ctx, const char* filename) {
  ...
  if (!file) {
    if (ctx->logFunction) {
      ctx->logFunction(
          ...
          __FILE__,
          __LINE__,
          "TUNER/ExamplePlugin: Config file %s not found, using defaults",
          filename);
    }
  ...
  if (configCount == 0) {
    if (ctx->logFunction) {
      ctx->logFunction(
          ...
          __FILE__,
          __LINE__,
          "TUNER/ExamplePlugin: No valid configurations found in %s",
          filename);
    }
    fclose(file);
    return ncclSuccess;
  ...
  if (!ctx->configs) {
    if (ctx->logFunction) {
      ctx->logFunction(
          ...
          __FILE__,
          __LINE__,
          "TUNER/ExamplePlugin: Failed to allocate memory for %d configurations",
          configCount);
    }
    fclose(file);
  ...
  if (ctx->logFunction) {
    ctx->logFunction(
        ...
        __FILE__,
        __LINE__,
        "TUNER/ExamplePlugin: Allocated memory for %d configurations",
        configCount);
  }
  ...
  while (fgets(line, sizeof(line), file) && ctx->numConfigs < ctx->maxConfigs) {
    ...
    // Validate field count: support required fields (8), with pipeOps (9), or
    // with regBuff (10)
    if (tokenCount >= CONFIG_FIELDS_REQUIRED &&
        tokenCount <= CONFIG_FIELDS_MAX) {
      TuningConfig* config = &ctx->configs[ctx->numConfigs];
      config->collType = parseCollType(tokens[CONFIG_FIELD_COLLTYPE]);
      config->minBytes =
          (size_t)strtoull(tokens[CONFIG_FIELD_MINBYTES], NULL, 10);
      config->maxBytes =
          (size_t)strtoull(tokens[CONFIG_FIELD_MAXBYTES], NULL, 10);
      ...
      if (ctx->logFunction) {
        if (config->numPipeOps == -1 && config->regBuff == -1) {
          ctx->logFunction(
              ...
              tokens[CONFIG_FIELD_COLLTYPE],
              config->minBytes,
              config->maxBytes,
              tokens[CONFIG_FIELD_ALGORITHM],
              tokens[CONFIG_FIELD_PROTOCOL],
              config->nChannels,
              config->nNodes,
              config->nRanks);
...
__hidden ncclResult_t pluginInit(
  ...
  if (NULL != constants) {
    ...
    //       Actual numbers may vary depending on the hardware and network
    //       topology. These numbers are not guaranteed to be optimal for all
    //       cases.
    // Limit the tree bandwidth to 15GB/s
    constants->perChMaxTreeBws[NCCL_BLACKWELL_COMPCAP_IDX]
                              [NCCL_TUNING_SCALE_4NODES] = 15.0;

    // Limit the ring bandwidth to 20GB/s
    constants->perChMaxRingLL128Bws[NCCL_BLACKWELL_COMPCAP_IDX]
                                   [NCCL_TUNING_SCALE_4NODES] = 20.0;

    // Set NVLSTree base network latency to 24us
    constants->hwLatencies[NCCL_HW_NET][NCCL_ALGO_NVLS][NCCL_PROTO_SIMPLE] =
        24.0;
  }
  ...
  for (int i = 0; i < ctx->numConfigs; i++) {
    ...
    if (ctx->logFunction) {
      ctx->logFunction(
          ...
          i,
          collTypeToString(config->collType),
          config->minBytes,
          config->maxBytes,
    ...
    // Check if this config matches the current collective, size range,
    // topology, pipeline ops, and regBuff
    if (config->collType == collType && nBytes >= config->minBytes &&
      ...
      if (config->algorithm < numAlgo && config->protocol < numProto) {
        if (table[config->algorithm][config->protocol] !=
          ...
          if (ctx->logFunction) {
            ctx->logFunction(
                ...
                algorithmToString(config->algorithm),
                protocolToString(config->protocol),
                &table[config->algorithm][config->protocol],
                table[config->algorithm][config->protocol]);
          }
          table[config->algorithm][config->protocol] =
              0.0; // Set low cost to prefer this configuration
...
__hidden ncclResult_t pluginGetChunkSize(
    void* context,
    ncclFunc_t collType,
    size_t nBytes,
    int algo,
