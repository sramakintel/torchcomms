...
typedef struct {
  ...
  int maxMultiRequestSize; // Maximum number of requests supported in a single
                           // multi-request.
...
typedef struct {
  ...
  // If *sendDevComm points to a valid object, then NCCL is requesting device
  ...
  // If *recvDevComm points to a valid object, then NCCL is requesting device
  // offload for this connection
  ncclResult_t (*accept)(
      void* listenComm,
      void** recvComm,
  ...
  ncclResult_t (
      *regMr)(void* comm, void* data, size_t size, int type, void** mhandle);
  /* DMA-BUF support */
