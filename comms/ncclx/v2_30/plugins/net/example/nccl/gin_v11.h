...
typedef struct {
  ...
  ncclResult_t (
      *init)(void** ctx, uint64_t commId, ncclDebugLogger_t logFunction);
  ...
  // Progress function. Will be called if non-NULL in GIN_PROXY mode, or if
  // devHandle.needsProxyProgress=1.
  ncclResult_t (*ginProgress)(void* collComm);

  // Query the last error for the GIN support. Particularly important when
  // ginProgress is not used, to report errors.
