...
#define MAX_SUPPORTED_TOKENS_PER_RANK \
  8192 // Must match kernel template in hybridep_adapter.cu
...
struct LowLatencyLayout {
  ...
  LowLatencyLayout(
    ...
    // to do data transformation NOTES: `num_scales * sizeof(nv_bfloat162)`
    // means the per-128-channel min/max
    ...
    size_t num_bytes_per_combine_msg =
        num_scales * sizeof(nv_bfloat162) + hidden * sizeof(nv_bfloat16);
    ...
    size_t signaling_buffer_bytes = std::max(
        dispatch_recv_count_buffer_bytes, combine_recv_flag_buffer_bytes);
