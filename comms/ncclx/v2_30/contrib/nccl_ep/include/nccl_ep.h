...
// Tensor tags required to identify the type of tensors in `ncclEpDispatch` and
// `ncclEpCombine`
typedef enum {
...
typedef struct {
  ...
  // Maximum number of tokens any single rank will dispatch. Must be the same
  // across all ranks. Each rank should be prepared to receive up to
  // max_tokens_per_rank * num_ranks tokens. REQUIRED for both LL and HT modes
  // (must be > 0). In a future release, NCCL_EP_AUTO will be supported for HT
  // mode, in which case the received token count will be determined by
  // ncclEpCreateHandle.
  unsigned int max_tokens_per_rank;
  unsigned int token_size_bytes; // Token size for buffer allocation
                                 // (independent of datatype)
  unsigned long int rdma_buffer_size; // RDMA buffer size in bytes (NCCL_EP_AUTO
                                      // for auto, defaults to a sufficiently
                                      // large buffer for any algorithm)
  unsigned int
      num_qp_per_rank; // Number of QPs per rank (NCCL_EP_AUTO for auto)
...
// Allocator and free function pointer types (matching cudaMalloc/cudaFree
// signatures)
...
//   alloc_fn   - [IN]  Optional custom allocator function (NULL for default
//   cudaMalloc) free_fn    - [IN]  Optional custom free function (NULL for
//   default cudaFree)
//
// Returns:
...
//   The implementation guarantees that the tensor is contiguous in memory
//   (including accordingly setting the strides to 1 for all dimensions).
//
//   When data is nullptr, memory is allocated using the EP group's allocator
//   and the tensor owns the memory (freed on destroy). When data is non-null,
//   the tensor wraps the user-provided pointer without taking ownership (data
//   is NOT freed on destroy).
//
// Arguments:
...
//   data         - [IN]  nullptr = library allocates, non-null = user-managed
//   pointer size0..size4 - [IN]  Dimension sizes
//
...
//   using the group's allocator. If the tensor wraps user-provided data, only
//   the handle is freed; the data pointer is NOT freed.
//
// Arguments:
...
typedef struct ncclEpHandleConfig*
    ncclEpHandleConfig_t; // Reserved for future use
...
//
// Arguments:
//   handle              - [OUT] Pointer to newly created and initialized EP
//   handle ep_group            - [IN]  A valid EP group topk_idx            -
//   [IN]  Tensor holding top-K expert indices (routing information)
//   local_tensors       - [IN/OUT, optional] Array of pointers to local
//   tensors.
//                         HT: accepts optional RECV_EXPERT_COUNTER tensor (1D,
//                         ncclInt32, size=num_local_experts) with tag
//                         NCCL_EP_TENSOR_TAG_RECV_EXPERT_COUNTER_HOST
//                         (pinned+mapped) or _DEVICE. Required when
//                         max_tokens_per_rank is NCCL_EP_AUTO. LL mode: does
//                         not accept local tensors (num_local_tensors must be
//                         0).
//   num_local_tensors   - [IN]  Number of local tensors.
//   config              - [IN]  Reserved for future options (should be set to
//   NULL) stream              - [IN]  CUDA stream use_fp8             - [IN]
//   Enable FP8 for dispatch (default: false)
//
// Notes:
...
typedef struct {
  unsigned int round_scales; // whether to round the scaling factors tensor into
                             // a power of 2
...
//     to indicate the types of tensor (i.e., tokens, topK indices, weights,
//     etc.)
//
// Arguments:
...
//                            The number of tokens must be equal across all
//                            tensors, but data_size may vary. Tensors are used
//                            to describe distinct pieces of data exchanged with
//                            experts. Must include token tensor
//                            (NCCL_EP_TENSOR_TAG_TOKENS) and (depending on the
//                            algorithm) may optionally be extended with
//                            metadata tensors (i.e., topK indices, weights,
//                            scales, etc.).
//   num_inputs    - [IN]     Number of input tensors
//   outputs       - [IN,OUT] Array of pointers to preallocated output tensors,
//   provided in the same order
//                            as input tensors;
//                            If the datatypes of input and output token tensors
//                            are diffent, then the additional output tensor
//                            with tag (NCCL_EP_TENSOR_TAG_SCALES) for scaling
//                            factors must be supplied.
//                            Scaling will be applied during the collective and
//                            the output tensor will be scaled. For HT: the
//                            dimensions of output tensors are [num_recv_tokens
//                            x data_size] (2D),
//                                    where num_recv_tokens = num_ranks *
//                                    max_tokens_per_rank.
//                            For LL: the dimensions of output tensors are
//                                    [local_experts x num_recv_tokens x
//                                    data_size] (3D, expert-major). The
//                                    dimensions of the scaling factors tensor
//                                    are: [local_experts x num_recv_tokens x
//                                    (hidden / 128)] (3D, ncclFloat32) where
//                                    num_recv_tokens = num_ranks *
//                                    max_tokens_per_rank.
//   num_outputs   - [IN]     Number of output tensors (equal to num_inputs plus
//   number of scaling tensors) local_tensors - [IN,OUT] Array of pointers to
//   preallocated tensors, with information that is local to the rank.
//                            LL mode: accepts 1 optional local tensor:
//                                    NUM_TOKENS_PER_EXPERTS: [OUT] a 1D tensor
//                                    of unsigned int [num_experts] that
//                                    contains the number of tokens received by
//                                    each expert on this rank.
//   num_local_tensors - [IN] Number of local tensors.
//   send_only     - [IN]     If true, the dispatch will only initiate data
//   transfers and immediately
//                            release GPU resources (without waiting for the
//                            data to be received). When set, a blocking
//                            `ncclEpComplete()` must be used to complete the
//                            operation. Note:
//                              - Supported for LL mode only.
//                              - The output tensors must still be preallocated
//                              even when send_only is set.
//   config        - [IN]     Dispatch configuration.
//   stream        - [IN]     CUDA stream. If `ncclEpDispatch()` is called on a
//   different stream than the stream used in
//                            `ncclEpCreateHandle()`,
...
// Perform EP combine
//   * Gathers outputs from experts and returns them to their source in original
//   token order.
//   * This call is collective and must be invoked by all ranks in the group.
//   * All tensors are tagged using tags with `NCCL_EP_TENSOR_TAG` prefix
//     to indicate the types of tensor (i.e., tokens, topK indices, weights,
...
//
// Arguments:
//   handle           - [IN,OUT] EP handle that was used for `ncclEpDispatch()`
//   operation inputs           - [IN]     Array of pointers to input tensors,
//   each containing expert outputs.
//                               For HT: inputs are [num_recv_tokens x
//                               data_size] (2D),
//                                       where num_recv_tokens = num_ranks *
//                                       max_tokens_per_rank.
//                               For LL: inputs are [local_experts x
//                               num_recv_tokens x data_size] (3D,
//                               expert-major),
//                                       where num_recv_tokens = num_ranks *
//                                       max_tokens_per_rank.
//   num_inputs       - [IN]     Number of input tensors
//   outputs          - [IN,OUT] Array of pointers to preallocated output nd
//   tensors, same number & order as inputs.
//                               All must be 2D [num_tokens x data_size]; tokens
//                               and metadata are restored to original order.
//   num_outputs      - [IN]     Number of output tensors (must equal
//   num_inputs) local_tensors    - [IN,OUT] Array of pointers to preallocated
//   tensors, with information that is local to the rank.
//                               LL mode: accepts 1 optional local tensor:
//                                       TOP_K_WEIGHTS - IN [num_tokens x top_k]
//                                       - top-k weights for each token.
//   num_local_tensors - [IN]    Number of local tensors.
//   send_only        - [IN]     If true, the combine will only initiate data
//   transfers and immediately
//                               release GPU resources (without waiting for the
//                               data to be received). When set, a blocking
//                               `ncclEpComplete()` must be used to complete the
//                               operation. Note:
//                                 - Supported for LL mode only.
//                                 - The output tensors must still be
//                                 preallocated even when send_only is set.
//   config           - [IN]     Reserved for future options (must be NULL).
//   stream           - [IN]     CUDA stream. If `ncclEpCombine()` is called on
//   a different stream than the stream
//                               used in `ncclEpCreateHandle()`, it is the
//                               responsibility of the user to synchronize
...
//   * This should be called after a prior `ncclEpDispatch()` or
//   `ncclEpCombine()` call with `send_only` flag set.
//
// Arguments:
...
//
// Notes:
//   - If `ncclEpComplete()` is called on a different stream than the operation
//   initiation call
//     (i.e., `ncclEpDispatch()` or `ncclEpCombine()`), it is the responsibility
...
// Query the number of received tokens that will be received after a call to EP
// dispatch (HT mode only).
//
// Arguments:
//   handle           - [IN]   A valid EP handle.
//   num_recv_tokens  - [OUT]  Pointer to int, will be set to the actual number
//   of tokens expected to be received on this rank
//
// Notes:
