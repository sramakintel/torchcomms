 ...
 */
// Throughput and validation methodology aligned with DeepEP
...
cuptiBufferRequested(uint8_t** buf, size_t* sz, size_t* maxRecords) {
...
static void CUPTIAPI cuptiBufferCompleted(
    CUcontext /*ctx*/,
    uint32_t /*streamId*/,
    uint8_t* buf,
    size_t /*sz*/,
 ...
 public:
  ...
  void dump(int rank) const {
    if (rank != 0)
      return;
    printf(
        "[KernelTimer] Captured %zu distinct kernel(s):\n",
        g_kernel_stats.size());
    for (const auto& kv : g_kernel_stats) {
      double avg_us =
          static_cast<double>(kv.second.total_ns) / kv.second.count / 1000.0;
      printf(
          "  count=%3d  avg=%.2f us  %s\n",
          kv.second.count,
          avg_us,
          kv.first.c_str());
    }
...
void setupLowLatencyTensors(
  ...
  // Dispatch output: 3D [num_local_experts, max_tokens_per_rank * nRanks,
  // hidden]
  NCCLCHECK(ncclEpTensorCreate(
      ep_group,
      &tensors.outputs[0],
...
void setupHighThroughputTensors(
  ...
  // HT combine uses only 1 input (expert_outputs) and 1 output
  // (combined_output)
  tensors.num_combine_inputs = 1;
  tensors.num_combine_outputs = 1;
  ...
  // Combine input: 2D expert outputs - same size as dispatch output (received
  // token count)
  NCCLCHECK(ncclEpTensorCreate(
      ep_group,
      &tensors.expert_outputs,
  ...
  NCCLCHECK(ncclEpTensorCreate(
      ep_group,
      &tensors.combine_output_topk_weights,
      2,
      ncclFloat32,
      NCCL_EP_TENSOR_TAG_TOPK_WEIGHTS,
      nullptr,
...
// ============================================================================
// Data Validation Support (similar to DeepEP test_internode.py /
// test_low_latency.py)
//
// Methodology:
...
//     compares against actual output using a cosine-similarity metric
//     (calc_diff).
// ============================================================================
...
// Helper: Convert float to BF16 (CPU-side, truncation — used only for
// initialization)
...
// Pattern: each element = (rank - RANK_OFFSET) except last TOKEN_ID_COLS
// columns = token_index
void initializeValidationData(
  ...
  // Fill token data with rank value, embed token index in last TOKEN_ID_COLS
  // columns. Token ID is split into high (t/256) and low (t%256) bytes to stay
  // within BF16's exact integer range (0-255). First TOKEN_ID column = high
  // byte, rest = low byte.
  ...
  for (unsigned int i = 0; i < num_tokens * top_k; i++) {
    ...
    if (topk_weights_host[i] < 1e-6f)
      topk_weights_host[i] = 1e-6f;
  }
...
// Extract (source_rank, token_id) from a received token row using first and
// last columns
static bool extractTokenIdentity(
  ...
  *out_token_id = static_cast<int>(token_hi + 0.5f) * 256 +
      static_cast<int>(token_lo + 0.5f);

  return (
      *out_source_rank >= 0 && *out_source_rank < nRanks &&
      *out_token_id >= 0 && *out_token_id < static_cast<int>(num_tokens));
}

// Verify a received token row has consistent data (all rank cols match, all
// token_id cols match)
static bool verifyTokenIntegrity(const uint16_t* row, unsigned int hidden) {
  uint16_t expected_rank_bf16 = row[0];
  for (unsigned int h = 1; h < hidden - TOKEN_ID_COLS; h++) {
    if (row[h] != expected_rank_bf16)
      return false;
  }
  ...
  for (unsigned int h = hidden - TOKEN_ID_COLS + 1; h < hidden - 1; h++) {
    if (row[h] != expected_token_lo_bf16)
      return false;
  }
  return true;
}

// Validate dispatch output: verify that expected tokens arrived at the correct
// experts. Recomputes every rank's topk_idx deterministically to build the
// expected set, then checks the dispatch output for missing, unexpected, or
// corrupted tokens.
ValidationResult validateDispatchOutput(
  ...
  if (!is_ht_mode) {
    ...
    NCCLCHECK(
        ncclEpTensorGetSizes(tensors.outputs[0], &out0_sizes, &out0_ndim));
    unsigned int max_tpe = out0_sizes[1];
    size_t total_size =
        static_cast<size_t>(num_local_experts) * max_tpe * hidden;
    ...
    // Build expected set: expected[local_expert] = set of (source_rank,
    // token_id)
      ...
      for (unsigned int t = 0; t < num_tokens; t++) {
        for (unsigned int k = 0; k < top_k; k++) {
          ...
          if (expert_id < 0)
            continue;
          int expert_rank =
              static_cast<int>(expert_id) / static_cast<int>(num_local_experts);
          int local_expert =
              static_cast<int>(expert_id) % static_cast<int>(num_local_experts);
    ...
    for (unsigned int e = 0; e < num_local_experts; e++) {
      ...
      if (count < 0 || count > static_cast<int>(max_tpe)) {
        if (errors_printed < max_errors_to_print) {
          printf(
              "[Rank %d] LL dispatch: expert %u has invalid count %d (max %u)\n",
              myRank,
              e,
              count,
              max_tpe);
      ...
      for (int j = 0; j < count; j++) {
        ...
        if (!extractTokenIdentity(
                row, hidden, nRanks, num_tokens, &source_rank, &token_id)) {
          if (errors_printed < max_errors_to_print) {
            printf(
                "[Rank %d] LL dispatch: expert %u slot %d: invalid identity (rank=%d, token=%d)\n",
                myRank,
                e,
                j,
                source_rank,
        ...
        if (!verifyTokenIntegrity(row, hidden)) {
          if (errors_printed < max_errors_to_print) {
            printf(
                "[Rank %d] LL dispatch: expert %u slot %d: data corruption (rank=%d, token=%d)\n",
                myRank,
                e,
                j,
                source_rank,
        ...
        if (expected[e].find(key) == expected[e].end()) {
          if (errors_printed < max_errors_to_print) {
            printf(
                "[Rank %d] LL dispatch: expert %u slot %d: unexpected token (rank=%d, token=%d)\n",
                myRank,
                e,
                j,
                source_rank,
      ...
      for (const auto& key : expected[e]) {
        if (found.find(key) == found.end()) {
          if (errors_printed < max_errors_to_print) {
            printf(
                "[Rank %d] LL dispatch: expert %u: missing token (rank=%d, token=%d)\n",
                myRank,
                e,
                key.first,
                key.second);
    ...
    // ==================== HT Mode ====================
    // FIXME: ncclEpHandleGetNumRecvTokens returns buffer max, not actual count
    // — scan recv_topk_idx as workaround. Output buffer is [nRanks *
    // max_tokens_per_rank, hidden], tokens packed contiguously at 0..N-1. We
    // use recv_topk_idx (outputs[2]) to identify valid rows (expert index >=
    // 0).

    const unsigned int* out0_sizes_ht;
    unsigned int out0_ndim_ht;
    NCCLCHECK(ncclEpTensorGetSizes(
        tensors.outputs[0], &out0_sizes_ht, &out0_ndim_ht));
    ...
    // Scan ALL rows, but only validate rows where recv_topk_idx has valid
    // entries
    std::set<std::pair<int, int>> found;
    ...
    for (const auto& key : expected) {
      if (found.find(key) == found.end()) {
        if (errors_printed < max_errors_to_print) {
          printf(
              "[Rank %d] HT dispatch: missing token (rank=%d, token=%d)\n",
              myRank,
              key.first,
              key.second);
          errors_printed++;
  ...
  if (!result.passed) {
    ...
    snprintf(
        buf,
        sizeof(buf),
        "%s dispatch validation: %d errors",
        is_ht_mode ? "HT" : "LL",
        errors);
    result.message = buf;
  }
...
ValidationResult validateCombineOutputLL(
  ...
  for (unsigned int t = 0; t < num_tokens; t++) {
    ...
    double token_hi_val = static_cast<double>(
        bf16ToFloat(floatToBf16(static_cast<float>(t / 256))));
    double token_lo_val = static_cast<double>(
        bf16ToFloat(floatToBf16(static_cast<float>(t % 256))));
  ...
  if (!result.passed) {
    ...
    snprintf(
        buf,
        sizeof(buf),
        "LL combine: calc_diff=%.6e (threshold=1e-5)%s",
        diff,
...
ValidationResult validateCombineOutputHT(
  ...
  int* unique_ranks = countUniqueRanksPerToken(
      topk_idx_host, num_tokens, num_experts, top_k, nRanks);
  ...
  if (!result.passed) {
    ...
    snprintf(
        buf,
        sizeof(buf),
        "HT combine: calc_diff=%.6e (threshold=5e-6)%s",
        diff,
        has_nan ? ", NaN detected" : "");
    result.message = buf;
  }
...
// This ensures dispatch and combine are always paired (required for
// correctness) while still measuring individual performance
PairedBenchResult runPairedBenchmark(
  ...
  // Note: cudaStreamSynchronize between dispatch and combine is required for HT
  // mode MPI_Barrier at end of each iteration ensures all ranks stay in sync
  // (critical for HT mode)
  ...
  // Events are recorded immediately after kernel launch (before sync) to
  // measure GPU time only Sync happens after event recording to not affect
  // timing MPI_Barrier at end of each iteration ensures all ranks stay in sync
  // (critical for HT mode)
  for (int i = 0; i < num_iters; i++) {
    ...
    CUDACHECK(cudaEventRecord(
        combine_start[i], stream)); // Record after sync, before combine
    combine_fn();
  ...
  for (int i = 0; i < num_iters; i++) {
    CUDACHECK(cudaEventElapsedTime(
        &dispatch_times[i], dispatch_start[i], dispatch_end[i]));
    CUDACHECK(cudaEventElapsedTime(
        &combine_times[i], combine_start[i], combine_end[i]));
    CUDACHECK(cudaEventElapsedTime(
        &total_times[i], dispatch_start[i], combine_end[i]));
  }
  ...
  // Helper to calculate stats from times vector (skip first iteration if we
  // have more than 1)
  auto calc_stats = [](const std::vector<float>& times,
                       size_t data_bytes) -> BenchResult {
    ...
    if (times_trimmed.empty()) {
      ...
      result.avg_ms =
          std::accumulate(times_trimmed.begin(), times_trimmed.end(), 0.0) /
          times_trimmed.size();
      result.min_ms =
          *std::min_element(times_trimmed.begin(), times_trimmed.end());
      result.max_ms =
          *std::max_element(times_trimmed.begin(), times_trimmed.end());
...
LowLatencyBytes calculateLowLatencyBytes(
  ...
  // FP8 bytes per selection: hidden + hidden/128*4 + 16 (scale factors +
  // metadata)
  ...
  // Combine: always BF16
  bytes.combine_bytes = static_cast<size_t>(bytes.num_valid_selections) *
      bf16_bytes_per_selection;

  return bytes;
}

// Six bandwidth metrics for High Throughput mode, all dividing by measured time
// t:
//
...
HighThroughputBytes calculateHighThroughputBytes(
  ...
  unsigned int num_experts_per_node =
      static_cast<unsigned int>(num_experts / num_nodes);
  int local_node = myRank / num_ranks_per_node;
  unsigned int num_experts_per_rank =
      num_experts / static_cast<unsigned int>(nRanks);

  // Send side: count unique (token, node) pairs from this rank's topk_idx
  // A token routed to multiple experts on the same node is counted only once,
  // even though NCCL EP sends it to each target rank individually via NVLink
  // P2P (not once per node).
  ...
  // Each (src_rank, token) pair is counted once regardless of how many experts
  // on myRank it targets.
  ...
  const size_t bytes_per_token = use_fp8
      ? static_cast<size_t>(bf16_bytes_per_token * fp8_factor)
...
// Uses FP8 bytes for dispatch, BF16 bytes for combine (matching DeepEP
// test_low_latency.py)
void printLowLatencyResults(
  ...
  printf(
      "[Rank %d] Dispatch:         avg=%.2f us, min=%.2f us, max=%.2f us, throughput=%.2f GB/s\n",
      myRank,
      dispatch_result.avg_ms * 1000,
      dispatch_result.min_ms * 1000,
      dispatch_result.max_ms * 1000,
      dispatch_result.throughput_gbps);

  printf(
      "[Rank %d] Combine:          avg=%.2f us, min=%.2f us, max=%.2f us, throughput=%.2f GB/s\n",
      myRank,
      combine_result.avg_ms * 1000,
      combine_result.min_ms * 1000,
      combine_result.max_ms * 1000,
      combine_result.throughput_gbps);

  printf(
      "[Rank %d] Dispatch+Combine: avg=%.2f us, min=%.2f us, max=%.2f us, throughput=%.2f GB/s\n",
      myRank,
      combined_result.avg_ms * 1000,
      combined_result.min_ms * 1000,
      combined_result.max_ms * 1000,
  ...
  if (myRank == 0) {
    ...
    double avg_dispatch_tp =
        (ll_bytes.dispatch_bytes / 1e9) / (global_dispatch_avg / 1000.0);
    double avg_combine_tp =
        (ll_bytes.combine_bytes / 1e9) / (global_combine_avg / 1000.0);
    double avg_total_tp =
        (total_data_bytes / 1e9) / (global_total_avg / 1000.0);

    printf("\n=== Summary (Low Latency, across %d ranks) ===\n", nRanks);
    printf(
        "Dispatch (%s):  avg=%.2f us, min=%.2f us, max=%.2f us\n",
    ...
    printf(
        "Combine (BF16):   avg=%.2f us, min=%.2f us, max=%.2f us\n",
...
// local_kernel_dk_us / local_kernel_ck_us: per-rank CUPTI kernel times for the
// per-rank lines. All "global_*" parameters are raw MPI_SUM across all ranks
// (rank 0 only; 0 on other ranks). The function averages them by nRanks before
// use.
void printHighThroughputResults(
  ...
  printf(
      "[Rank %d] Dispatch:         total=%.2f us  kernel=%.2f us\n",
      myRank,
      dispatch_result.avg_ms * 1000,
      local_kernel_dk_us);
  printf(
      "[Rank %d] Combine:          total=%.2f us  kernel=%.2f us\n",
      myRank,
      combine_result.avg_ms * 1000,
      local_kernel_ck_us);
  printf(
      "[Rank %d] Dispatch+Combine: total=%.2f us\n",
      myRank,
      combined_result.avg_ms * 1000);
  ...
  if (myRank == 0) {
    ...
    double dk_total_s =
        global_dispatch_avg * 1e-3; // avg total dispatch time in seconds
    double ck_total_s = global_combine_avg * 1e-3;

    printf(
        "\n=== Summary (High Throughput %s, across %d ranks) ===\n",
        ht_bytes.is_fp8 ? "FP8" : "BF16",
        nRanks);
...
// Generate topk indices for LL mode (consistent with DeepEP
// test_low_latency.py) abs(randn)+1 scores → topk selection → random -1 masking
// (simulates dropped tokens)
void generateRandomTopkIndicesLL(
    int64_t* topk_idx_host,
    unsigned int num_tokens,
    unsigned int num_experts,
...
void printUsage(const char* programName, int myRank) {
  if (myRank == 0) {
    ...
    printf(
        "                          ht or high-throughput:  High throughput mode\n");
    printf(
        "  --tokens <num>          Number of tokens (default: LL=128, HT=4096)\n");
    printf("  --hidden <num>          Hidden dimension (default: 7168)\n");
    printf("  --top-k <num>           Top-k experts per token (default: 8)\n");
    printf(
        "  --experts <num>         Total number of experts (default: 256)\n");
    ...
    printf(
        "  --profile               Enable NVTX profiling mode (use with nsys)\n");
    printf(
        "  --disable-nvlink        Disable NVLink, force RDMA for intranode communication (LL only)\n");
    printf(
        "  --validate              Validate dispatch/combine data correctness\n");
    printf(
        "  --dynamic-tokens        Enable dynamic token allocation (HT only, required for random topk)\n");
...
int main(int argc, char* argv[]) {
  ...
  bool dynamic_tokens =
      false; // Enable dynamic token allocation (HT only, for random topk)
  ...
  while (
      (opt = getopt_long(
           argc, argv, "a:t:d:k:e:w:i:pnfVMh", long_options, &option_index)) !=
      -1) {
    switch (opt) {
      case 'a':
        if (strcmp(optarg, "ll") == 0 || strcmp(optarg, "low-latency") == 0) {
          algorithm = NCCL_EP_ALGO_LOW_LATENCY;
            ...
            strcmp(optarg, "ht") == 0 ||
            strcmp(optarg, "high-throughput") == 0) {
  ...
  if (num_experts % nRanks != 0) {
    if (myRank == 0) {
      printf(
          "Error: num_experts (%u) must be divisible by nRanks (%d)\n",
          num_experts,
          nRanks);
    }
    MPI_Finalize();
  ...
  // --dynamic-tokens (NCCL_EP_AUTO for max_tokens_per_rank) is intended for HT
  // mode only. Not yet supported in the current release; code paths are kept
  // for future use.
  if (dynamic_tokens) {
    if (myRank == 0) {
      if (algorithm != NCCL_EP_ALGO_HIGH_THROUGHPUT)
        printf(
            "Error: --dynamic-tokens is only applicable to HT mode (--algorithm ht)\n");
      else
        printf(
            "Error: --dynamic-tokens (NCCL_EP_AUTO for max_tokens_per_rank) is not yet supported.\n"
  ...
  // Print configuration
  if (myRank == 0) {
    const char* algo_name = (algorithm == NCCL_EP_ALGO_LOW_LATENCY)
        ? "LOW_LATENCY"
        : "HIGH_THROUGHPUT";
    ...
    printf(
        "  NVLink:          %s\n",
        disable_nvlink ? "disabled (force RDMA intranode, LL only)"
                       : "enabled");
  ...
  if (myRank == 0)
    ncclGetUniqueId(&id);
  MPICHECK(MPI_Bcast(
      static_cast<void*>(&id), sizeof(id), MPI_BYTE, 0, MPI_COMM_WORLD));
  ...
  // max_tokens_per_rank is the per-rank batch size (max tokens any single rank
  // will send).
  config.max_tokens_per_rank = dynamic_tokens ? NCCL_EP_AUTO : num_tokens;

  config.token_size_bytes = hidden * 2; // bfloat16
  // Use NCCL_EP_AUTO for buffer sizes (required for dynamic tokens with larger
  // batches) For LL mode with disable_nvlink: NCCL_P2P_DISABLE env var handles
  // NCCL GIN P2P
  config.rdma_buffer_size = NCCL_EP_AUTO;
  // num_qp_per_rank: LL mode requires >= num_local_experts, HT mode uses auto
  config.num_qp_per_rank = (algorithm == NCCL_EP_ALGO_LOW_LATENCY)
      ? num_local_experts
      : NCCL_EP_AUTO;
  config.num_channels = NCCL_EP_AUTO;

  printf(
      "Rank %d: Testing ncclEpCreateGroup with algorithm: %s\n",
      myRank,
      (algorithm == NCCL_EP_ALGO_LOW_LATENCY) ? "LOW_LATENCY"
                                              : "HIGH_THROUGHPUT");
  MPICHECK(MPI_Barrier(MPI_COMM_WORLD));
  double group_create_start = MPI_Wtime();
  NCCLCHECK(ncclEpCreateGroup(
      &ep_group, comm, &config, stream, cudaAllocCallback, cudaFreeCallback));
  CUDACHECK(cudaStreamSynchronize(stream));
  ...
  // LL: abs(randn)+1 scores + topk + -1 masking, consistent with DeepEP
  // (test_low_latency.py)
  int64_t* topk_idx_host = new int64_t[num_tokens * top_k];

  if (algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) {
    generateTopkIndicesHT(
        topk_idx_host, num_tokens, num_experts, top_k, myRank);
    if (myRank == 0) {
      printf("Using randperm topk_idx for HT mode (uniform distribution)\n\n");
    }
  } else {
    generateRandomTopkIndicesLL(
        topk_idx_host, num_tokens, num_experts, top_k, myRank);
    if (myRank == 0) {
  ...
  if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ll_bytes = calculateLowLatencyBytes(
        topk_idx_host, num_tokens, top_k, hidden, use_fp8);
  } else {
  ...
  // Create recv_expert_counter tensor for dynamic token allocation (HT +
  // dynamic mode)
  ...
  printf(
      "Rank %d: ncclEpCreateHandle took %.2f ms\n", myRank, handle_create_ms);

  // max_tokens_per_rank is the per-rank dispatch count.
  // num_recv_tokens is the max tokens this rank can receive (nRanks *
  // max_tokens_per_rank).
  ...
  // HT recv bytes are pre-computed in calculateHighThroughputBytes via routing
  // simulation
  ...
  // Initialize validation data if enabled (fills tensors with rank-based
  // patterns)
  if (validate_data) {
    if (myRank == 0) {
  ...
  // ==================== Always run dispatch and combine paired to ensure
  // correct internal state (matching DeepEP's benchmarking approach)

  // Debug: print tensor setup for HT mode
  int num_dispatch_local = (algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) ? 0 : 1;
  if (algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT && myRank == 0) {
    printf(
        "HT Dispatch: %d inputs, %d outputs, %d local_tensors\n",
        tensors.num_dispatch_inputs,
        tensors.num_dispatch_outputs,
        num_dispatch_local);
    printf(
        "HT Combine: %d inputs, %d outputs, %d local_tensors\n",
  ...
  int num_dispatch_local_tensors =
      (algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) ? 0 : 1;
  ...
  // HT mode uses "cached" mode for iterations after the first (handle state is
  // reused)
  ...
  // ==================== Debug: show all captured kernels on rank 0 (uncomment
  // to inspect names) ktimer.dump(myRank);
  ...
  if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ...
    if (myRank == 0) {
      ...
      if (avg_dk_us == 0.0 || avg_ck_us == 0.0) {
        ...
        printf(
            "  Uncomment ktimer.dump() above to inspect captured kernel names.\n");
      }
      fflush(stdout);
    }
  } else {
  ...
  // Aggregate group/handle creation times across ranks
  {
    ...
    if (myRank == 0) {
      ...
      printf(
          ...
          global_group_max);
      printf(
          "ncclEpCreateHandle:  avg=%.2f ms, min=%.2f ms, max=%.2f ms\n",
          global_handle_avg,
          global_handle_min,
          global_handle_max);
    }
  ...
  if (validate_data) {
    ...
    // For validation, we just pass through the received tokens
    {
      ...
      if (algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT) {
        ...
        NCCLCHECK(
            ncclEpTensorGetSizes(tensors.expert_outputs, &eo_sizes, &eo_ndim));
        size_t data_size = eo_sizes[0] * eo_sizes[1] * sizeof(uint16_t);
        CUDACHECK(cudaMemcpy(
            eo_data, output0_data, data_size, cudaMemcpyDeviceToDevice));
      } else {
        ...
        NCCLCHECK(
            ncclEpTensorGetSizes(tensors.outputs[0], &out0_sizes, &out0_ndim));
        size_t data_size =
            out0_sizes[0] * out0_sizes[1] * out0_sizes[2] * sizeof(uint16_t);
        CUDACHECK(cudaMemcpy(
            eo_data, output0_data, data_size, cudaMemcpyDeviceToDevice));
      }
    }
    ...
    if (myRank == 0) {
      ...
      printf(
          "Combine validation:  %s",
          combine_valid.passed ? "PASSED" : "FAILED");
