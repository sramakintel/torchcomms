...
__forceinline__ __device__ uint64_t ncclGetP2pPtr(
  ...
  // Always use commId=0: single devComm with all GIN contexts (1-comm N-context
  // design).
  ...
  auto const p2pPtr = reinterpret_cast<uint64_t>(
...
#pragma unroll
      ...
      sendBufVec[i] = *reinterpret_cast<
          typename std::conditional<kUseFP8, int2, int4>::type*>(&dataInt4);
    }
  }
}
...
cleanNextRecvCntBuf(int* nextRecvCntBuf, int nextRecvCntBufSize, int laneId) {
  if (!dP2pDisabled) {
#pragma unroll
  ...
  if (not isRankMasked(rankMask, dstRank)) {
    if (dstP2pPtr == 0) {
      ...
      st_release_sys_global(
          reinterpret_cast<int*>(dstP2pPtr), -numTokensSent - 1);
    }
  }
}
...
__forceinline__ __device__ int waitForRecvTokens(
  ...
  if (not isRankMasked(rankMask, srcRank)) {
    size_t srcOffset =
        recvCntOff + (localExpertIdx * numRanks + srcRank) * sizeof(int);
    auto srcP2pPtr = ncclGetP2pPtr(
        0x01, srcOffset, currRank, srcRank, localExpertIdx, windows, devComms);
    ...
    if (srcP2pPtr == 0) {
      ...
      do {
        curValue =
            net.readSignal(signalsBase + localExpertIdx * numRanks + srcRank);
      } while (curValue < 1 // data not arrived
               && (waitRecvCost = clock64() - startTime) <=
                   NUM_TIMEOUT_CYCLES // not timeout
      );
      ...
      while ((numRecvTokens = ld_acquire_sys_global(
                  (recvCntBuf + localExpertIdx * numRanks + srcRank))) ==
                 0 // data not arrived
             && (waitRecvCost = clock64() - startTime) <=
                 NUM_TIMEOUT_CYCLES // not timeout
      )
  ...
  if (waitStats != nullptr)
    atomicAdd(
        reinterpret_cast<unsigned long long*>(waitStats + srcRank),
        waitRecvCost);
...
__forceinline__ __device__ void copyRecvTokenData(
  ...
  // Copy source info
  const auto recvBufSrcIdx =
      reinterpret_cast<const int*>(recvBufUint8 + tokenIdx * numBytesPerMsg);
  if (laneId == 0)
  ...
  const auto recvDataInt4 = reinterpret_cast<const int4*>(
      reinterpret_cast<const uint8_t*>(recvBufSrcIdx) + sizeof(int4));
  const auto outDataInt4Ptr =
      outDataInt4 + (recvTokenBeginIdx + tokenIdx) * hiddenInt4;
  UNROLLED_WARP_COPY(
      7,
      laneId,
      hiddenInt4,
  ...
  if constexpr (kUseFP8) {
    ...
    const auto recvScales = reinterpret_cast<const float*>(
        reinterpret_cast<const uint8_t*>(recvDataInt4) + hiddenBytes);
    const auto numElemsPerPack =
    ...
    if (laneId < numScales) {
      ...
      auto scale = extract_required_scale_format<kUseUE8M0>(
          ld_nc_global(recvScales + laneId));
      outScales[tokenIdxFinal * tokenStride + packIdx * packStride + elemIdx] =
          scale;
    }
    if (laneId + 32 < numScales) {
      ...
      auto scale = extract_required_scale_format<kUseUE8M0>(
          ld_nc_global(recvScales + laneId + 32));
      outScales[tokenIdxFinal * tokenStride + packIdx * packStride + elemIdx] =
          scale;
    }
  }
...
__global__ __launch_bounds__(1024, 1) void dispatch(// INPUT
  ...
  using packed_t = std::conditional_t<kUseUE8M0, uint32_t, float>;
  EP_STATIC_ASSERT(
      sizeof(packed_t) % sizeof(scale_t) == 0, "Invalid vector length");
  ...
  // Message package: index at source (int), 3 reserved int fields, hidden data,
  // FP8 scales NOTES: currently we have 3 reserved int fields for future use
  using vec_t = std::conditional_t<kUseFP8, int2, int4>;
  const size_t numBytesPerMsg = sizeof(int4) +
      (kUseFP8 ? (kHidden + numScales * sizeof(float))
  ...
  if (warpId < numWarps - 1) {
    ...
    EP_STATIC_ASSERT(
        kNumElemsPerRead * 32 % kNumPerChannels == 0, "Invalid vectorization");
    ...
    for (int tokenIdx = smId; tokenIdx < numTokens; tokenIdx += numSms) {
      const auto srcDataInt4 =
          static_cast<const int4*>(inData) + tokenIdx * hiddenBf16Int4;
      const auto sendBufSrcIdx = reinterpret_cast<int*>(
          static_cast<uint8_t*>(sendBuf) + tokenIdx * numBytesPerMsg);
      const auto sendBufVec = reinterpret_cast<vec_t*>(
          reinterpret_cast<uint8_t*>(sendBufSrcIdx) + sizeof(int4));
      const auto sendBufScales = reinterpret_cast<float*>(
          reinterpret_cast<uint8_t*>(sendBufVec) + hiddenBytes);

      // Overlap top-k index read and source token index writes
      auto dstExpertIdx = warpId < numTopk
          ? static_cast<int>(__ldg(inTopkIdx + tokenIdx * numTopk + warpId))
          : -1;
      ...
      if (dstExpertIdx >= 0) {
        ...
        __syncwarp();
        laneId == 0 ? atomic_add_release_global(expertDone + dstExpertIdx, 1)
                    : 0;
      }
    }
...
#pragma unroll
  ...
  if (responsibleExpertIdx < numExperts and subWarpId == 0 and laneId == 0) {
    ...
    const auto numTokensSent = sharedNumTokensSentPerExpert
        [responsibleExpertIdx - smId * numWarpGroups];

    // Wait local sends issued and send expert counts
    while (ld_acquire_global(expertDone + responsibleExpertIdx) !=
           FINISHED_SUM_TAG * 2)
      ;
    auto recvCntPtr = reinterpret_cast<uint64_t>(
        recvCntBuf + dstExpertLocalIdx * numRanks + currRank);
    size_t recvCntOffset =
        recvCntOff + (dstExpertLocalIdx * numRanks + currRank) * sizeof(int);
...
LOW_LATENCY_DISPATCH_RECV:
  ...
  if (responsibleExpertIdx < numExperts) {
    ...
    const auto recvSrcInfo =
        outSrcInfo + localExpertIdx * numRanks * maxTokensPerRank;
    const auto recvRange = outLayout + localExpertIdx * numRanks;
    const auto numAlignedScales =
        align<int>(numScales, sizeof(float) / sizeof(scale_t));
    const auto outScales = static_cast<scale_t*>(outScalesBuf) +
        localExpertIdx * numRanks * maxTokensPerRank * numAlignedScales;

    // Shared between sub-warps in warp groups
    __shared__ int sharedNumRecvTokens[kNumMaxWarpGroups],
        sharedRecvTokenBeginIdx[kNumMaxWarpGroups];
    ...
    if (subWarpId == 1 and laneId == 0) {
      ...
      recvRange[srcRank] =
          pack2<int, int64_t>(numRecvTokens, recvTokenBeginIdx);
    }
    asm volatile(
        "bar.sync %0, %1;" ::"r"(warpGroupId + 2), "r"(numWarpsPerGroup * 32));
...
logfmtEncode(void* buffer, nv_bfloat162* sharedAmaxmin, const int& laneId) {
  ...
  const auto& loadBuf = reinterpret_cast<uint32_t*>(
      static_cast<uint8_t*>(buffer) +
      laneId * (kNumSendUnrolls * sizeof(int4)));
  const auto& storeBuf = reinterpret_cast<uint32_t*>(
      static_cast<uint8_t*>(buffer) +
      laneId * (kNumSendUnrolls * sizeof(int4) * 10 / 16));
...
#pragma unroll
  ...
  auto amax = std::max(
      static_cast<float>(bf162Amax.x), static_cast<float>(bf162Amax.y));
  auto amin = std::min(
      static_cast<float>(bf162Amin.x), static_cast<float>(bf162Amin.y));
  constexpr static int kNumLanesToReduce =
      128 * sizeof(nv_bfloat16) / (kNumSendUnrolls * sizeof(int4));
  ...
  if (enableCast) {
    ...
    const auto rounding =
        2.0f - log2f_approx((1.0f + exp2f_approx(step)) * 0.5f) * stepInv;
    const auto fusedRounding = rounding - logAmin * stepInv;

    // Pack every 256 bits into 160 bits
    EP_STATIC_ASSERT(
        kNumSendUnrolls == 2 or kNumSendUnrolls == 4,
        "kNumSendUnrolls == 2 or 4 only");
...
#pragma unroll
      for (int k = 0; k < kNumElemsPerInt4; ++k) {
        const auto& [x, y] =
            __bfloat1622float2(bf162Data[i * kNumElemsPerInt4 + k]);
        encodedData[k * 2 + 0] = __float2uint_rd(
            fmaxf(log2f_approx(x) * stepInv + fusedRounding, 0));
        encodedData[k * 2 + 1] = __float2uint_rd(
            fmaxf(log2f_approx(y) * stepInv + fusedRounding, 0));
      }
      storeBuf[i * 5 + 0] = (encodedData[0] >> 0) | (encodedData[1] << 9) |
          (encodedData[2] << 18) | (encodedData[3] << 27);
      storeBuf[i * 5 + 1] = (encodedData[3] >> 5) | (encodedData[4] << 4) |
          (encodedData[5] << 13) | (encodedData[6] << 22) |
          (encodedData[7] << 31);
      storeBuf[i * 5 + 2] = (encodedData[7] >> 1) | (encodedData[8] << 8) |
          (encodedData[9] << 17) | (encodedData[10] << 26);
      storeBuf[i * 5 + 3] = (encodedData[10] >> 6) | (encodedData[11] << 3) |
          (encodedData[12] << 12) | (encodedData[13] << 21) |
          (encodedData[14] << 30);
...
#pragma unroll
    for (int i = 0; i < 2; ++i) {
      ...
      logAmin[i] = amin == 0 ? logAmax[i] - kMinClip
                             : fmaxf(log2f_approx(amin), logAmax[i] - kMinClip);
      enableCast =
          enableCast and logAmax[i] < kLogThreshold and logAmin[i] < logAmax[i];
    }
    sharedLogAmax[laneId] = make_float2(logAmax[0], logAmax[1]);
    sharedLogAmin[laneId] = make_float2(logAmin[0], logAmin[1]);
  }

  const auto& casted = warp_reduce_and<kNumSendUnrolls>(enableCast)
      ? 1u << (laneId / kNumRecvUnrolls)
      : 0u;
  const auto& numCastedPrefix = __popc(
      warp_reduce_or<kNumRecvUnrolls, true>(casted) &
      ((1u << (laneId / kNumRecvUnrolls)) - 1));

  if (laneId < kNumLanes and laneId % kNumRecvUnrolls == 0)
    sharedCastInfo[laneId / kNumRecvUnrolls] =
        (numCastedPrefix << 1) | (casted ? 1u : 0u);
  __syncwarp();
}
...
__forceinline__ __device__ void decodeAndAccumulate(
  ...
  if (enableCast) {
    ...
    auto decode = [=](const uint32_t& encoded, const uint32_t& sign) {
      const auto decoded =
          encoded == 0 ? .0f : exp2f_approx((encoded - 1) * step + logAmin);
      return sign ? -decoded : decoded;
    };

    EP_STATIC_ASSERT(
        kNumRecvUnrolls == 2 or kNumRecvUnrolls == 4,
        "kNumRecvUnrolls == 2 or 4 only");
#pragma unroll
...
#pragma unroll
      for (int k = 1; k < 5; ++k)
        concatData[k] = (ldBuffer[i * 5 + k - 1] >> (32 - k * 5)) |
            (ldBuffer[i * 5 + k] << (k * 5));
...
#pragma unroll
      for (int k = 0; k < 5; ++k) {
        accum[i * 16 + k * 3 + 0] +=
            decode(
                (concatData[k] >> 0) & 0x1ff, (localSigns >> (k * 3 + 0)) & 1) *
            weight;
        accum[i * 16 + k * 3 + 1] +=
            decode(
                (concatData[k] >> 9) & 0x1ff, (localSigns >> (k * 3 + 1)) & 1) *
            weight;
        accum[i * 16 + k * 3 + 2] += decode(
                                         (concatData[k] >> 18) & 0x1ff,
                                         (localSigns >> (k * 3 + 2)) & 1) *
            weight;
      }
      accum[i * 16 + 15] +=
          decode(concatData[5] & 0x1ff, (localSigns >> 15) & 1) * weight;
    }
  } else {
#pragma unroll
...
__forceinline__ __device__ void waitForRecvFlag(
  ...
  if (not isRankMasked(rankMask, srcRank)) {
    if (srcP2pPtr == 0) {
      ...
      while (ld_acquire_sys_global(recvFlagBuf + responsibleExpertIdx) ==
                 0 // recv not ready
             && (waitRecvCost = clock64() - startTime) <=
                 NUM_TIMEOUT_CYCLES // not timeout
      )
...
__forceinline__ __device__ void processAndSendToken(
  ...
  if (not zeroCopy or dstP2pPtr != 0) {
    ...
    const auto copySrcPtr =
        zeroCopy ? reinterpret_cast<int4*>(sendBufPtr) : srcDataInt4Ptr;
    const auto copyDstPtr = dstP2pPtr == 0 ? reinterpret_cast<int4*>(sendBufPtr)
                                           : reinterpret_cast<int4*>(dstP2pPtr);
...
#pragma unroll
    for (int i = laneId * kNumSendUnrolls, iterIdx = 0; i < hiddenBf16Int4Pad;
      ...
      if (iterIdx + 1 < kNumIters and elect_one_sync()) {
        ...
        tmaLoadAndArrive(
            nextStageIdx, copySrcPtr + offsetInt4, getNumTmaBytes(offsetInt4));
      }
      __syncwarp();
      ...
      if constexpr (kUseLogFMT) {
        ...
        int numTmaBytes = logfmtEncode<kNumSendUnrolls>(
            ...
            (i % kNumInt4PerDivision == 0)
                ? metaBuffers + i / kNumInt4PerDivision
                : nullptr,
            laneId);
        if (elect_one_sync())
          tma_store_1d(
...
__global__ __launch_bounds__(1024, 1) void combine(// INPUT
  ...
  const auto numThreads =
      __shfl_sync(0xffffffff, static_cast<int>(blockDim.x), 0);
  const auto warpId = __shfl_sync(0xffffffff, threadId / 32, 0),
             laneId = get_lane_id();
  const auto numLocalExperts = numExperts / numRanks;
  const auto warpGroupId = warpId / numWarpsPerGroup;
  const auto subWarpId = warpId % numWarpsPerGroup;
  ...
  constexpr int hiddenBf16Int4Pad =
      align(static_cast<int>(hiddenBf16Int4), 32 * kNumSendUnrolls);
  EP_STATIC_ASSERT(
      kHidden % (32 * 2 * sizeof(int4) / sizeof(nv_bfloat16)) == 0,
      "Invalid hidden");
  EP_STATIC_ASSERT(
      kNumSendUnrolls <= kNumMaxUnrolls and kNumRecvUnrolls <= kNumMaxUnrolls,
      "Invalid unrolls");
  EP_STATIC_ASSERT(hiddenBf16Int4 % kNumSendUnrolls == 0, "Invalid hidden");
  EP_STATIC_ASSERT(
      kNumSendUnrolls >= kNumRecvUnrolls, "Invalid unroll factors");
  ...
  EP_STATIC_ASSERT(
      numBytesPerSlot % sizeof(int4) == 0, "Invalid vectorization");
  ...
  if (responsibleExpertIdx < numExperts) {
    ...
    const auto layout =
        __ldg(layoutRange + localExpertIdx * numRanks + dstRank);
    const auto srcDataInt4 = static_cast<const int4*>(inData) +
        localExpertIdx * numRanks * maxTokensPerRank * hiddenBf16Int4;
    const auto localSrcInfo =
        srcInfo + localExpertIdx * numRanks * maxTokensPerRank;
    ...
    auto smemPtr = smemBuffer +
        warpId * (kNumStages * (kNumTMABufferBytes + 16) + kNumMetaBytes);
    uint32_t tmaPhase = 0;
    auto tmaBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<int4*>(smemPtr + i * (kNumTMABufferBytes + 16));
    });
    auto fullBarriers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<uint64_t*>(
          smemPtr + i * (kNumTMABufferBytes + 16) + kNumTMABufferBytes);
    });
    auto metaBuffers = kUseLogFMT
        ? reinterpret_cast<nv_bfloat162*>(
              smemPtr + kNumStages * (kNumTMABufferBytes + 16))
        : nullptr;
    EP_STATIC_ASSERT(
        kNumSendUnrolls * kNumStages <= 12, "TMA buffer size exceed limit");
    ...
    auto tmaLoadAndArrive =
        [&](const int& stageIdx, const int4* gmemPtr, const int& numBytes) {
          tma_load_1d(
              tmaBuffers[stageIdx], gmemPtr, fullBarriers[stageIdx], numBytes);
          mbarrier_arrive_and_expect_tx(fullBarriers[stageIdx], numBytes);
        };
    auto getNumTmaBytes = [&](const int& offsetInt4) {
      return min(
          kNumTMABufferBytes,
          static_cast<int>((hiddenBf16Int4 - offsetInt4) * sizeof(int4)));
    };

    // Issue IBGDA send
    if (not isRankMasked<true>(rankMask, dstRank)) {
      for (int tokenIdx = offset + subWarpId;
           tokenIdx < offset + numTokensToSend;
           tokenIdx += numWarpsPerGroup) {
        const auto srcDataInt4Ptr = srcDataInt4 + tokenIdx * hiddenBf16Int4;
        const auto sendBufTypeRow =
            reinterpret_cast<int*>(sendBufUint8 + tokenIdx * numBytesPerSlot);
        const auto sendBufDataRow = reinterpret_cast<uint8_t*>(sendBufTypeRow);

        // Copy directly to local rank, or copy to buffer and issue RDMA
        const auto srcIdx =
            __shfl_sync(0xffffffff, __ldg(localSrcInfo + tokenIdx), 0);
    ...
    asm volatile(
        "bar.sync %0, %1;" ::"r"(warpGroupId + 1), "r"(numWarpsPerGroup * 32));
    if (subWarpId == 1 and laneId == 0) {
      while (ld_acquire_global(atomicCleanFlag) == 0)
        ;
      auto recvFlagPtr =
          reinterpret_cast<uint64_t>(recvFlagBuf + globalExpertIdx);
...
LOW_LATENCY_COMBINE_RECV:
  ...
  const int numGroups =
      min(kMaxNumGroups, (numThreads / 32) / (numDecodeWarps + 1));
  const int decodeWarpIdx =
      __shfl_sync(0xffffffff, warpId % (numDecodeWarps + 1), 0);
  const int groupIdx =
      __shfl_sync(0xffffffff, warpId / (numDecodeWarps + 1), 0);
  EP_STATIC_ASSERT(
      kHidden % (32 * kNumElemsPerInt4) == 0, "Invalid vectorization");
  ...
  if (groupIdx < numGroups) {
    ...
    constexpr int kNumBF16PerWarpBytes =
        32 * kNumRecvUnrolls * kNumElemsPerInt4 * 2;
    ...
    constexpr int kNumBytesPerGroup = kNumStages * kNumTMABufferBytes +
        kHidden * 2 + kNumStages * kNumDivisionBytes * 3;

    // Reallocate shared memory
    const auto smemGroupBuffer = smemBuffer + kNumBytesPerGroup * groupIdx;
    auto fullBarriers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<uint64_t*>(
          smemGroupBuffer + i * kNumTMABufferBytes);
    });
    auto emptyBarriers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<uint64_t*>(
          smemGroupBuffer + i * kNumTMABufferBytes + 8);
    });
    auto tmaLdBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<uint8_t*>(
          smemGroupBuffer + i * kNumTMABufferBytes + 16);
    });
    auto tmaStBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<uint32_t*>(
          smemGroupBuffer + kNumStages * kNumTMABufferBytes +
          i * kNumBF16PerWarpBytes);
    });

    // Redundant when logfmt is disabled
    const auto smemGroupPtr =
        smemGroupBuffer + kNumStages * kNumTMABufferBytes + kHidden * 2;
    auto logAmaxBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<float*>(smemGroupPtr + i * kNumDivisionBytes);
    });
    auto logAminBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<float*>(
          smemGroupPtr + kNumStages * kNumDivisionBytes +
          i * kNumDivisionBytes);
    });
    auto castInfoBuffers = PatternVisitor([=](const int& i) {
      return reinterpret_cast<int*>(
          smemGroupPtr + kNumStages * kNumDivisionBytes * 2 +
          i * kNumDivisionBytes);
    });
    ...
    asm volatile(
        "bar.sync %0, %1;" ::"r"(groupIdx + 1), "r"((numDecodeWarps + 1) * 32));
    ...
    if (decodeWarpIdx == numDecodeWarps) {
      ...
      for (int tokenIdx = smId + numSms * groupIdx;
        ...
        if (laneId < numTopk)
          topkIdxByLane =
              static_cast<int>(__ldg(inTopkIdx + tokenIdx * numTopk + laneId));
        for (int i = 0; i < numTopk; ++i) {
          ...
          auto recvBufData = static_cast<uint8_t*>(recvBuf) +
              (topkIdxReg * maxTokensPerRank + tokenIdx) * numBytesPerSlot;
          if constexpr (kUseLogFMT) {
            logfmtCheckAmaxmin<
                kNumDivisions / 2,
                kNumSendUnrolls,
          ...
          if (elect_one_sync()) {
            ...
            int numTmaBytes = numCasted * kNumLogFMTPerWarpBytes +
                (numDecodeWarps - numCasted) * kNumBF16PerWarpBytes;
            tma_load_1d(
      ...
      for (int tokenIdx = smId + numSms * groupIdx;
        ...
        for (int i = 0; i < numTopk; ++i) {
          ...
          if (isRankMasked(rankMask, topkIdxReg / numLocalExperts))
            continue;
          const auto& topkWeight =
              __shfl_sync(0xffffffff, topkWeightsByLane, i);
          ...
          if constexpr (kUseLogFMT) {
            ...
            int divisionIdx = decodeWarpIdx * (kNumRecvUnrolls * 2) +
                laneId * kNumRecvUnrolls / 16;
            decodeAndAccumulate<kNumRecvUnrolls>(
                reinterpret_cast<uint32_t*>(
                    tmaLdBuffers[stageIdx] + tmaOffset +
                    (enableCast ? kNumLogFMTPerWarpBytes
            ...
            decodeAndAccumulate<kNumRecvUnrolls>(
                reinterpret_cast<uint32_t*>(
                    tmaLdBuffers[stageIdx] + tmaOffset +
                    kNumBF16PerWarpBytes / 32 * laneId),
                combinedData,
                0,
                0,
                false,
...
#pragma unroll
        for (int k = 0; k < kNumRecvUnrolls * 4; ++k) {
          auto combinedPack =
              __nv_bfloat162(combinedData[k * 2], combinedData[k * 2 + 1]);
          tmaStBuffers[decodeWarpIdx][kNumRecvUnrolls * 4 * laneId + k] =
              *reinterpret_cast<uint32_t*>(&combinedPack);
        }
        tma_store_fence();
        if (elect_one_sync()) {
          tma_store_1d(
              tmaStBuffers[decodeWarpIdx],
              static_cast<int4*>(outData) + tokenIdx * hiddenBf16Int4 +
                  decodeWarpIdx * kNumRecvUnrolls * 32,
              kNumBF16PerWarpBytes);
        }
        __syncwarp();
      }
...
void combine(
  ...
  EP_HOST_ASSERT(
      numWarpGroups > 0 and numWarpsPerGroup > 0 and numRecvPerSm >= 0);

  const auto numWarps = numWarpGroups * numWarpsPerGroup;
  const auto numSms =
      max(ceil_div(numExperts, numWarpGroups),
  ...
  const int smemSendSize =
      numWarps * (kNumStages * numSendTmaBytes + numMetaBytes);

  // Receive buffer size
  const int numRecvTmaBytes = 16 + hidden * 2;
  const int smemRecvSize = kMaxNumGroups *
      (kNumStages * numRecvTmaBytes + hidden * 2 +
       kNumStages * numMetaBytes * 3);
...
#define COMBINE_LAUNCH_CASE(hidden)                                       \
  {                                                                       \
    if (useLogfmt) {                                                      \
      SET_SHARED_MEMORY_FOR_TMA(                                          \
          (combine<true, hidden, kCombineMaxTopk, kCombineMaxUnrolls>));  \
      LAUNCH_KERNEL(                                                      \
          &cfg,                                                           \
          combine<true, hidden, kCombineMaxTopk, kCombineMaxUnrolls>,     \
          inData,                                                         \
      ...
      SET_SHARED_MEMORY_FOR_TMA(                                          \
          (combine<false, hidden, kCombineMaxTopk, kCombineMaxUnrolls>)); \
      LAUNCH_KERNEL(                                                      \
          &cfg,                                                           \
          combine<false, hidden, kCombineMaxTopk, kCombineMaxUnrolls>,    \
          inData,                                                         \
