...
// ============================================================================
// NCCL API uses sparse format: topk_weights[token][k] = weight,
// topk_idx[token][k] = expert_id HT uses dense format: prob[token][expert] =
// weight (0 for non-selected)
void sparse_to_dense_prob(
    ...
    float* dense_prob, // [num_tokens, num_experts] - output dense prob
                       // (pre-zeroed)
    int num_tokens,
    int num_topk,
    int num_experts,
...
// The order matches dispatch output (dense_to_sparse_prob): experts scanned in
// order 0,1,2... NOTE: No topk_idx needed - expert mapping comes from
// local_expert_routing_map
void sparse_to_dense_prob_combine(
    const float*
        topk_weights, // [num_tokens, topk] - sparse weights from combine input
    const bool* local_expert_routing_map, // [num_tokens, experts_per_rank] -
                                          // from handle
...
// HT expects: recv_topk_weights[recv_token][topk],
// recv_topk_idx[recv_token][topk] Uses local_expert_routing_map to know which
// experts each token is routed to. NOTE: recv_topk_idx contains LOCAL expert
// indices (0-based within this rank)
//       to match NCCL API expectations.
void dense_to_sparse_prob(
    const float*
        dense_prob, // [num_recv_tokens, experts_per_node] - from IPC buffer
    const bool* local_expert_routing_map, // [num_recv_tokens, experts_per_rank]
                                          // - from preprocessing
    float* recv_topk_weights, // [num_recv_tokens, topk] - output sparse weights
    int64_t* recv_topk_idx, // [num_recv_tokens, topk] - output LOCAL expert
                            // indices (0-based)
    int num_recv_tokens,
    int topk,
    int experts_per_rank,
...
// sparse format with GLOBAL expert indices (matching original dispatch input
// format). Uses local_routing_map (original tokens → global experts) for the
// conversion.
void dense_to_sparse_prob_combine(
    const float* dense_prob, // [num_tokens, num_experts] - from kernel output
    const uint8_t* routing_bitmap, // [num_tokens, ceil(num_experts / 8)] -
                                   // bitmap routing map
    float* combined_topk_weights, // [num_tokens, topk] - output sparse weights
    int64_t* combined_topk_idx, // [num_tokens, topk] - output GLOBAL expert
                                // indices (optional, can be nullptr)
    int num_tokens,
    int topk,
    int num_experts,
    cudaStream_t stream);
...
// Instantiates templates with LSA_TEAM_SIZE, sizing register-file arrays
// exactly. MNNVL configurations (NVL72, LSA_TEAM_SIZE > 32) are not yet
// supported: the scan kernel in metadata_preprocessing uses warp-reduction and
// requires LSA_TEAM_SIZE <= 32. Extend this macro (and the scan kernel) when
// adding MNNVL support.
#define HYBRIDEP_SWITCH_LSA_TEAM_SIZE(lsa_val, ...)                       \
  do {                                                                    \
    switch (lsa_val) {                                                    \
      case 4: {                                                           \
        constexpr int LSA_TEAM_SIZE = 4;                                  \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 8: {                                                           \
        constexpr int LSA_TEAM_SIZE = 8;                                  \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 12: {                                                          \
        constexpr int LSA_TEAM_SIZE = 12;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 16: {                                                          \
        constexpr int LSA_TEAM_SIZE = 16;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 20: {                                                          \
        constexpr int LSA_TEAM_SIZE = 20;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 24: {                                                          \
        constexpr int LSA_TEAM_SIZE = 24;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 28: {                                                          \
        constexpr int LSA_TEAM_SIZE = 28;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      case 32: {                                                          \
        constexpr int LSA_TEAM_SIZE = 32;                                 \
        __VA_ARGS__;                                                      \
      } break;                                                            \
      default:                                                            \
        assert(                                                           \
            false &&                                                      \
            "Unsupported LSA team size (must be multiple of 4, max 32)"); \
    }                                                                     \
  } while (0)
...
#define HYBRIDEP_SWITCH_NUM_LSA_TEAMS(num_lsa_domains_val, ...) \
  do {                                                          \
    switch (num_lsa_domains_val) {                              \
      ...
      case 2: {                                                 \
        constexpr int NUM_LSA_TEAMS = 2;                        \
        __VA_ARGS__;                                            \
      } break;                                                  \
      case 3: {                                                 \
        constexpr int NUM_LSA_TEAMS = 3;                        \
        __VA_ARGS__;                                            \
      } break;                                                  \
      case 4: {                                                 \
        constexpr int NUM_LSA_TEAMS = 4;                        \
        __VA_ARGS__;                                            \
      } break;                                                  \
      case 8: {                                                 \
        constexpr int NUM_LSA_TEAMS = 8;                        \
        __VA_ARGS__;                                            \
      } break;                                                  \
      default:                                                  \
        assert(                                                 \
            false &&                                            \
            "Unsupported LSA domain count for HT (max="         \
            "NUM_MAX_RDMA_PEERS)");                             \
    }                                                           \
  } while (0)
...
// Note: Caller must provide a pre-allocated scan temp buffer (see
// get_preprocessing_scan_tmp_size). Also computes per-expert token counts for
// NCCL API compatibility when requested.
void call_metadata_preprocessing(
    ...
    uint8_t* token_rank_mask, // Scratch: cached per-token rank mask [tokens *
                              // ranks_per_node * nodes]
    int32_t* num_tokens_for_experts, // Output: total tokens for local experts
    bool*
        local_expert_routing_map, // Output: per-expert routing for local tokens
    int32_t* per_expert_token_counts, // Optional output: per-expert counts
                                      // (nullptr to skip)
    void* scan_tmp, // Pre-allocated temp buffer (from
                    // get_preprocessing_scan_tmp_size)
...
// Returns required size in bytes for the scan temp buffer used by
// call_metadata_preprocessing. Caller must allocate at least this many bytes
// and pass the pointer to call_metadata_preprocessing.
...
// All parameters needed for dispatch kernel, needed redifine because those in
// hybrid_ep.cuh are with template on data type.
...
struct CombineParams {
  ...
  // These pointers are copied into the kernel param struct for fast
  // __grid_constant__ access.
  int hidden_dim; // Model hidden dimension
  int experts_per_rank; // Experts per GPU
  int num_ranks_per_node; // Ranks per node (NVLink domain size, 1-8)
  uint16_t* const*
      expert_input_token_ptrs; // HOST array[num_ranks_per_node], BF16 only
