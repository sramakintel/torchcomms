...
// which experts each token is routed to, matching the order from dispatch
// output. Each thread handles one token.
__global__ void sparse_to_dense_prob_combine_kernel(
    const float* __restrict__ topk_weights, // [num_tokens, topk]
    const bool* __restrict__ local_expert_routing_map, // [num_tokens,
                                                       // experts_per_rank]
  ...
  for (int e = 0; e < experts_per_rank && k_in < num_topk; e++) {
    if (local_expert_routing_map[token * experts_per_rank + e]) {
      // This expert is active for this token - take next weight from sparse
      // input
      ...
      int dense_idx =
          token * experts_per_node + local_rank * experts_per_rank + e;
...
__global__ void dense_to_sparse_prob_kernel(
    ...
    const bool* __restrict__ local_expert_routing_map, // [num_recv_tokens,
                                                       // experts_per_rank]
  ...
  for (int e = 0; e < experts_per_rank && k_out < topk; e++) {
    ...
    if (local_expert_routing_map[token * experts_per_rank + e]) {
      // Use local expert id for NCCL API compatibility (expects 0-based local
      // indices)
      int64_t local_expert = static_cast<int64_t>(e);

      // Get weight from dense output (indexed by local expert within node)
      // dense_prob layout: [token, experts_per_node] where experts_per_node =
      // experts_per_rank * ranks_per_node Local rank's experts are at offset:
...
// ============================================================================
// Used for combine backward pass. Converts kernel's dense output to sparse
// format with GLOBAL expert indices (matching original dispatch input format).
...
__global__ void dense_to_sparse_prob_combine_kernel(
    ...
    const uint8_t* __restrict__ routing_bitmap, // [num_tokens, ceil(num_experts
                                                // / 8)]
  ...
  for (int e = 0; e < num_experts && k_out < topk; e++) {
    if ((routing_bitmap[token * packed_cols + e / 8] >> (e % 8)) & 1) {
      ...
      if (combined_topk_idx != nullptr) {
        combined_topk_idx[token * topk + k_out] =
            static_cast<int64_t>(e); // GLOBAL expert ID
      }
      k_out++;
    }
  }
...
void call_metadata_preprocessing(
  ...
  if (per_expert_token_counts != nullptr) {
    ...
    CUDA_CHECK(cudaMemsetAsync(
        ...
        0,
        experts_per_rank * sizeof(int32_t),
        stream));
  }

  // MNNVL configurations (> 32 GPUs per LSA domain) are not yet supported: the
  // scan kernel uses warp-reduction (LSA_TEAM_SIZE <= 32) and
  // HYBRIDEP_SWITCH_LSA_TEAM_SIZE is only instantiated up to 32. Extend both
  // when adding MNNVL support.
  EP_HOST_ASSERT(
      num_ranks_per_node <= 32 &&
      "metadata_preprocessing: LSA team size > 32 not yet supported (MNNVL)");
  ...
  HYBRIDEP_SWITCH_NUM_LSA_TEAMS(num_nodes, {
    HYBRIDEP_SWITCH_LSA_TEAM_SIZE(num_ranks_per_node, {
      ...
      HybridEPType::template metadata_preprocessing<
          HYBRIDEP_NUM_THREADS_PER_BLOCK_PREPROCESSING,
          HYBRIDEP_NUM_BLOCKS_PREPROCESSING>(
          global_routing_map,
...
size_t get_preprocessing_scan_tmp_size(int num_ranks_per_node) {
  return HYBRIDEP_NUM_BLOCKS_PREPROCESSING * num_ranks_per_node *
      sizeof(::hybrid_ep::tmp_state_t);
}
...
build_dispatch_param(const DispatchParams& params) {
  ...
  // User input buffers
  kp.attn_input_token =
      reinterpret_cast<const TOKEN_DATA_TYPE*>(params.attn_input_token);
  kp.attn_input_prob = params.attn_input_prob;
  kp.attn_input_token_scaling_factor = params.attn_input_scaling_factor;

  // Copy IPC buffer pointers from HOST arrays into embedded param struct
  // arrays. This allows fast __grid_constant__ access in the kernel (vs slow
  // global memory indirection).
  for (int i = 0; i < params.num_ranks_per_node; i++) {
    ...
    kp.expert_output_prob[i] = params.expert_output_prob_ptrs
        ? params.expert_output_prob_ptrs[i]
        : nullptr;
    kp.expert_output_scaling_factor[i] =
        params.expert_output_scaling_factor_ptrs
        ? params.expert_output_scaling_factor_ptrs[i]
        : nullptr;
  }
  ...
  kp.intra_node_write_completion_flags =
  ...
  kp.mr_info = {
      ...
      .attn_input_scaling_factor_offset =
  ...
  HYBRIDEP_SWITCH_DATATYPE(use_fp8, {
    HYBRIDEP_SWITCH_NUM_LSA_TEAMS(num_nodes, {
      HYBRIDEP_SWITCH_LSA_TEAM_SIZE(params.num_ranks_per_node, {
        // TMA requires prob buffer (experts_per_node * sizeof(float)) to be 16B
        // aligned Check alignment at runtime now that experts_per_rank is
        // dynamic
        const int experts_per_node =
            params.experts_per_rank * params.num_ranks_per_node;
        assert(
            (experts_per_node * sizeof(float)) % 16 == 0 &&
...
void call_dispatch(
  ...
  if (forward_dispatch) {
    dispatch_impl<true>(
        params, max_tokens_per_rank, num_nodes, use_fp8, stream);
...
::hybrid_ep::combine_kernel_param_t<LSA_TEAM_SIZE> build_combine_param(
  ...
  for (int i = 0; i < params.num_ranks_per_node; i++) {
    ...
    kp.expert_input_prob[i] = params.expert_input_prob_ptrs
        ? params.expert_input_prob_ptrs[i]
        : nullptr;
  }
  ...
  kp.expected_intra_node_flag_value =
  ...
  // Use offsets relative to gin_base_ptr
  kp.mr_info = {
      .rdma_intra_node_red_token_offset =
          params.mr_info.rdma_intra_node_red_token_offset,
      .combine_rdma_inter_node_group_token_offset =
          params.mr_info.combine_rdma_inter_node_group_token_offset,
      .rdma_intra_node_red_prob_offset =
          params.mr_info.rdma_intra_node_red_prob_offset,
      .combine_rdma_inter_node_group_prob_offset =
          params.mr_info.combine_rdma_inter_node_group_prob_offset};
...
void combine_impl(
  ...
  HYBRIDEP_SWITCH_NUM_LSA_TEAMS(num_nodes, {
    HYBRIDEP_SWITCH_LSA_TEAM_SIZE(params.num_ranks_per_node, {
      ...
      // Select config based on NUM_LSA_TEAMS (single-node: 12 stages/2
      // pipelines, multi-node: 5 stages/1 pipeline)
...
void call_combine(
    ...
    cudaStream_t stream) {
  if (backward_combine) {
    combine_impl<true>(params, max_tokens_per_rank, num_nodes, stream);
  } else {
    combine_impl<false>(params, max_tokens_per_rank, num_nodes, stream);
  }
}
