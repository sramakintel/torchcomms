...
// Register head size for combine (BF16 tokens). Tail (if any) accumulates in
// shared memory.
constexpr int COMBINE_REG_HEAD_HIDDEN_DIM = 4096;
static_assert(
    (COMBINE_REG_HEAD_HIDDEN_DIM % 2) == 0,
    "COMBINE_REG_HEAD_HIDDEN_DIM must be even for BF16x2.");
static_assert(
    COMBINE_REG_HEAD_HIDDEN_DIM <= MAX_HIDDEN_DIM,
...
using Reduce_t = typename std::conditional<
    ...
    typename std::conditional<
        ...
        uint32_t,
        typename std::
            conditional<NUM_OF_BOOL_TO_REDUCE % 2 == 0, uint16_t, uint8_t>::
                type>::type>::type;
...
using Copy_t = typename std::conditional<
    ...
    typename std::conditional<
        ...
        typename std::conditional<
            ...
            uint32_t,
            typename std::
                conditional<NUM_OF_BYTES_TO_COPY % 2 == 0, uint16_t, uint8_t>::
                    type>::type>::type>::type;
...
enum scan_state { EMPTY = 0, PRIV_SUM = 1 };
...
// Offsets are relative to gin_base_ptr (stored as size_t for offset
// calculation)
struct dispatch_memory_region_info_t {
  size_t attn_input_token_offset; // Offset of token staging buffer from
                                  // gin_base_ptr
  size_t
      attn_input_prob_offset; // Offset of prob staging buffer from gin_base_ptr
  size_t attn_input_scaling_factor_offset; // Offset of scaling factor staging
                                           // buffer
  ...
  size_t rdma_inter_node_group_packed_offset; // Offset of packed receive buffer
                                              // (token+prob+sf per entry)
  ...
  unsigned signals_tail_base; // Base signal ID for tail tracking (sender ->
                              // receiver)
...
struct combine_memory_region_info_t {
  size_t rdma_intra_node_red_token_offset; // Offset of intra-node reduced token
                                           // buffer
  size_t combine_rdma_inter_node_group_token_offset; // Offset of combine rdma
                                                     // token buffer
  size_t rdma_intra_node_red_prob_offset; // Offset of intra-node reduced prob
                                          // buffer
  size_t combine_rdma_inter_node_group_prob_offset; // Offset of combine rdma
                                                    // prob buffer
} __attribute__((__aligned__(8)));

// ============================================================================
// Warp-parallel memory copy helper for RDMA staging
// All 32 threads participate using int4 (16-byte) loads/stores for maximum
// bandwidth
// ============================================================================
...
struct combine_smem_layout_t {
  ...
  uint32_t* rdma_streaming_counter; // [1] cumulative tokens produced for the
                                    // current chunk

  // Accessor methods for staged buffers
  __device__ __forceinline__ uint16_t* get_intra_node_token_G2S(
      int stage) const {
    return intra_node_token_G2S_buffer + stage * token_G2S_stage_stride;
  }
  __device__ __forceinline__ uint16_t* get_intra_node_token_S2G(
      int stage) const {
    return intra_node_token_S2G_buffer + stage * token_S2G_stage_stride;
  }
  __device__ __forceinline__ float2* get_intra_node_token_tail_S2G(
      int stage) const {
    return intra_node_token_tail_S2G_buffer +
        stage * token_tail_S2G_stage_stride;
  }
  __device__ __forceinline__ uint16_t* get_inter_node_token_G2S(
      int stage) const {
    return inter_node_token_G2S_buffer + stage * token_G2S_stage_stride;
  }
  __device__ __forceinline__ uint16_t* get_inter_node_token_S2G(
      int stage) const {
    return inter_node_token_S2G_buffer + stage * token_S2G_stage_stride;
  }
  __device__ __forceinline__ float2* get_inter_node_token_tail_S2G(
      int stage) const {
    return inter_node_token_tail_S2G_buffer +
        stage * token_tail_S2G_stage_stride;
  }
  ...
  // Accessor methods for mbarrier buffers (producer = stage*2, consumer =
  // stage*2+1)
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_G2S_producer(
      int stage) const {
    return intra_node_mbarrier_G2S_buffer + stage * 2;
  }
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_G2S_consumer(
      int stage) const {
    return intra_node_mbarrier_G2S_buffer + stage * 2 + 1;
  }
  __device__ __forceinline__ uint64_t* get_inter_node_mbarrier_G2S_producer(
      int stage) const {
    return inter_node_mbarrier_G2S_buffer + stage * 2;
  }
  __device__ __forceinline__ uint64_t* get_inter_node_mbarrier_G2S_consumer(
      int stage) const {
...
struct dispatch_smem_layout_t {
  ...
  __device__ __forceinline__ void* get_token_buffer(int stage) const {
    return reinterpret_cast<void*>(
        reinterpret_cast<uint8_t*>(intra_node_token_buffer) +
        stage * token_buffer_stage_stride);
  }
  __device__ __forceinline__ float* get_prob_buffer(int stage) const {
    return reinterpret_cast<float*>(
        reinterpret_cast<uint8_t*>(intra_node_prob_buffer) +
        stage * prob_buffer_stage_stride);
  }
  __device__ __forceinline__ float* get_sf_buffer(int stage) const {
    return reinterpret_cast<float*>(
        reinterpret_cast<uint8_t*>(intra_node_scaling_factor_buffer) +
        stage * sf_buffer_stage_stride);
  }
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_producer(
      int stage) const {
    return intra_node_mbarrier_buffer + stage * 2;
  }
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_consumer(
      int stage) const {
    return intra_node_mbarrier_buffer + stage * 2 + 1;
  }

  // Pipeline-indexed stage accessors: translate (pipeline_id, local_stage) to
  // absolute stage
  __device__ __forceinline__ void* get_token_buffer(
      int pipeline_id,
      int local_stage) const {
    return get_token_buffer(pipeline_id * stages_per_pipeline + local_stage);
  }
  __device__ __forceinline__ float* get_prob_buffer(
      int pipeline_id,
      int local_stage) const {
    return get_prob_buffer(pipeline_id * stages_per_pipeline + local_stage);
  }
  __device__ __forceinline__ float* get_sf_buffer(
      int pipeline_id,
      int local_stage) const {
  ...
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_producer(
      int pipeline_id,
      int local_stage) const {
    return get_intra_node_mbarrier_producer(
        pipeline_id * stages_per_pipeline + local_stage);
  }
  __device__ __forceinline__ uint64_t* get_intra_node_mbarrier_consumer(
      int pipeline_id,
      int local_stage) const {
    return get_intra_node_mbarrier_consumer(
        pipeline_id * stages_per_pipeline + local_stage);
  }

  // Per-pipeline s2d_map accessors: each pipeline has its own 2 ping-pong
  // stages
  __device__ __forceinline__ int32_t*
  get_s2d_map_buffer(int pipeline_id, int stage, int token_idx) const {
    int abs_stage = pipeline_id * 2 + stage;
    return reinterpret_cast<int32_t*>(
               reinterpret_cast<uint8_t*>(sparse_to_dense_map_buffer) +
               abs_stage * s2d_map_stage_stride) +
        token_idx * num_of_ranks_per_node;
  }
  __device__ __forceinline__ int32_t* get_s2d_map_buffer_base(
      int pipeline_id,
      int stage) const {
    int abs_stage = pipeline_id * 2 + stage;
    return reinterpret_cast<int32_t*>(
  ...
  __device__ __forceinline__ int32_t* get_s2d_map_buffer(
      int stage,
      int token_idx) const {
  ...
  __device__ __forceinline__ uint64_t* get_s2d_map_mbar(
      int pipeline_id,
      int stage) const {
    return sparse_to_dense_map_mbarrier_buffer + pipeline_id * 2 + stage;
  }
  // Per-pipeline S2G group mbarrier
  __device__ __forceinline__ uint64_t* get_S2G_group_mbar(
...
__device__ dispatch_smem_layout_t create_dispatch_smem_layout(
  ...
  layout.token_buffer_stage_stride =
      (layout.token_buffer_stage_stride + 127) & ~127;
  layout.intra_node_token_buffer =
      reinterpret_cast<void*>(reinterpret_cast<uint8_t*>(smem_base) + offset);
  ...
  // Prob buffer (only if forward dispatch, 16B aligned) -- total stages
  // unchanged
  if (config.forward_dispatch) {
    ...
    layout.prob_buffer_stage_stride =
        (layout.prob_buffer_stage_stride + 15) & ~15;
  ...
  // attn_to_rdma_map buffer (16B aligned, only if multinode, shared across
  // pipelines)
  if (model.num_of_nodes > 1) {
    offset = (offset + 15) & ~15;
    layout.attn_to_rdma_map_buffer =
        reinterpret_cast<bool*>(reinterpret_cast<uint8_t*>(smem_base) + offset);
    offset += config.num_of_tokens_per_chunk * (model.num_of_nodes - 1) *
        sizeof(bool);
  } else {
    layout.attn_to_rdma_map_buffer = nullptr;
  }

  // Mbarrier buffers (8B aligned) -- total stages unchanged (producer+consumer
  // per stage)
  offset = (offset + 7) & ~7;
  ...
  if (model.num_of_nodes > 1) {
    ...
    layout.dispatch_memory_region_info =
        reinterpret_cast<dispatch_memory_region_info_t*>(
            reinterpret_cast<uint8_t*>(smem_base) + offset);
...
static size_t calculate_dispatch_smem_layout_size(
  ...
  // Sparse to dense map buffer: 2 ping-pong stages PER PIPELINE (128B aligned)
  int s2d_map_stage_stride = config.num_of_tokens_per_chunk *
      model.num_of_ranks_per_node * sizeof(int32_t);
  ...
  if (config.forward_dispatch) {
    int prob_buffer_stage_stride = model.num_of_experts_per_rank *
        model.num_of_ranks_per_node * sizeof(float);
    prob_buffer_stage_stride = (prob_buffer_stage_stride + 15) & ~15;
    total_size += config.num_of_stages * prob_buffer_stage_stride;
  }

  // Scaling factor buffer (16B aligned per stage, only if FP8) -- total stages
  // unchanged
  if (config.token_data_type == 0) {
  ...
  if (model.num_of_nodes > 1) {
    ...
    total_size += config.num_of_tokens_per_chunk * (model.num_of_nodes - 1) *
        sizeof(bool);
  }
  ...
  if (model.num_of_nodes > 1) {
    total_size = (total_size + 7) & ~7;
    total_size +=
        (model.num_of_nodes - 1) * sizeof(dispatch_memory_region_info_t);
  }
...
__device__ combine_smem_layout_t create_combine_smem_layout(
  ...
  // In the single-node case (num_of_nodes == 1), the combine kernel does not
  // use the intra-node staging buffers. Skipping these buffers can cut SMEM
  // roughly in half.
  ...
  const int tail_hidden_dim = model.hidden_dim > COMBINE_REG_HEAD_HIDDEN_DIM
      ? (model.hidden_dim - COMBINE_REG_HEAD_HIDDEN_DIM)
      : 0;
  layout.token_tail_S2G_stage_stride = tail_hidden_dim / 2;
  layout.prob_G2S_stage_stride =
      model.num_of_experts_per_rank * model.num_of_ranks_per_node;
  layout.prob_S2G_stage_stride =
      model.num_of_experts_per_rank * model.num_of_ranks_per_node;
  layout.prob_S2G_inter_stage_stride =
      layout.prob_S2G_stage_stride * model.num_of_nodes;
  ...
  if (multinode) {
    ...
    if (tail_hidden_dim > 0) {
      ...
      layout.intra_node_token_tail_S2G_buffer = reinterpret_cast<float2*>(
          reinterpret_cast<uint8_t*>(smem_base) + offset);
      offset += num_of_stages_s2g * layout.token_tail_S2G_stage_stride *
          sizeof(float2);
    } else {
  ...
  if (tail_hidden_dim > 0) {
    ...
    layout.inter_node_token_tail_S2G_buffer = reinterpret_cast<float2*>(
        reinterpret_cast<uint8_t*>(smem_base) + offset);
    offset +=
        num_of_stages_s2g * layout.token_tail_S2G_stage_stride * sizeof(float2);
  } else {
  ...
  if (model.num_of_nodes > 1) {
    int max_num_of_chunks_per_rank =
        (model.max_num_of_tokens_per_rank + num_of_tokens_per_chunk - 1) /
        num_of_tokens_per_chunk;
    align_offset(8);
    layout.intra_node_to_rdma_mbarrier_buffer = reinterpret_cast<uint64_t*>(
        reinterpret_cast<uint8_t*>(smem_base) + offset);
    offset += (model.num_of_nodes - 1) * max_num_of_chunks_per_rank *
        sizeof(uint64_t);
  } else {
  ...
  if (model.num_of_nodes > 1) {
    ...
    layout.combine_memory_region_info =
        reinterpret_cast<combine_memory_region_info_t*>(
            reinterpret_cast<uint8_t*>(smem_base) + offset);
  ...
  if (multinode) {
    layout.intra_node_flag_G2S_buffer =
        reinterpret_cast<bool*>(reinterpret_cast<uint8_t*>(smem_base) + offset);
  ...
  layout.inter_node_flag_G2S_buffer =
      reinterpret_cast<bool*>(reinterpret_cast<uint8_t*>(smem_base) + offset);
  offset += num_of_stages_g2s * sizeof(bool);
...
template <
    ...
    int NUM_LSA_TEAMS,
    bool BACKWARD_COMBINE>
static size_t calculate_combine_smem_layout_size(const model_config_t& model) {
  ...
  const int max_num_of_chunks_per_rank =
      (MAX_NUM_OF_TOKENS_PER_RANK + NUM_OF_TOKENS_PER_CHUNK - 1) /
      NUM_OF_TOKENS_PER_CHUNK;
  ...
  const int tail_hidden_dim = hidden_dim > COMBINE_REG_HEAD_HIDDEN_DIM
      ? (hidden_dim - COMBINE_REG_HEAD_HIDDEN_DIM)
      : 0;
  const int token_tail_S2G_stage_stride =
      tail_hidden_dim / 2; // float2 elements
  if (tail_hidden_dim > 0) {
    if constexpr (multinode) {
      total_size = (total_size + 15) & ~15;
      total_size +=
          NUM_OF_STAGES_S2G * token_tail_S2G_stage_stride * sizeof(float2);
    }
  }
  ...
  if constexpr (multinode) {
    total_size = (total_size + 7) & ~7;
    total_size +=
        (NUM_LSA_TEAMS - 1) * max_num_of_chunks_per_rank * sizeof(uint64_t);
  }
...
struct dispatch_kernel_param_t {
  ...
  const float* attn_input_prob; // Needed by expert layer, so only valid in
                                // forward dispatch.
  const float*
      attn_input_token_scaling_factor; // If input token is FP8 dtype, we need
                                       // scaling factor for tokens.
  // Output buffers. These buffers are both local and remote buffers.
  // NOTE: The source pointer arrays are allocated with cudaHostAllocMapped on
  // the host side. Device dereferencing of host-mapped pointer tables is very
  // slow. Keep a fixed-size array here and copy pointers on the host into this
  // param struct.
  TOKEN_DATA_TYPE* expert_output_token[LSA_TEAM_SIZE];
  float* expert_output_prob[LSA_TEAM_SIZE]; // Only valid in forward dispatch.
  float* expert_output_scaling_factor[LSA_TEAM_SIZE]; // Only valid for FP8
                                                      // token type.
  // Internal temp buffers. These buffers are local buffers.
  uint64_t* rdma_inter_node_group_flags; // For RDMA Atomic flags.
  uint32_t* intra_node_write_completion_flags; // For intra-node S2G write
  ...
  // Grid barrier counter for fused device_sync in dispatch tail (per-rank, not
  // IPC-shared)
  uint32_t* dispatch_grid_barrier_counter;
#ifdef HYBRIDEP_ENABLE_WARP_TIMING
  struct warp_timing_entry_t {
...
struct combine_kernel_param_t {
  ...
  uint32_t* intra_node_write_completion_flags; // For intra-node src ready
                                               // notification.
  // Metadata buffers. These buffers are local buffers.
  const bool* rdma_to_attn_map;
  const bool* attn_to_rdma_map;
...
// We want to use 1 for intra-node reduction warp group, >= 2 for inter-node
// reduction warp group, RDMA warp group currently only contains 1 warp so does
// not use named bar yet, if it need to use, it should use 2 +
// NUM_OF_DATA_PIPELINE_PER_BLOCK.
__forceinline__ __device__ void arrive_and_wait(
...
// Used for 6-comm x 4-ctx GIN configuration (6 communicators with 4 contexts
// each = 24 total channels)
__forceinline__ __device__ void get_comm_ctx(
    int global_channel,
...
// There can be only 1 inter-node warp per CUDA block! Uses ncclGin API
// (net.put, net.signal)
template <
    ...
    int NUM_OF_BLOCKS,
    bool FORWARD_DISPATCH>
__forceinline__ __device__ void N2N_warp_group_device_function(
  ...
  // Load attn_to_rdma_map using LDG.128. Each token will need 1 bool from this
  // map.
  int NUM_OF_CHUNKS_PER_RANK =
      (num_of_tokens_per_rank - 1) / NUM_OF_TOKENS_PER_CHUNK + 1;
  constexpr int GIN_NUM_RATIO =
      1 + std::is_same<TOKEN_DATA_TYPE, uint8_t>::value + FORWARD_DISPATCH;

  static_assert(
      INTER_NODE_GROUP::size() >= NUM_LSA_TEAMS - 1,
      "mr_info should be loaded at once.");
  static_assert(
      NUM_OF_TOKENS_PER_CHUNK % 32 == 0,
  ...
  constexpr int MAX_CHUNKS_PER_RANK =
      MAX_NUM_OF_TOKENS_PER_RANK / NUM_OF_TOKENS_PER_CHUNK;
  ...
  for (int chunk_idx = blockIdx.x * N2N_WARPS + n2n_warp_id;
    ...
    for (int idx = 0; idx < NUM_LSA_TEAMS - 1; ++idx) {
      ...
      int dense_dst_offset =
          smem_mr_info_ptr->rdma_inter_node_group_packed_offset +
      ...
      for (int token_idx_in_chunk = ncclCoopWarp().thread_rank();
           token_idx_in_chunk < token_range;
           token_idx_in_chunk += ncclCoopWarp().size()) {
        int token_idx = token_idx_in_chunk + chunk_base_token_idx;
        bool need_write =
            attn_to_rdma_map[token_idx * (NUM_LSA_TEAMS - 1) + remote_idx];
        size_t base_offset = dense_dst_offset;

        for (int i = 0; i < token_idx_in_chunk; i++) {
          bool prev_need_write = attn_to_rdma_map
              [((i + chunk_base_token_idx) * (NUM_LSA_TEAMS - 1)) + remote_idx];
          if (prev_need_write) {
        ...
        if (need_write) {
          net.put(
              ...
              ncclGinOptFlagsAggregateRequests); // TODO(Katie): check thread
                                                 // scope args
          ...
          if constexpr (std::is_same<TOKEN_DATA_TYPE, uint8_t>::value) {
            size_t offset =
                base_offset + token_bytes + (FORWARD_DISPATCH ? prob_bytes : 0);
            net.put(
                world,
                remote_node_id,
                nccl_window,
      ...
      unsigned tail_signal_id = smem_mr_info_ptr->signals_tail_base +
          node_rank *
              (NUM_LSA_TEAMS * num_of_ranks_per_node * MAX_CHUNKS_PER_RANK) +
          remote_node_id * (num_of_ranks_per_node * MAX_CHUNKS_PER_RANK) +
          local_rank * MAX_CHUNKS_PER_RANK + chunk_idx;
      net.signal(
          world,
          remote_node_id,
...
// processing disjoint chunks through its own partition of the shared memory
// FIFO.
template <
    ...
    bool FORWARD_DISPATCH,
    int NUM_PIPELINES>
__forceinline__ __device__ void G2S_warp_group_device_function(
    const int local_rank,
    const int node_rank,
    const int num_of_tokens_per_rank,
    const int num_of_ranks_per_node,
  ...
  static_assert(
      sizeof(bool) == 1, "Routing map loads assume sizeof(bool) == 1");
  static_assert(
  ...
  static_assert(
      MAX_NUM_OF_TOKENS_PER_RANK % NUM_OF_TOKENS_PER_CHUNK == 0,
      "MAX_NUM_OF_TOKENS_PER_RANK must be multiple of NUM_OF_TOKENS_PER_CHUNK.");
  constexpr int NUM_OF_ROUTING_INFO_LOAD_ITER_PER_CHUNK =
      NUM_OF_TOKENS_PER_CHUNK / sizeof(rdma_to_attn_map_load_t);
  constexpr int NUM_OF_TOKENS_PER_LOAD_ITER =
      sizeof(rdma_to_attn_map_load_t) / sizeof(bool);
  constexpr int STAGES_PER_PIPELINE = NUM_OF_STAGES / NUM_PIPELINES;

  const int pipeline_rank = INTRA_NODE_G2S_GROUP::warp_rank();
  const int remainder_chunk_size =
      num_of_tokens_per_rank % NUM_OF_TOKENS_PER_CHUNK;
  const int num_of_chunks_per_rank =
      ((num_of_tokens_per_rank - 1) / NUM_OF_TOKENS_PER_CHUNK) + 1;
  const int max_num_of_chunks_per_rank =
      ((MAX_NUM_OF_TOKENS_PER_RANK - 1) / NUM_OF_TOKENS_PER_CHUNK) + 1;
  const int rdma_to_attn_map_size_per_node =
      (((num_of_tokens_per_rank - 1) / 16) + 1) * 16;
  int stage = 0;
  ...
  if (cuda::ptx::elect_sync(~0)) {
    ...
    for (int i = blockIdx.x; i < num_of_chunks_per_rank; i += NUM_OF_BLOCKS) {
      ...
      if (remainder_chunk_size != 0 && i == num_of_chunks_per_rank - 1) {
        ...
        num_of_routing_info_load_iter_for_current_chunk =
            NUM_OF_ROUTING_INFO_LOAD_ITER_PER_CHUNK;
        current_chunk_size = NUM_OF_TOKENS_PER_CHUNK;
      }
      for (int j = 0; j < NUM_LSA_TEAMS; j++) {
        int node_id =
            node_rank >= j ? node_rank - j : node_rank + NUM_LSA_TEAMS - j;
        ...
        if (node_id != node_rank) {
          ...
          unsigned tail_signal_id = mr_info->signals_tail_base +
              node_id *
                  (NUM_LSA_TEAMS * num_of_ranks_per_node *
                   MAX_CHUNKS_PER_RANK) +
              node_rank * (num_of_ranks_per_node * MAX_CHUNKS_PER_RANK) +
              local_rank * MAX_CHUNKS_PER_RANK + i;
          constexpr int N2N_WARPS =
              (NUM_LSA_TEAMS == 1) ? 1 : HYBRIDEP_DISPATCH_N2N_WARPS;
        ...
        const rdma_to_attn_map_load_t* rdma_to_attn_map_load_base_addr =
            reinterpret_cast<const rdma_to_attn_map_load_t*>(
                rdma_to_attn_map +
                (node_id * rdma_to_attn_map_size_per_node +
        ...
        if (node_id != node_rank) {
          ...
          packed_base = reinterpret_cast<const uint8_t*>(gin_base_ptr) +
              ...
              rdma_buffer_tile_id * mr_info->max_tokens_per_dest *
                  mr_info->bytes_per_entry +
              static_cast<size_t>(i * NUM_OF_TOKENS_PER_CHUNK) *
                  mr_info->bytes_per_entry;
        } else {
          int chunk_first_token_id = i * NUM_OF_TOKENS_PER_CHUNK;
          token_load_base_addr =
              attn_input_token + chunk_first_token_id * HIDDEN_DIM;
          if constexpr (FORWARD_DISPATCH) {
            prob_load_base_addr = attn_input_prob +
                chunk_first_token_id *
                    (experts_per_rank * num_of_ranks_per_node * NUM_LSA_TEAMS);
          }
          if constexpr (std::is_same<TOKEN_DATA_TYPE, uint8_t>::value) {
            scaling_factor_load_base_addr = attn_input_token_scaling_factor +
                chunk_first_token_id * (HIDDEN_DIM / 128);
          }
        }
        int packed_dense_idx = 0;
        for (int k = 0; k < num_of_routing_info_load_iter_for_current_chunk;
             k++) {
          rdma_to_attn_map_load_t rdma_to_attn_map_data =
              rdma_to_attn_map_load_base_addr[k];
#pragma unroll
          for (int n = 0; n < NUM_OF_TOKENS_PER_LOAD_ITER; n++) {
            ...
            bool token_needed_by_this_node =
                *(reinterpret_cast<bool*>(&rdma_to_attn_map_data) + n);
            if (token_needed_by_this_node) {
              if (tokens_produced >= STAGES_PER_PIPELINE) {
                while (!cuda::ptx::mbarrier_try_wait_parity(
                    smem_buffer_ptr->get_intra_node_mbarrier_consumer(
                        pipeline_rank, stage),
                    consumer_parity)) {
                }
              }
              ...
              if (use_packed_layout) {
                const uint8_t* packed_entry =
                    packed_base + packed_dense_idx * mr_info->bytes_per_entry;
                cuda::ptx::cp_async_bulk(
                    ...
                    smem_buffer_ptr->get_intra_node_mbarrier_producer(
                        pipeline_rank, stage));
                ...
                if constexpr (FORWARD_DISPATCH) {
                  cuda::ptx::cp_async_bulk(
                      cuda::ptx::space_shared,
                      cuda::ptx::space_global,
                      reinterpret_cast<void*>(smem_buffer_ptr->get_prob_buffer(
                          pipeline_rank, stage)),
                ...
                if constexpr (std::is_same<TOKEN_DATA_TYPE, uint8_t>::value) {
                  size_t sf_offset_in_entry =
                      token_bytes + (FORWARD_DISPATCH ? prob_bytes : 0);
                  cuda::ptx::cp_async_bulk(
                      cuda::ptx::space_shared,
                      cuda::ptx::space_global,
                      ...
                      reinterpret_cast<const void*>(
                          packed_entry + sf_offset_in_entry),
                      (uint32_t)sf_bytes,
                      smem_buffer_ptr->get_intra_node_mbarrier_producer(
                          pipeline_rank, stage));
                ...
                cuda::ptx::cp_async_bulk(
                    ...
                    smem_buffer_ptr->get_token_buffer(pipeline_rank, stage),
                    reinterpret_cast<const void*>(
                        token_load_base_addr + (current_token_id * HIDDEN_DIM)),
                    (uint32_t)token_bytes,
                    smem_buffer_ptr->get_intra_node_mbarrier_producer(
                        pipeline_rank, stage));
                total_tx_size += (uint32_t)token_bytes;
                ...
                if constexpr (std::is_same<TOKEN_DATA_TYPE, uint8_t>::value) {
                  cuda::ptx::cp_async_bulk(
                      ...
                      reinterpret_cast<void*>(
                          smem_buffer_ptr->get_sf_buffer(pipeline_rank, stage)),
                      reinterpret_cast<const void*>(
                          scaling_factor_load_base_addr +
                          (current_token_id * (HIDDEN_DIM / 128))),
                      (uint32_t)sf_bytes,
                      smem_buffer_ptr->get_intra_node_mbarrier_producer(
                          pipeline_rank, stage));
                  total_tx_size += (uint32_t)sf_bytes;
  ...
  if (INTRA_NODE_G2S_GROUP::warp_rank() == 0) {
    int residue_flag_count =
        max_num_of_chunks_per_rank - num_of_chunks_per_rank;
    for (int node_id = blockIdx.x; node_id < NUM_LSA_TEAMS - 1;
         node_id += gridDim.x) {
      uint64_t* residue_flag_base_ptr = rdma_inter_node_group_flags +
          (node_id * max_num_of_chunks_per_rank + num_of_chunks_per_rank);
      if (INTRA_NODE_G2S_GROUP::thread_rank() < residue_flag_count) {
        residue_flag_base_ptr[INTRA_NODE_G2S_GROUP::thread_rank()] =
            expected_flag_value;
      }
    }
  }
...
template <
    ...
    int NUM_PIPELINES,
    int LSA_TEAM_SIZE>
__forceinline__ __device__ void S2G_warp_group_device_function(
    const int local_rank,
    const int node_rank,
    const int num_of_tokens_per_rank,
    const int num_of_ranks_per_node,
  ...
  const int NUM_OF_SPARSE_TO_DENSE_MAP_LOAD_ITER_PER_INPUT_TOKEN =
      (num_of_ranks_per_node * sizeof(int32_t) +
       sizeof(sparse_to_dense_map_load_t) - 1) /
      sizeof(sparse_to_dense_map_load_t);
  constexpr int NUM_OF_OUTPUT_TOKENS_PER_LOAD_ITER =
      sizeof(sparse_to_dense_map_load_t) / sizeof(int32_t);
  ...
  if (cuda::ptx::elect_sync(~0)) {
      ...
      for (int i = blockIdx.x; i < num_of_chunks_per_rank; i += NUM_OF_BLOCKS) {
        if ((chunk_iter++ % NUM_PIPELINES) == pipeline_rank) {
          ...
          const int32_t* s2d_base = sparse_to_dense_map +
              (node_rank * num_of_tokens_per_rank +
               i * NUM_OF_TOKENS_PER_CHUNK) *
                  num_of_ranks_per_node;
          cuda::ptx::cp_async_bulk(
              cuda::ptx::space_shared,
              cuda::ptx::space_global,
              reinterpret_cast<void*>(smem_buffer_ptr->get_s2d_map_buffer_base(
                  pipeline_rank, sparse_to_dense_map_stage)),
              reinterpret_cast<const void*>(s2d_base),
              (uint32_t)(current_chunk_size * num_of_ranks_per_node *
                         sizeof(int32_t)),
              smem_buffer_ptr->get_s2d_map_mbar(
                  pipeline_rank, sparse_to_dense_map_stage));
          cuda::ptx::mbarrier_arrive_expect_tx(
              cuda::ptx::sem_release,
              cuda::ptx::scope_cta,
    ...
    for (int i = blockIdx.x; i < num_of_chunks_per_rank; i += NUM_OF_BLOCKS) {
      ...
      for (int j = 0; j < NUM_LSA_TEAMS; j++) {
        ...
        uint64_t state_token = cuda::ptx::mbarrier_arrive(
            smem_buffer_ptr->get_S2G_group_mbar(pipeline_rank));
        while (!cuda::ptx::mbarrier_try_wait(
            smem_buffer_ptr->get_S2G_group_mbar(pipeline_rank), state_token)) {
        }
        ...
        // Prefetch s2d map for next (chunk, node) pair for THIS pipeline
        {
          ...
          if (next_node_iter < NUM_LSA_TEAMS) {
            next_chunk_id = i;
            next_node_id = node_rank >= next_node_iter
                ? node_rank - next_node_iter
                : node_rank + NUM_LSA_TEAMS - next_node_iter;
          } else {
            // Find the next chunk this pipeline will process
            int future_chunk_iter = chunk_iter; // chunk_iter was already
                                                // incremented for current chunk
            next_chunk_id = -1;
            for (int fi = i + NUM_OF_BLOCKS; fi < num_of_chunks_per_rank;
                 fi += NUM_OF_BLOCKS) {
          ...
          if (next_chunk_id >= 0 && next_chunk_id < num_of_chunks_per_rank) {
            ...
            if (remainder_chunk_size != 0 &&
                next_chunk_id == num_of_chunks_per_rank - 1) {
              next_chunk_size = remainder_chunk_size;
            } else {
              next_chunk_size = NUM_OF_TOKENS_PER_CHUNK;
            }
            const int32_t* s2d_base = sparse_to_dense_map +
                (next_node_id * num_of_tokens_per_rank +
                 next_chunk_id * NUM_OF_TOKENS_PER_CHUNK) *
                    num_of_ranks_per_node;
            cuda::ptx::cp_async_bulk(
                cuda::ptx::space_shared,
                cuda::ptx::space_global,
                reinterpret_cast<void*>(
                    smem_buffer_ptr->get_s2d_map_buffer_base(
                        pipeline_rank, sparse_to_dense_map_stage ^ 1)),
                reinterpret_cast<const void*>(s2d_base),
                (uint32_t)(next_chunk_size * num_of_ranks_per_node *
                           sizeof(int32_t)),
                smem_buffer_ptr->get_s2d_map_mbar(
                    pipeline_rank, sparse_to_dense_map_stage ^ 1));
            cuda::ptx::mbarrier_arrive_expect_tx(
          ...
          for (int n = 0; n < NUM_OF_TOKENS_PER_LOAD_ITER; n++) {
            ...
            if (token_needed_by_this_node) {
              const sparse_to_dense_map_load_t* sparse_to_dense_map_load_addr =
                  reinterpret_cast<const sparse_to_dense_map_load_t*>(
                      smem_buffer_ptr->get_s2d_map_buffer(
                          pipeline_rank,
                          sparse_to_dense_map_stage,
                          k * NUM_OF_TOKENS_PER_LOAD_ITER + n));
              while (!cuda::ptx::mbarrier_try_wait_parity(
                  smem_buffer_ptr->get_intra_node_mbarrier_producer(
                      pipeline_rank, stage),
                  producer_parity)) {
              ...
              // Each pipeline has 1 S2G warp, so this thread handles ALL
              // destination ranks
              for (int m = 0;
                   m < NUM_OF_SPARSE_TO_DENSE_MAP_LOAD_ITER_PER_INPUT_TOKEN;
                   m++) {
                sparse_to_dense_map_load_t sparse_to_dense_map_data =
                    sparse_to_dense_map_load_addr[m];
#pragma unroll
                for (int t = 0; t < NUM_OF_OUTPUT_TOKENS_PER_LOAD_ITER; t++) {
                  int32_t output_buffer_index =
                      *(reinterpret_cast<int32_t*>(&sparse_to_dense_map_data) +
                        t);
                  if (output_buffer_index != -1) {
                    int remote_rank_id =
                        m * NUM_OF_OUTPUT_TOKENS_PER_LOAD_ITER + t;
                    TOKEN_DATA_TYPE* remote_token_addr =
                        remote_expert_output_token[remote_rank_id] +
                        (output_buffer_index * HIDDEN_DIM);
                    cuda::ptx::cp_async_bulk(
                        cuda::ptx::space_global,
                        cuda::ptx::space_shared,
                    ...
                    if constexpr (FORWARD_DISPATCH) {
                      ...
                      cuda::ptx::cp_async_bulk(
                          ...
                          reinterpret_cast<const void*>(
                              smem_buffer_ptr->get_prob_buffer(
                                  pipeline_rank, stage)),
                          (uint32_t)((experts_per_rank *
                                      num_of_ranks_per_node) *
                                     sizeof(float)));
                    }
                    if constexpr (std::is_same<TOKEN_DATA_TYPE, uint8_t>::
                                      value) {
                      float* remote_scaling_factor_addr =
                          remote_expert_output_scaling_factor[remote_rank_id] +
                          (output_buffer_index * (HIDDEN_DIM / 128));
                      cuda::ptx::cp_async_bulk(
                          cuda::ptx::space_global,
                          cuda::ptx::space_shared,
                          reinterpret_cast<void*>(remote_scaling_factor_addr),
                          reinterpret_cast<const void*>(
                              smem_buffer_ptr->get_sf_buffer(
                                  pipeline_rank, stage)),
              ...
              if (in_flight_s2g > NUM_OF_IN_FLIGHT_S2G) {
                cuda::ptx::cp_async_bulk_wait_group_read(
                    cuda::ptx::n32_t<NUM_OF_IN_FLIGHT_S2G>{});
                in_flight_s2g -= 1;
...
// Device function for intra-node G2S warp for combine kernel. There can be only
// 1 such warp per CUDA block!
template <
    typename SMEM_TYPE,
    int NUM_OF_STAGES_G2S,
    int NUM_OF_TOKENS_PER_CHUNK,
    ...
    int NUM_OF_BLOCKS,
    bool BACKWARD_COMBINE>
__forceinline__ __device__ void intra_node_G2S_warp_group_device_function(
    const int node_rank,
    const int num_of_tokens_per_rank,
    const int num_of_ranks_per_node,
    const bool* rdma_to_attn_map,
  ...
  // The intra node reduction warp group of each CUDA block produce a chunk at a
  // time. The chunk order is: first produce the same chunk id for all other
  // nodes id, then produce following chunk id. (i.e. chunk 0 for node + 1, node
  // + 2, ... node - 1, then chunk 1 for node + 1, node + 2, ... node - 1) The
  // RDMA warp group of a CUDA block will consume the chunk by the same order.
  // So each CUDA block will produce and consume the same set of chunks id. The
  // reason to distribute chunk in this order is that the inter-node reduction
  // will need the same chunk id from all other nodes, so we need to produce and
  // send chunks in this order.
  ...
  // rdma_to_attn_map_load_t per node. The largest size of
  // rdma_to_attn_map_load_t allowed in all Hybrid-EP kernels are 16B(16 bools),
  // so need to be paded to 16B per node. That means the size of
  // rdma_to_attn_map should be rdma_to_attn_map_size_per_node * NUM_LSA_TEAMS.
  const int rdma_to_attn_map_size_per_node =
      (((num_of_tokens_per_rank - 1) / 16) + 1) * 16;
  ...
  // For a lane with global rank R among valid entries, its stage and parity
  // are:
  ...
  for (int i = blockIdx.x; i < total_num_of_chunks; i += NUM_OF_BLOCKS) {
    ...
    const bool* rdma_to_attn_map_load_base_addr = rdma_to_attn_map +
        (node_id * rdma_to_attn_map_size_per_node +
         chunk_id * NUM_OF_TOKENS_PER_CHUNK);

    const int32_t* sparse_to_dense_map_load_base_addr = sparse_to_dense_map +
        (node_id * num_of_tokens_per_rank +
         chunk_id * NUM_OF_TOKENS_PER_CHUNK) *
            num_of_ranks_per_node;

    // Iterate through all dst tokens within this chunk.
    for (int current_token_id = 0; current_token_id < current_chunk_size;
         current_token_id++) {
      // Check whether this dst token is needed by this node. If not needed,
      // just skip.
      bool token_needed_by_this_node =
          rdma_to_attn_map_load_base_addr[current_token_id];
      if (!token_needed_by_this_node) {
        continue;
      }

      const int32_t* sparse_to_dense_row = sparse_to_dense_map_load_base_addr +
          current_token_id * num_of_ranks_per_node;

      // First pass: count total valid ranks across all slices.
      int total_valid_count = 0;
      for (int rank_base = 0; rank_base < num_of_ranks_per_node;
           rank_base += WARP_SIZE) {
        ...
        const unsigned mask =
            __ballot_sync(0xffffffff, lane_active && s2d_val != -1);
      ...
      while (ranks_issued < total_valid_count) {
        ...
        for (int rank_base = 0; rank_base < num_of_ranks_per_node;
          ...
          const int rank_id = rank_base + lane_id;
          const bool lane_active = (rank_id < num_of_ranks_per_node);
          const int32_t s2d_val =
              lane_active ? sparse_to_dense_row[rank_id] : -1;
          const unsigned valid_mask =
              __ballot_sync(0xffffffff, lane_active && s2d_val != -1);
          const int slice_valid = __popc(valid_mask);
          const bool lane_valid = lane_active && s2d_val != -1;
          const int local_lane_rank =
              __popc(valid_mask & ((1u << lane_id) - 1));
          const int global_rank = slice_offset + local_lane_rank;
          const bool in_batch = lane_valid && global_rank >= ranks_issued &&
              global_rank < batch_end;
          ...
          if (in_batch) {
            ...
            const uint32_t parity =
                1u ^ ((uint32_t)(my_abs_offset / ring_len) & 1u);

            while (!cuda::ptx::mbarrier_try_wait_parity(
                smem_buffer_ptr->get_intra_node_mbarrier_G2S_consumer(
                    stage_idx),
                parity)) {
            }
            ...
            cuda::ptx::cp_async_bulk(
                ...
                cuda::ptx::space_global,
                reinterpret_cast<void*>(
                    smem_buffer_ptr->get_intra_node_token_G2S(stage_idx)),
                reinterpret_cast<const void*>(
                    rank_token_ptr + (s2d_val * HIDDEN_DIM)),
                token_bytes,
                smem_buffer_ptr->get_intra_node_mbarrier_G2S_producer(
                    stage_idx));
            ...
            if constexpr (BACKWARD_COMBINE) {
              ...
              cuda::ptx::cp_async_bulk(
                  ...
                  cuda::ptx::space_global,
                  reinterpret_cast<void*>(
                      smem_buffer_ptr->get_intra_node_prob_G2S(stage_idx)),
                  reinterpret_cast<const void*>(
                      rank_prob_ptr +
                      (s2d_val * (experts_per_rank * num_of_ranks_per_node))),
                  prob_bytes,
                  smem_buffer_ptr->get_intra_node_mbarrier_G2S_producer(
                      stage_idx));
            ...
            smem_buffer_ptr->intra_node_flag_G2S_buffer[stage_idx] =
                (global_rank == total_valid_count - 1);
...
__forceinline__ __device__ void intra_node_red_warp_group_device_function(
    ...
    const int HIDDEN_DIM,
    const int experts_per_rank) {
  // Vectorized loads from rdma_to_attn_map. Each destination token contributes
  // one bool.
  using rdma_to_attn_map_load_t = uint4;
  static_assert(
      sizeof(bool) == 1, "Routing map loads assume sizeof(bool) == 1");
  static_assert(
      NUM_OF_TOKENS_PER_CHUNK % sizeof(rdma_to_attn_map_load_t) == 0,
      "NUM_OF_TOKENS_PER_CHUNK must be multiple of rdma_to_attn_map_load_t.");
  constexpr int NUM_OF_RDMA_TO_ATTN_LOAD_ITER_PER_CHUNK =
      NUM_OF_TOKENS_PER_CHUNK / sizeof(rdma_to_attn_map_load_t);
  constexpr int NUM_OF_TOKENS_PER_RDMA_TO_ATTN_LOAD_ITER =
      sizeof(rdma_to_attn_map_load_t) / sizeof(bool);

  // Token values are processed as BF16x2 and accumulated in FP32. HIDDEN_DIM
  // must be even.

  const int NUM_OF_BF16X2_ELEMENTS_PER_TOKEN = HIDDEN_DIM / 2;
  // Keep a fixed-size prefix of each token in registers; spill the remaining
  // suffix to SMEM.
  constexpr int REG_HEAD_BF16X2_ELEMENTS_PER_TOKEN =
      COMBINE_REG_HEAD_HIDDEN_DIM / 2;
  ...
  constexpr int MAX_NUM_OF_PROB_VEC_ELEMENT_PER_THREAD =
      ((NUM_MAX_LOCAL_EXPERTS * LSA_TEAM_SIZE - 1) /
       INTRA_NODE_RED_GROUP::size()) +
      1;

  // This warp group emits chunks in the same per-destination order consumed by
  // the RDMA warp group: chunk 0 for node + 1, node + 2, ... node - 1, then
  // chunk 1 for node + 1, ... That ordering lets the downstream inter-node
  // stage observe matching chunk IDs across peers.
  ...
  const int total_num_of_chunks = (NUM_LSA_TEAMS - 1) * num_of_chunks_per_rank;
  // Pad each node's rdma_to_attn_map slice to one vector-load granularity (16
  // bytes / 16 bools).
  const int rdma_to_attn_map_size_per_node =
      (((num_of_tokens_per_rank - 1) / 16) + 1) * 16;
  ...
  // The counter is CUMULATIVE across all chunks (never reset), avoiding
  // inter-chunk races.
  constexpr int STREAMING_BATCH = HYBRIDEP_COMBINE_RDMA_STREAMING_BATCH;
  int streaming_pending =
      0; // tokens TMA-committed but not yet signaled to counter
  uint32_t cumulative_produced =
      0; // total active tokens whose TMA S2G is complete (across all chunks)
  ...
  for (int i = blockIdx.x; i < total_num_of_chunks; i += NUM_OF_BLOCKS) {
    ...
    int rdma_intra_node_red_id =
        rdma_remote_node_id * MAX_NUM_OF_TOKENS_PER_RANK +
        chunk_id * NUM_OF_TOKENS_PER_CHUNK;
    ...
    if (remainder_chunk_size != 0 && chunk_id == num_of_chunks_per_rank - 1) {
      ...
      num_of_routing_info_load_iter_for_current_chunk =
          NUM_OF_RDMA_TO_ATTN_LOAD_ITER_PER_CHUNK;
      current_chunk_size = NUM_OF_TOKENS_PER_CHUNK;
    }
    ...
    uint16_t* rdma_intra_node_red_token_base_ptr =
        rdma_intra_node_red_token + rdma_intra_node_red_id * HIDDEN_DIM;
    float* rdma_intra_node_red_prob_base_ptr;
    if constexpr (BACKWARD_COMBINE) {
      const int experts_per_node = experts_per_rank * num_of_ranks_per_node;
      rdma_intra_node_red_prob_base_ptr =
          rdma_intra_node_red_prob + rdma_intra_node_red_id * experts_per_node;
    }
    ...
    for (int j = 0; j < num_of_routing_info_load_iter_for_current_chunk; j++) {
      rdma_to_attn_map_load_t rdma_to_attn_map_data =
          rdma_to_attn_map_load_base_addr[j];
#pragma unroll
      for (int k = 0; k < NUM_OF_TOKENS_PER_RDMA_TO_ATTN_LOAD_ITER; k++) {
        ...
        bool token_needed_by_this_node =
            *(reinterpret_cast<bool*>(&rdma_to_attn_map_data) + k);
        // If so, one or more contributing source tokens are already being
        // staged through G2S.
        if (token_needed_by_this_node) {
          ...
          using acc_prob_storage_type = acc_prob_storage_t<
              BACKWARD_COMBINE,
              MAX_NUM_OF_PROB_VEC_ELEMENT_PER_THREAD>;
          ...
          // Producer marks the final contributor for this destination token
          // with this flag.
          bool last_src_token = false;
#pragma unroll
          for (int n = 0; n < MAX_NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD_INTRA;
               n++) {
...
#pragma unroll
          ...
          if (tail_bf16x2 > 0) {
            // Tail accumulation uses the per-stage S2G scratch buffer in shared
            // memory.
            acc_token_tail_smem4 = reinterpret_cast<float4*>(
                smem_buffer_ptr->get_intra_node_token_tail_S2G(
                    dst_token_stage));
          }

          // Consume source tokens for this destination token until the producer
          // marks the last one.
          do {
            // Current source token / optional prob slice in the G2S FIFO stage.
            __nv_bfloat162* load_token_base_ptr =
                reinterpret_cast<__nv_bfloat162*>(
                    smem_buffer_ptr->get_intra_node_token_G2S(token_stage));
            float* load_prob_base_ptr;
            if constexpr (BACKWARD_COMBINE) {
              load_prob_base_ptr =
                  smem_buffer_ptr->get_intra_node_prob_G2S(token_stage);
            }

            // Warp 0 waits for the producer; then the whole reduction group can
            // read this stage.
...
#pragma unroll
            for (int n = 0; n < MAX_NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD_INTRA;
                 n++) {
              int element_id = (n * INTRA_NODE_RED_GROUP::size()) +
                  INTRA_NODE_RED_GROUP::thread_rank();
            ...
            // Accumulate the token tail in shared memory to cap register usage
            // for large hidden dims.
            if (tail_bf16x2 > 0) {
              if (!tail_initialized) {
                ...
                for (; p + 7 * INTRA_NODE_RED_GROUP::size() < tail_float4;
                  ...
                  const int p2 = p + 2 * INTRA_NODE_RED_GROUP::size(),
                            p3 = p + 3 * INTRA_NODE_RED_GROUP::size();
                  const int p4 = p + 4 * INTRA_NODE_RED_GROUP::size(),
                            p5 = p + 5 * INTRA_NODE_RED_GROUP::size();
                  const int p6 = p + 6 * INTRA_NODE_RED_GROUP::size(),
                            p7 = p + 7 * INTRA_NODE_RED_GROUP::size();
                  const int e0 = head_bf16x2 + (p0 << 1),
                            e1 = head_bf16x2 + (p1 << 1);
                  const int e2 = head_bf16x2 + (p2 << 1),
                            e3 = head_bf16x2 + (p3 << 1);
                  const int e4 = head_bf16x2 + (p4 << 1),
                            e5 = head_bf16x2 + (p5 << 1);
                  const int e6 = head_bf16x2 + (p6 << 1),
                            e7 = head_bf16x2 + (p7 << 1);
                  __nv_bfloat162 s0a = load_token_base_ptr[e0],
                                 s0b = load_token_base_ptr[e0 + 1];
                  __nv_bfloat162 s1a = load_token_base_ptr[e1],
                                 s1b = load_token_base_ptr[e1 + 1];
                  __nv_bfloat162 s2a = load_token_base_ptr[e2],
                                 s2b = load_token_base_ptr[e2 + 1];
                  __nv_bfloat162 s3a = load_token_base_ptr[e3],
                                 s3b = load_token_base_ptr[e3 + 1];
                  __nv_bfloat162 s4a = load_token_base_ptr[e4],
                                 s4b = load_token_base_ptr[e4 + 1];
                  __nv_bfloat162 s5a = load_token_base_ptr[e5],
                                 s5b = load_token_base_ptr[e5 + 1];
                  __nv_bfloat162 s6a = load_token_base_ptr[e6],
                                 s6b = load_token_base_ptr[e6 + 1];
                  __nv_bfloat162 s7a = load_token_base_ptr[e7],
                                 s7b = load_token_base_ptr[e7 + 1];
                  float2 f0a = __bfloat1622float2(s0a),
                         f0b = __bfloat1622float2(s0b);
                  float2 f1a = __bfloat1622float2(s1a),
                         f1b = __bfloat1622float2(s1b);
                  float2 f2a = __bfloat1622float2(s2a),
                         f2b = __bfloat1622float2(s2b);
                  float2 f3a = __bfloat1622float2(s3a),
                         f3b = __bfloat1622float2(s3b);
                  float2 f4a = __bfloat1622float2(s4a),
                         f4b = __bfloat1622float2(s4b);
                  float2 f5a = __bfloat1622float2(s5a),
                         f5b = __bfloat1622float2(s5b);
                  float2 f6a = __bfloat1622float2(s6a),
                         f6b = __bfloat1622float2(s6b);
                  float2 f7a = __bfloat1622float2(s7a),
                         f7b = __bfloat1622float2(s7b);
                  acc_token_tail_smem4[p0] =
                      make_float4(f0a.x, f0a.y, f0b.x, f0b.y);
                  acc_token_tail_smem4[p1] =
                      make_float4(f1a.x, f1a.y, f1b.x, f1b.y);
                  acc_token_tail_smem4[p2] =
                      make_float4(f2a.x, f2a.y, f2b.x, f2b.y);
                  acc_token_tail_smem4[p3] =
                      make_float4(f3a.x, f3a.y, f3b.x, f3b.y);
                  acc_token_tail_smem4[p4] =
                      make_float4(f4a.x, f4a.y, f4b.x, f4b.y);
                  acc_token_tail_smem4[p5] =
                      make_float4(f5a.x, f5a.y, f5b.x, f5b.y);
                  acc_token_tail_smem4[p6] =
                      make_float4(f6a.x, f6a.y, f6b.x, f6b.y);
                  acc_token_tail_smem4[p7] =
                      make_float4(f7a.x, f7a.y, f7b.x, f7b.y);
                }
                for (; p < tail_float4; p += INTRA_NODE_RED_GROUP::size()) {
                  const int e = head_bf16x2 + (p << 1);
                  __nv_bfloat162 sa = load_token_base_ptr[e],
                                 sb = load_token_base_ptr[e + 1];
                  float2 fa = __bfloat1622float2(sa),
                         fb = __bfloat1622float2(sb);
                  acc_token_tail_smem4[p] = make_float4(fa.x, fa.y, fb.x, fb.y);
                }
                tail_initialized = true;
              } else {
                // Later contributors add into the existing SMEM tail
                // accumulator.
                int p = INTRA_NODE_RED_GROUP::thread_rank();
                for (; p + 7 * INTRA_NODE_RED_GROUP::size() < tail_float4;
                     p += 8 * INTRA_NODE_RED_GROUP::size()) {
...
#pragma unroll
              for (int n = 0; n < NUM_OF_PROB_VEC_ELEMENT_PER_THREAD; n++) {
                int prob_element_id = INTRA_NODE_RED_GROUP::thread_rank() +
                    n * INTRA_NODE_RED_GROUP::size();
                if (prob_element_id <
                    experts_per_rank * num_of_ranks_per_node) {
            ...
            // Producer sets this on the last source token for the current
            // destination token.
            last_src_token =
                smem_buffer_ptr->intra_node_flag_G2S_buffer[token_stage];

            // All reduction threads must finish consuming this G2S stage before
            // the producer reuses it.
            arrive_and_wait(INTRA_NODE_RED_GROUP::size(), 1);
            if (INTRA_NODE_RED_GROUP::warp_rank() == 0) {
              if (cuda::ptx::elect_sync(~0)) {
                cuda::ptx::mbarrier_arrive(
          ...
          // Current reduced destination token / optional prob slice in the S2G
          // FIFO stage.
          ...
          // Ensure any earlier TMA read from this S2G stage has completed
          // before we overwrite it.
          if (INTRA_NODE_RED_GROUP::warp_rank() == 0) {
            if (cuda::ptx::elect_sync(~0)) {
              cuda::ptx::cp_async_bulk_wait_group_read(
                  cuda::ptx::n32_t<NUM_OF_STAGES_S2G - 1>{});
            }
          }
          // All reduction threads wait here before storing new data into this
          // stage.
          arrive_and_wait(INTRA_NODE_RED_GROUP::size(), 1);
...
#pragma unroll
          for (int n = 0; n < MAX_NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD_INTRA;
            ...
            int element_id = (n * INTRA_NODE_RED_GROUP::size()) +
                INTRA_NODE_RED_GROUP::thread_rank();
            if (element_id < head_bf16x2) {
              store_token_base_ptr[element_id] =
                  __float22bfloat162_rn(acc_token_fp32[n]);
            }
          }
          // Store the token tail from the SMEM accumulator, or zeros if no tail
          // element was touched.
          if (tail_bf16x2 > 0) {
            const __nv_bfloat162 zero_bf16x2 =
            ...
            if (!tail_initialized) {
              ...
              for (; p + 7 * INTRA_NODE_RED_GROUP::size() < tail_float4;
                ...
                const int e6 = head_bf16x2 + (p6 << 1),
                          e7 = head_bf16x2 + (p7 << 1);
                store_token_base_ptr[e0] = zero_bf16x2;
                store_token_base_ptr[e0 + 1] = zero_bf16x2;
                store_token_base_ptr[e1] = zero_bf16x2;
                store_token_base_ptr[e1 + 1] = zero_bf16x2;
                store_token_base_ptr[e2] = zero_bf16x2;
                store_token_base_ptr[e2 + 1] = zero_bf16x2;
                store_token_base_ptr[e3] = zero_bf16x2;
                store_token_base_ptr[e3 + 1] = zero_bf16x2;
                store_token_base_ptr[e4] = zero_bf16x2;
                store_token_base_ptr[e4 + 1] = zero_bf16x2;
                store_token_base_ptr[e5] = zero_bf16x2;
                store_token_base_ptr[e5 + 1] = zero_bf16x2;
                store_token_base_ptr[e6] = zero_bf16x2;
                store_token_base_ptr[e6 + 1] = zero_bf16x2;
                store_token_base_ptr[e7] = zero_bf16x2;
                store_token_base_ptr[e7 + 1] = zero_bf16x2;
              }
              for (; p < tail_float4; p += INTRA_NODE_RED_GROUP::size()) {
                const int e = head_bf16x2 + (p << 1);
              ...
              for (; p + 7 * INTRA_NODE_RED_GROUP::size() < tail_float4;
                ...
                float4 acc6 = acc_token_tail_smem4[p6];
                float4 acc7 = acc_token_tail_smem4[p7];
                store_token_base_ptr[e0] =
                    __float22bfloat162_rn(make_float2(acc0.x, acc0.y));
                store_token_base_ptr[e0 + 1] =
                    __float22bfloat162_rn(make_float2(acc0.z, acc0.w));
                store_token_base_ptr[e1] =
                    __float22bfloat162_rn(make_float2(acc1.x, acc1.y));
                store_token_base_ptr[e1 + 1] =
                    __float22bfloat162_rn(make_float2(acc1.z, acc1.w));
                store_token_base_ptr[e2] =
                    __float22bfloat162_rn(make_float2(acc2.x, acc2.y));
                store_token_base_ptr[e2 + 1] =
                    __float22bfloat162_rn(make_float2(acc2.z, acc2.w));
                store_token_base_ptr[e3] =
                    __float22bfloat162_rn(make_float2(acc3.x, acc3.y));
                store_token_base_ptr[e3 + 1] =
                    __float22bfloat162_rn(make_float2(acc3.z, acc3.w));
                store_token_base_ptr[e4] =
                    __float22bfloat162_rn(make_float2(acc4.x, acc4.y));
                store_token_base_ptr[e4 + 1] =
                    __float22bfloat162_rn(make_float2(acc4.z, acc4.w));
                store_token_base_ptr[e5] =
                    __float22bfloat162_rn(make_float2(acc5.x, acc5.y));
                store_token_base_ptr[e5 + 1] =
                    __float22bfloat162_rn(make_float2(acc5.z, acc5.w));
                store_token_base_ptr[e6] =
                    __float22bfloat162_rn(make_float2(acc6.x, acc6.y));
                store_token_base_ptr[e6 + 1] =
                    __float22bfloat162_rn(make_float2(acc6.z, acc6.w));
                store_token_base_ptr[e7] =
                    __float22bfloat162_rn(make_float2(acc7.x, acc7.y));
                store_token_base_ptr[e7 + 1] =
                    __float22bfloat162_rn(make_float2(acc7.z, acc7.w));
              }
              for (; p < tail_float4; p += INTRA_NODE_RED_GROUP::size()) {
                const int e = head_bf16x2 + (p << 1);
                float4 acc = acc_token_tail_smem4[p];
                store_token_base_ptr[e] =
                    __float22bfloat162_rn(make_float2(acc.x, acc.y));
                store_token_base_ptr[e + 1] =
...
#pragma unroll
          ...
          // All threads must finish populating this S2G stage before the TMA
          // thread launches the copy.
          arrive_and_wait(INTRA_NODE_RED_GROUP::size(), 1);
            ...
            if (cuda::ptx::elect_sync(~0)) {
              uint16_t* current_token_addr =
                  ...
                  (j * NUM_OF_TOKENS_PER_RDMA_TO_ATTN_LOAD_ITER + k) *
                      HIDDEN_DIM;
              // Copy the reduced token from the S2G shared stage to the
              // per-destination global buffer.
              cuda::ptx::cp_async_bulk(
                  cuda::ptx::space_global,
                  cuda::ptx::space_shared,
                  reinterpret_cast<void*>(current_token_addr),
                  reinterpret_cast<const void*>(
                      smem_buffer_ptr->get_intra_node_token_S2G(
                          dst_token_stage)),
              ...
              if constexpr (BACKWARD_COMBINE) {
                ...
                cuda::ptx::cp_async_bulk(
                    ...
                    reinterpret_cast<const void*>(
                        smem_buffer_ptr->get_intra_node_prob_S2G(
                            dst_token_stage)),
                    (uint32_t)((experts_per_rank * num_of_ranks_per_node) *
                               sizeof(float)));
              }
              // Group the token/prob copies for this destination token into one
              // async-copy commit.
              cuda::ptx::cp_async_bulk_commit_group();
            }
          }
            ...
            if (streaming_pending >= STREAMING_BATCH) {
              if (INTRA_NODE_RED_GROUP::warp_rank() == 0) {
                if (cuda::ptx::elect_sync(~0)) {
                  ...
                  // Volatile store instead of atomicExch: same block, shared
                  // memory is not cached on sm_90. __threadfence_block()
                  // ensures TMA S2G writes are visible before the counter
                  // update.
                  cumulative_produced += streaming_pending;
                  __threadfence_block();
                  *((volatile uint32_t*)
                        smem_buffer_ptr->rdma_streaming_counter) =
                      cumulative_produced;
                }
              }
    ...
    if (streaming_pending > 0 || additional_in_flight_s2g > 0) {
      if (INTRA_NODE_RED_GROUP::warp_rank() == 0) {
        if (cuda::ptx::elect_sync(~0)) {
          ...
          if constexpr (STREAMING_BATCH > 0) {
            cumulative_produced += streaming_pending;
            __threadfence(); // device scope: flush L2→VRAM for NIC visibility
                             // (GDR)
    ...
    if constexpr (NUM_LSA_TEAMS != 1) {
      if (INTRA_NODE_RED_GROUP::warp_rank() == 0) {
        if (cuda::ptx::elect_sync(~0)) {
          cuda::ptx::mbarrier_arrive(
              &smem_buffer_ptr->intra_node_to_rdma_mbarrier_buffer
                   [rdma_remote_node_id * MAX_NUM_OF_CHUNKS_PER_RANK +
                    chunk_id]);
        }
      }
    }
  ...
  // No post-loop cleanup needed: every chunk is fully drained and signaled at
  // chunk end.
}

// Device function for inter-node node2node(RDMA) warp for combine kernel. There
// can be only 1 inter-node warp per CUDA block! Uses ncclGin API (net.put,
// net.signal)
template <
...
__forceinline__ __device__ void inter_node_N2N_warp_group_device_function(
    ...
    const int HIDDEN_DIM,
    const int experts_per_rank) {
  // Load rdma_to_attn_map using LDG.128. Each token will need 1 bool from this
  // map.
  using rdma_to_attn_map_load_t = uint4;
  static_assert(
      sizeof(bool) == 1, "Routing map loads assume sizeof(bool) == 1");
  static_assert(
      ...
      "INTER_NODE_RDMA_GROUP should be 1 warp.");
  static_assert(
      INTER_NODE_RDMA_GROUP::size() >= NUM_LSA_TEAMS - 1,
      "mr_info should be loaded at once.");
  static_assert(
  ...
  // The inter_node_N2N_warp should process the same chunk as
  // intra_node_red_warp(They belong to the same block.)
  uint32_t token_consumer_parity = 0;
  uint32_t cumulative_sent = 0; // Cumulative count of active tokens RDMA-put
                                // across all chunks (never reset)
  // Loop for every chunks.
  for (int i = blockIdx.x; i < TOTAL_NUM_OF_CHUNKS; i += NUM_OF_BLOCKS) {
    // Which node this chunk will be sent to.
    int node_id = (i % (NUM_LSA_TEAMS - 1) + (node_rank + 1)) % NUM_LSA_TEAMS;
    // With split comm (nNodes ranks), node index IS the rank in the
    // communicator
    ...
    ncclTeam world = ncclTeamWorld(dcomms[comm_idx]);
    int rdma_remote_node_id = node_id > node_rank ? node_id - 1 : node_id;
    int chunk_base_token_idx = node_id * rdma_to_attn_map_size_per_node +
        chunk_id * NUM_OF_TOKENS_PER_CHUNK;
    int token_range = NUM_OF_TOKENS_PER_CHUNK;
    if (chunk_id * NUM_OF_TOKENS_PER_CHUNK + token_range >
    ...
    if constexpr (STREAMING_BATCH > 0) {
      ...
      // ---- cumulative_sent tracks total active tokens across all chunks (no
      // reset).
      ...
      if (INTER_NODE_RDMA_GROUP::thread_rank() == 0) {
        ...
        for (int token_idx_in_chunk = 0; token_idx_in_chunk < token_range;
             ++token_idx_in_chunk) {
          bool need_write =
              rdma_to_attn_map[token_idx_in_chunk + chunk_base_token_idx];
          bool is_last = (token_idx_in_chunk == token_range - 1);

          if (need_write) {
            if (batch_count == 0)
              batch_start_in_chunk = token_idx_in_chunk;
            batch_count++;
          }
          ...
          if (should_flush) {
            ...
            int batch_start_token =
                batch_start_in_chunk + chunk_id * NUM_OF_TOKENS_PER_CHUNK;
            ...
            size_t token_dst_offset =
                ...
                (rank_in_remote * MAX_NUM_OF_TOKENS_PER_RANK +
                 batch_start_token) *
                    HIDDEN_DIM * sizeof(uint16_t);
            net.put(
                world,
            ...
            if constexpr (BACKWARD_COMBINE) {
              ...
              net.put(
                  ...
                  nccl_window,
                  prob_src_offset,
                  batch_count * (experts_per_rank * num_of_ranks_per_node) *
                      sizeof(float),
                  ncclGin_None{},
                  ncclGin_None{},
                  ncclCoopThread());
...
template <
    ...
    int NUM_OF_TOKENS_PER_GROUP,
    bool BACKWARD_COMBINE>
__forceinline__ __device__ void inter_node_G2S_warp_group_device_function(
    const int local_rank,
    const int node_rank,
    const int num_of_tokens_per_rank,
    const int num_of_ranks_per_node,
    ...
    const int HIDDEN_DIM,
    const int experts_per_rank) {
  // The warps from inter-node G2S warp group will be divided into multiple
  // independent pipeline. Each pipeline can only have 1 warp, so
  // INTER_NODE_G2S_GROUP::warp_size() == NUM_OF_DATA_PIPELINE_PER_BLOCK and
  // warp has the same meaning as pipeline in inter-node G2S warp group. Number
  // of pipeline should match inter-node red warp group, so they can coupled
  // into multiple independent data pipeline within a CUDA block. Evenly
  // distribute the inter-node G2S FIFO to every pipeline(warp) within the
  // inter-node G2S warp group. When inter-node G2S warp group only has 1 warp,
  // then the algorith is the same as old version(1 pipeline per CUDA block).
  static_assert(
  ...
  constexpr int NUM_OF_STAGES_G2S_PER_WARP =
      NUM_OF_STAGES_G2S / INTER_NODE_G2S_GROUP::warp_size();
  // All chunks in output buffer(attn buffer) will be divided into token groups
  // and assigned to different CUDA blocks. This is different than other
  // functions where chunks are assigned to different CUDA blocks.
  static_assert(
      NUM_OF_TOKENS_PER_CHUNK % NUM_OF_TOKENS_PER_GROUP == 0,
      "NUM_OF_TOKENS_PER_CHUNK must be multiple of NUM_OF_TOKENS_PER_GROUP.");
  constexpr int NUM_OF_TOKEN_GROUPS_PER_CHUNK =
      NUM_OF_TOKENS_PER_CHUNK / NUM_OF_TOKENS_PER_GROUP;

  static_assert(
      sizeof(bool) == 1, "Routing map loads assume sizeof(bool) == 1");

  // The inter node reduction warp group of each CUDA block produce a token
  // group of a chunk at a time. Token groups of each chunk assigned to each
  // CUDA block in interleave pattern. The chunk order is: i.e. chunk 0, then
  // chunk 1, ... the last chunk of attn output buffer. The RDMA network for
  // current rank will produce the same chunk id from node - 1, node - 2 ...
  // node + 1. So inter node reduction warp group will consume the src chunk in
  // the same order.
  ...
  const int max_num_of_chunks_per_rank =
      ((MAX_NUM_OF_TOKENS_PER_RANK - 1) / NUM_OF_TOKENS_PER_CHUNK) + 1;
  // Total number of chunks to process in the output buffer(attn buffer). output
  // buffer(attn buffer) will only have 1 rank's tokens.
  ...
  const int starting_G2S_index =
      NUM_OF_STAGES_G2S_PER_WARP * INTER_NODE_G2S_GROUP::warp_rank();
  const int ending_G2S_index =
      NUM_OF_STAGES_G2S_PER_WARP * (INTER_NODE_G2S_GROUP::warp_rank() + 1);
  if constexpr (NUM_LSA_TEAMS == 1) {
    // Single-node optimized path: warp-cooperative G2S.
    // Lanes load sparse_to_dense_map in parallel (up to 32 ranks per warp
    // pass), then valid lanes issue TMA to different stages simultaneously. RED
    // processes stages sequentially.
    //
    // Parity protocol: G2S tracks a "global_offset" counting total stages
    // filled. For a lane with global rank R among valid entries, its stage and
    // parity are:
    ...
    // Each slice's valid lanes get a global_rank = slice_offset +
    // local_lane_rank. The last-token flag is set only by the globally last
    // valid lane.
    constexpr int WARP_SIZE = 32;
    const int lane_id = (int)(threadIdx.x & (WARP_SIZE - 1));
    ...
    for (int i = 0; i < total_num_of_chunks; i++) {
      ...
      int current_chunk_size;
      if (remainder_chunk_size != 0 && i == num_of_chunks_per_rank - 1) {
        num_of_token_groups_for_current_chunk =
            ((remainder_chunk_size - 1) / NUM_OF_TOKENS_PER_GROUP) + 1;
        current_chunk_size = remainder_chunk_size;
      } else {
      ...
      const bool* rdma_to_attn_map_load_base_addr = rdma_to_attn_map +
          (node_rank * rdma_to_attn_map_size_per_node +
           i * NUM_OF_TOKENS_PER_CHUNK);
      const int32_t* sparse_to_dense_map_load_base_addr = sparse_to_dense_map +
          (node_rank * num_of_tokens_per_rank + i * NUM_OF_TOKENS_PER_CHUNK) *
              num_of_ranks_per_node;

      for (int j = blockIdx.x; j < num_of_token_groups_for_current_chunk;
           j += NUM_OF_BLOCKS) {
        for (int k = INTER_NODE_G2S_GROUP::warp_rank();
             k < NUM_OF_TOKENS_PER_GROUP;
          ...
          const int32_t* sparse_to_dense_row =
              sparse_to_dense_map_load_base_addr +
              (j * NUM_OF_TOKENS_PER_GROUP + k) * num_of_ranks_per_node;
          ...
          while (ranks_issued < total_valid_count) {
            ...
            for (int rank_base = 0; rank_base < num_of_ranks_per_node;
              ...
              if (in_batch) {
                ...
                const int my_abs_offset = global_offset + rank_in_batch;
                const int stage_idx =
                    starting_G2S_index + (my_abs_offset % ring_len);
                const uint32_t parity =
                    1u ^ ((uint32_t)(my_abs_offset / ring_len) & 1u);

                while (!cuda::ptx::mbarrier_try_wait_parity(
                    smem_buffer_ptr->get_inter_node_mbarrier_G2S_consumer(
                        stage_idx),
                    parity)) {
                }

                const uint16_t* rank_token_ptr =
                    remote_expert_input_token[rank_id];
                uint32_t total_tx_size = 0;
                cuda::ptx::cp_async_bulk(
                    cuda::ptx::space_shared,
                    cuda::ptx::space_global,
                    reinterpret_cast<void*>(
                        smem_buffer_ptr->get_inter_node_token_G2S(stage_idx)),
                    reinterpret_cast<const void*>(
                        rank_token_ptr + (s2d_val * HIDDEN_DIM)),
                    token_bytes,
                    smem_buffer_ptr->get_inter_node_mbarrier_G2S_producer(
                        stage_idx));

                total_tx_size += token_bytes;

                if constexpr (BACKWARD_COMBINE) {
                  const float* rank_prob_ptr =
                      remote_expert_input_prob[rank_id];
                  cuda::ptx::cp_async_bulk(
                      cuda::ptx::space_shared,
                      cuda::ptx::space_global,
                      reinterpret_cast<void*>(
                          smem_buffer_ptr->get_inter_node_prob_G2S(stage_idx)),
                      reinterpret_cast<const void*>(
                          rank_prob_ptr +
                          (s2d_val *
                           (experts_per_rank * num_of_ranks_per_node))),
                ...
                smem_buffer_ptr->inter_node_flag_G2S_buffer[stage_idx] =
                    (global_rank == total_valid_count - 1);
    ...
    // Multi-node warp-cooperative path: local block uses full warp (same as
    // NUM_LSA_TEAMS==1), then RDMA block runs on lane 0 only, sharing the same
    // global_offset stream. After RDMA, global_offset is broadcast back to all
    // lanes via __shfl_sync.
    constexpr int WARP_SIZE = 32;
    const int lane_id = (int)(threadIdx.x & (WARP_SIZE - 1));
    const int ring_len = ending_G2S_index - starting_G2S_index;
    const uint32_t token_bytes = (uint32_t)(HIDDEN_DIM * sizeof(uint16_t));
    ...
    for (int i = 0; i < total_num_of_chunks; i++) {
      ...
      if constexpr (NUM_LSA_TEAMS > 1) {
        attn_to_rdma_map_load_base_addr = attn_to_rdma_map +
            (i * NUM_OF_TOKENS_PER_CHUNK) * (NUM_LSA_TEAMS - 1);

        // Pre-wait for ALL remote signals for this chunk before processing any
        // tokens. This guarantees RDMA data is available for all tokens in this
        // chunk, eliminating the per-token signal-wait overhead inside the
        // token loop.
        if (lane_id == 0) {
          constexpr int MAX_CHUNKS_PER_RANK =
              MAX_NUM_OF_TOKENS_PER_RANK / NUM_OF_TOKENS_PER_CHUNK;
          ...
          for (int n = 1; n < NUM_LSA_TEAMS; n++) {
            int node_id_for_signal =
                node_rank >= n ? node_rank - n : node_rank + NUM_LSA_TEAMS - n;
            unsigned signal_id = signals_base + combine_signal_offset +
                local_rank * (NUM_LSA_TEAMS * MAX_CHUNKS_PER_RANK) +
...
#pragma unroll
      ...
      // Iterate through all token groups within this chunk which assign to this
      // CUDA block.
      for (int j = blockIdx.x; j < num_of_token_groups_for_current_chunk;
           j += NUM_OF_BLOCKS) {
        // Iterate through all dst(output) tokens within this token group.
        // Assign each dst token to each G2S warp(pipeline) using a round-robin
        // fasion.
        for (int k = INTER_NODE_G2S_GROUP::warp_rank();
             k < NUM_OF_TOKENS_PER_GROUP;
             k += INTER_NODE_G2S_GROUP::warp_size()) {
          ...
          // Each dst token need to accumulate src tokens from local node's
          // ranks, and src tokens from rdma inter-node buffers. Accumulate
          // local tokens first (warp-cooperative), then rdma tokens (lane 0
          // only).

          // Check whether this dst token is needed by this(local) node. If not
          // needed, skip local accumulation only. NOTE: Do NOT use 'continue'
          // here -- RDMA block must still run even when
          // token_needed_by_this_node is false.
          bool token_needed_by_this_node =
              rdma_to_attn_map_load_base_addr[current_token_id];
          if (token_needed_by_this_node) {
            const int32_t* sparse_to_dense_row =
              ...
              while (ranks_issued < total_valid_count) {
                ...
                for (int rank_base = 0; rank_base < num_of_ranks_per_node;
                  ...
                  const int local_lane_rank =
                      __popc(valid_mask & ((1u << lane_id) - 1));
                  const int global_rank = slice_offset + local_lane_rank;
                  const bool in_batch = lane_valid &&
                      global_rank >= ranks_issued && global_rank < batch_end;
                  ...
                  if (in_batch) {
                    ...
                    cuda::ptx::cp_async_bulk(
                        ...
                        cuda::ptx::space_global,
                        reinterpret_cast<void*>(
                            smem_buffer_ptr->get_inter_node_token_G2S(
                                stage_idx)),
                        reinterpret_cast<const void*>(
                            rank_token_ptr + (s2d_val * HIDDEN_DIM)),
                        token_bytes,
                    ...
                    if constexpr (BACKWARD_COMBINE) {
                      ...
                      cuda::ptx::cp_async_bulk(
                          ...
                          cuda::ptx::space_global,
                          reinterpret_cast<void*>(
                              smem_buffer_ptr->get_inter_node_prob_G2S(
                                  stage_idx)),
                          reinterpret_cast<const void*>(
                              rank_prob_ptr +
                              (s2d_val *
          ...
          if constexpr (NUM_LSA_TEAMS > 1) {
            // Warp-cooperative RDMA: each lane maps to a remote node (lane_id
            // -> n = lane_id + 1). Valid lanes issue TMAs in parallel to
            // different stages. Signal waits remain on lane 0 only (ncclGin
            // safety). Since NUM_LSA_TEAMS <= 33, a single warp pass covers all
            // remote nodes.
            static_assert(
                NUM_LSA_TEAMS <= 33,
            ...
            const bool* attn_to_rdma_map_load_addr =
                attn_to_rdma_map_load_base_addr +
                (j * NUM_OF_TOKENS_PER_GROUP + k) * (NUM_LSA_TEAMS - 1);
            ...
            if (rdma_lane_active) {
              rdma_node_id = node_rank >= rdma_n
                  ? node_rank - rdma_n
                  : node_rank + NUM_LSA_TEAMS - rdma_n;
              rdma_buffer_tile_id =
                  rdma_node_id > node_rank ? rdma_node_id - 1 : rdma_node_id;
              rdma_entry_valid =
                  attn_to_rdma_map_load_addr[rdma_buffer_tile_id];
            }

            // Count valid RDMA entries -- identical for all 32 lanes.
            const unsigned rdma_valid_mask =
                __ballot_sync(0xffffffff, rdma_lane_active && rdma_entry_valid);
            const int rdma_valid_count = __popc(rdma_valid_mask);

            if (rdma_valid_count > 0) {
              // Signal already pre-waited at chunk start (before per-token
              // loop). Proceed directly to warp-cooperative TMA load. Each
              // valid lane computes its rank among valid entries (ascending
              // lane_id = ascending n order).
              const int rdma_local_rank =
                  __popc(rdma_valid_mask & ((1u << lane_id) - 1));
              ...
              if (rdma_lane_active && rdma_entry_valid) {
                ...
                // When rdma_valid_count > ring_len, overflow ranks block here
                // until RED consumes earlier stages (parity protocol).
                while (!cuda::ptx::mbarrier_try_wait_parity(
                    smem_buffer_ptr->get_inter_node_mbarrier_G2S_consumer(
                        stage_idx),
                    parity)) {
                ...
                // Load the src token from this rdma inter-node group buffer
                // chunk to shared memory entry.
                uint32_t total_tx_size = 0;
                const uint16_t* rdma_inter_node_group_token_load_addr =
                    rdma_inter_node_group_token +
                    (rdma_buffer_tile_id * MAX_NUM_OF_TOKENS_PER_RANK +
                     i * NUM_OF_TOKENS_PER_CHUNK + j * NUM_OF_TOKENS_PER_GROUP +
                     k) *
                        HIDDEN_DIM;
                cuda::ptx::cp_async_bulk(
                    cuda::ptx::space_shared,
                    ...
                    reinterpret_cast<void*>(
                        smem_buffer_ptr->get_inter_node_token_G2S(stage_idx)),
                    reinterpret_cast<const void*>(
                        rdma_inter_node_group_token_load_addr),
                    token_bytes,
                    smem_buffer_ptr->get_inter_node_mbarrier_G2S_producer(
                        stage_idx));
                ...
                if constexpr (BACKWARD_COMBINE) {
                  const float* rdma_inter_node_group_prob_load_addr =
                  ...
                  cuda::ptx::cp_async_bulk(
                      ...
                      reinterpret_cast<void*>(
                          smem_buffer_ptr->get_inter_node_prob_G2S(stage_idx)),
                      reinterpret_cast<const void*>(
                          rdma_inter_node_group_prob_load_addr),
                      prob_bytes,
                      smem_buffer_ptr->get_inter_node_mbarrier_G2S_producer(
                          stage_idx));
                ...
                // Inter-node token does not need flag since the red warp group
                // will also read attn_to_rdma_map.
                cuda::ptx::mbarrier_arrive_expect_tx(
                    cuda::ptx::sem_release,
                    cuda::ptx::scope_cta,
                    cuda::ptx::space_shared,
              ...
              // rdma_valid_count is identical for all 32 lanes (from __popc of
              // same __ballot_sync result).
  ...
  if constexpr (NUM_LSA_TEAMS > 1) {
    ...
    for (int node_id = blockIdx.x; node_id < NUM_LSA_TEAMS - 1;
      ...
      uint64_t* residue_flag_base_ptr = rdma_inter_node_group_flags +
          (node_id * max_num_of_chunks_per_rank + num_of_chunks_per_rank);
      for (int flag_id = INTER_NODE_G2S_GROUP::thread_rank();
           flag_id < residue_flag_count;
...
__forceinline__ __device__ void inter_node_red_warp_group_device_function(
    ...
    const int HIDDEN_DIM,
    const int experts_per_rank) {
  // The warps from inter-node red warp group will be divided into multiple
  // independent pipeline. Each pipeline has INTER_NODE_RED_GROUP::warp_size() /
  // NUM_OF_DATA_PIPELINE_PER_BLOCK warps. Number of pipeline should match
  // inter-node G2S warp group, so they can coupled into multiple independent
  // data pipeline within a CUDA block.
  static_assert(
  ...
  constexpr int NUM_OF_THREADS_PER_PIPELINE =
      (INTER_NODE_RED_GROUP::warp_size() / NUM_OF_DATA_PIPELINE_PER_BLOCK) *
      WARP_SIZE;
  // Evenly distribute the inter-node G2S FIFO to every pipeline within the
  // inter-node red warp group. When NUM_OF_DATA_PIPELINE_PER_BLOCK = 1 and
  // INTER_NODE_RED_GROUP::warp_size() = 4, then the algorith is the same as old
  // version(1 pipeline w/ 4 warps per CUDA block).
  static_assert(
      NUM_OF_STAGES_G2S % NUM_OF_DATA_PIPELINE_PER_BLOCK == 0,
      "NUM_OF_STAGES_G2S must be multiple of data pipeline per CUDA block.");
  constexpr int NUM_OF_STAGES_G2S_PER_PIPELINE =
      NUM_OF_STAGES_G2S / NUM_OF_DATA_PIPELINE_PER_BLOCK;
  // Evenly distribute the inter-node S2G FIFO to every pipeline within the
  // inter-node red warp group.
  static_assert(
      NUM_OF_STAGES_S2G % NUM_OF_DATA_PIPELINE_PER_BLOCK == 0,
      "NUM_OF_STAGES_S2G must be multiple of data pipeline per CUDA block.");
  constexpr int NUM_OF_STAGES_S2G_PER_PIPELINE =
      NUM_OF_STAGES_S2G / NUM_OF_DATA_PIPELINE_PER_BLOCK;
  // All chunks in output buffer(attn buffer) will be divided into token groups
  // and assigned to different CUDA blocks. This is different than other
  // functions where chunks are assigned to different CUDA blocks.
  static_assert(
  ...
  // The head fits in registers per thread (bf16x2 lanes), giving a fixed
  // per-thread workload and avoiding large register arrays when HIDDEN_DIM is
  // large. The tail (if any) is handled separately (float4 = 2 bf16x2) from
  // shared memory.
  constexpr int NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD =
      ((REG_HEAD_BF16X2_ELEMENTS_PER_TOKEN - 1) / NUM_OF_THREADS_PER_PIPELINE) +
      1;
  const int head_bf16x2 =
      NUM_OF_BF16X2_ELEMENTS_PER_TOKEN > REG_HEAD_BF16X2_ELEMENTS_PER_TOKEN
      ? REG_HEAD_BF16X2_ELEMENTS_PER_TOKEN
  ...
  const int ending_G2S_index =
      NUM_OF_STAGES_G2S_PER_PIPELINE * (pipeline_rank + 1);
  ...
  const int ending_S2G_index =
      NUM_OF_STAGES_S2G_PER_PIPELINE * (pipeline_rank + 1);
  // Dst token stage id.
  ...
  for (int i = 0; i < total_num_of_chunks; i++) {
    ...
    int current_chunk_size;
    if (remainder_chunk_size != 0 &&
        i == num_of_chunks_per_rank - 1) { // tail processing
      num_of_token_groups_for_current_chunk =
          ((remainder_chunk_size - 1) / NUM_OF_TOKENS_PER_GROUP) + 1;
      current_chunk_size = remainder_chunk_size;
    } else {
    ...
    uint16_t* attn_output_token_base_ptr =
        attn_output_token + (i * NUM_OF_TOKENS_PER_CHUNK) * HIDDEN_DIM;
    float* attn_output_prob_base_ptr;
    if constexpr (BACKWARD_COMBINE) {
      attn_output_prob_base_ptr = attn_output_prob +
          (i * NUM_OF_TOKENS_PER_CHUNK) *
              (experts_per_rank * num_of_ranks_per_node * NUM_LSA_TEAMS);
    }
    // Iterate through all token groups within this chunk which assign to this
    ...
    for (int j = blockIdx.x; j < num_of_token_groups_for_current_chunk;
      ...
      for (int k = pipeline_rank; k < NUM_OF_TOKENS_PER_GROUP;
           k += NUM_OF_DATA_PIPELINE_PER_BLOCK) {
        int current_token_id = j * NUM_OF_TOKENS_PER_GROUP + k;
        // If the current token is out-of-bound, then just end this load iter.
        if (current_token_id >= current_chunk_size) {
          break;
        ...
        // Each dst token need to accumulate src tokens from local node's
        // ranks(this part is the same as intra-node reduction), and src tokens
        // from rdma inter-node buffers. Accumulate local tokens first, then
        // rdma tokens. Accumulator for this dst token. Token must be
        // accumulated in FP32.
        ...
        // 0 used for local node's prob, [1, NUM_LSA_TEAMS - 1] used for remote
        // node's prob. Flattened array: acc_prob_ptr[n *
        // NUM_OF_PROB_VEC_ELEMENT_PER_THREAD + m] for 2D access Use MAX size
        // for compile-time array allocation, actual size determined by runtime
        // experts_per_rank
        using acc_prob_storage_type = acc_prob_storage_t<
            BACKWARD_COMBINE,
...
#pragma unroll
        ...
        // Check whether this dst token is needed by this(local) node. If not
        // needed, just skip local accumulation.
        bool token_needed_by_this_node =
            rdma_to_attn_map_load_base_addr[current_token_id];
        // If this dst token is needed by this node, load the local src token
        // from shared memory and accumulate them.
        if (token_needed_by_this_node) {
          // End reduction group flag.
          bool last_local_node_src_token = false;
          // Continue loading local src token for this dst token and reduce them
          // to accumulator until all local src token for this dst token have
          // been accumulated.
          do {
            // Base address for current token and prob(optional) in shared
            // memory.
            __nv_bfloat162* load_token_base_ptr =
                reinterpret_cast<__nv_bfloat162*>(
            ...
            if constexpr (BACKWARD_COMBINE) {
              load_prob_base_ptr =
                  smem_buffer_ptr->get_inter_node_prob_G2S(token_stage);
            }
            ...
            // named barrier: we wait for number of threads(all threads in the
            // pipline) that must arrive before any can proceed
            arrive_and_wait(NUM_OF_THREADS_PER_PIPELINE, 2 + pipeline_rank);

// Accumulate token and prob(optional).
#pragma unroll
            for (int n = 0; n < NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD; n++) {
              int element_id = (n * NUM_OF_THREADS_PER_PIPELINE) +
            ...
            if (tail_bf16x2 > 0) {
              if (!tail_initialized) {
                // First iteration: load source directly into smem (no
                // accumulation)
                ...
                for (; p + 7 * NUM_OF_THREADS_PER_PIPELINE < tail_float4;
                  ...
                  const int p2 = p + 2 * NUM_OF_THREADS_PER_PIPELINE,
                            p3 = p + 3 * NUM_OF_THREADS_PER_PIPELINE;
                  const int p4 = p + 4 * NUM_OF_THREADS_PER_PIPELINE,
                            p5 = p + 5 * NUM_OF_THREADS_PER_PIPELINE;
                  const int p6 = p + 6 * NUM_OF_THREADS_PER_PIPELINE,
                            p7 = p + 7 * NUM_OF_THREADS_PER_PIPELINE;
                  const int e0 = head_bf16x2 + (p0 << 1),
                            e1 = head_bf16x2 + (p1 << 1);
                  const int e2 = head_bf16x2 + (p2 << 1),
                            e3 = head_bf16x2 + (p3 << 1);
                ...
                // Subsequent iterations: load, add to smem accumulator, store
                // back
                int p = thread_rank_within_pipeline;
                for (; p + 7 * NUM_OF_THREADS_PER_PIPELINE < tail_float4;
                     p += 8 * NUM_OF_THREADS_PER_PIPELINE) {
...
#pragma unroll
              for (int n = 0; n < NUM_OF_PROB_VEC_ELEMENT_PER_THREAD; n++) {
                int element_id = thread_rank_within_pipeline +
                    n * NUM_OF_THREADS_PER_PIPELINE;
                if (element_id < experts_per_rank * num_of_ranks_per_node) {
                  float src_data = load_prob_base_ptr[element_id];
                  acc_prob_ptr[0 * NUM_OF_PROB_VEC_ELEMENT_PER_THREAD + n] +=
                      src_data;
                }
              }
            }
            ...
            last_local_node_src_token =
                smem_buffer_ptr->inter_node_flag_G2S_buffer[token_stage];

            // Make sure all threads within the pipeline have finished loading
            // the token entry and accumulate it to the register accumulator.
            // Then notify the producer warp to load next token entry to the
            // shared memory as the shared memory can be reused.
            arrive_and_wait(NUM_OF_THREADS_PER_PIPELINE, 2 + pipeline_rank);
            if (warp_rank_within_pipeline == 0) {
              if (cuda::ptx::elect_sync(~0)) {
                cuda::ptx::mbarrier_arrive(
        ...
        if constexpr (NUM_LSA_TEAMS > 1) {
          // Then accumulate from rdma inter-node buffers. There are total
          // NUM_LSA_TEAMS - 1 (possible) src tokens from rdma buffer to reduce.
          const bool* attn_to_rdma_map_load_addr =
              attn_to_rdma_map_load_base_addr +
              (j * NUM_OF_TOKENS_PER_GROUP + k) * (NUM_LSA_TEAMS - 1);
#pragma unroll
          for (int n = 1; n < NUM_LSA_TEAMS; n++) {
            // The current node been processed. For each chunk id, node_id order
            // is (no local_node itself, which is already been accumulated
            // above) local_node - 1, local_node - 2, ......, local_node + 1 and
            // will wrap around.
            int node_id =
                node_rank >= n ? node_rank - n : node_rank + NUM_LSA_TEAMS - n;
            // The tile id within the rdma buffers(include attn_to_rdma map) for
            // the current node id. Because these rdma buffers only have
            // NUM_LSA_TEAMS - 1 tile or element.
            int rdma_buffer_tile_id =
                node_id > node_rank ? node_id - 1 : node_id;
            // Check wether current dst token need src token from this (remote)
            // node.
            if (attn_to_rdma_map_load_addr[rdma_buffer_tile_id]) {
              ...
              if (warp_rank_within_pipeline ==
                  0) { // this means that only wrap 0 in the pipeline
                       // participates
                if (cuda::ptx::elect_sync(~0)) {
                  while (!cuda::ptx::mbarrier_try_wait_parity(
                      smem_buffer_ptr->get_inter_node_mbarrier_G2S_producer(
              ...
              arrive_and_wait(
                  NUM_OF_THREADS_PER_PIPELINE,
                  2 + pipeline_rank); // named barrier, we wait for number of
                                      // threads that must arrive before any can
                                      // proceed

// Accumulate token and prob(optional).
#pragma unroll
              for (int m = 0; m < NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD; m++) {
                int element_id = (m * NUM_OF_THREADS_PER_PIPELINE) +
                    thread_rank_within_pipeline;
                if (element_id < head_bf16x2) {
                  __nv_bfloat162 src_data = load_token_base_ptr[element_id];
                  float2 src_data_fp32 = __bfloat1622float2(src_data);
...
#pragma unroll
        ...
        // Because each attn token must have go to TOPK rank in dispatch, so it
        // must have been reduced in combine. So each attn dst token must be
        // written back. Base address for current dst token and prob(optional)
        // in shared memory.
        __nv_bfloat162* store_token_base_ptr =
            reinterpret_cast<__nv_bfloat162*>(
                smem_buffer_ptr->get_inter_node_token_S2G(dst_token_stage));
        float* store_prob_base_ptr;
        if constexpr (BACKWARD_COMBINE) {
          store_prob_base_ptr =
        ...
        // Select the TMA thread within the pipeline to wait for previously
        // issued TMA S2G operations finish reading this entry.
        if (warp_rank_within_pipeline == 0) {
          if (cuda::ptx::elect_sync(~0)) {
            cuda::ptx::cp_async_bulk_wait_group_read(
        ...
        // Make sure all threads within the pipeline have wait for previously
        // issued TMA S2G operations finish reading this entry before storing
        // new data to this entry.
        arrive_and_wait(NUM_OF_THREADS_PER_PIPELINE, 2 + pipeline_rank);
...
#pragma unroll
        for (int n = 0; n < NUM_OF_HEAD_ACC_ELEMENTS_PER_THREAD; n++) {
          int element_id =
              (n * NUM_OF_THREADS_PER_PIPELINE) + thread_rank_within_pipeline;
          if (element_id < head_bf16x2) {
            // Convert accumulated token back to BF16 and store the result back
            // to shared memory token entry.
            store_token_base_ptr[element_id] =
                __float22bfloat162_rn(acc_token_fp32[n]);
          }
        }
...
#pragma unroll
          for (int n = 0; n < NUM_LSA_TEAMS; n++) {
            int attn_prob_output_node_id = (node_rank - n) >= 0
                ? node_rank - n
                : node_rank + NUM_LSA_TEAMS - n;
            int element_base_id = attn_prob_output_node_id *
                (experts_per_rank * num_of_ranks_per_node);
#pragma unroll
            for (int m = 0; m < NUM_OF_PROB_VEC_ELEMENT_PER_THREAD; m++) {
              int element_id =
                  thread_rank_within_pipeline + m * NUM_OF_THREADS_PER_PIPELINE;
              if (element_id < experts_per_rank * num_of_ranks_per_node) {
        ...
        // Make sure the shared memory stored by current thread is visible by
        // async proxy.
        cuda::ptx::fence_proxy_async(cuda::ptx::space_shared);

        // Make sure all threads within the pipeline have finished storing the
        // current token entry and making it visible to async proxy.
        arrive_and_wait(NUM_OF_THREADS_PER_PIPELINE, 2 + pipeline_rank);

        // Select the TMA thread within the pipeline to issue S2G TMA operations
        // for current token entry.
        if (warp_rank_within_pipeline == 0) {
          if (cuda::ptx::elect_sync(~0)) {
            ...
            if constexpr (BACKWARD_COMBINE) {
              ...
              cuda::ptx::cp_async_bulk(
                  ...
                  reinterpret_cast<void*>(current_prob_addr),
                  reinterpret_cast<const void*>(
                      smem_buffer_ptr->get_inter_node_prob_S2G(
                          dst_token_stage)),
                  (uint32_t)((experts_per_rank * num_of_ranks_per_node *
                              NUM_LSA_TEAMS) *
                             sizeof(float)));
            }
            // Commit S2G TMA operations for this dst token into a bulk async
            // copy group.
            cuda::ptx::cp_async_bulk_commit_group();
          }
        }
  ...
  // Because the attn output buffers will only be produced by local combine
  // kernel, not by the combine kernels on other ranks, so we only need to wait
  // for local combine kernel to finish writing all token data back to output
  // buffer before we can exit. Also, a kernel will be considered completed from
  // CUDA stream's perspective if and only if all the threads are exit and all
  // memory operations(including TMA operations) issued by all threads have been
  // completed and made visible to sys scope. So the CUDA stream's kernel
  // boundary implicit synchronization should be enough to sync with all TMA
  // operations issued in the combine kernel. So we can directly exit w/o any
  // explicit synchronization with TMA operations.
}

// __launch_bounds__(1, 1)
// __global__ void device_sync_kernel(uint32_t* intra_node_remote_flags, const
// uint32_t* expected_flag_value)
// {
//   // Atomically reduce add 1 to the u32 flag on rank #0 in current NVLink
//   domain.
//   // Need a strong system-scope red to make sure all ranks from current
//   NVLink domain can see the side effect.
//   // But no memory fence(i.e. .release) needed since CUDA stream already do
//   that for us.
...
//   // Polling flag value from the u32 flag on rank #0 in current NVLink
//   domain.
//   // Keep polling until reach the expected value.
//   uint32_t flag_data = 0;
//   do {
//       flag_data = 0;
//       // Need a strong system-scope load to observe other ranks' Atomic
//       result.
//       // But no no memory fence(i.e. .aquired) needed since no memory
//       operation behind this. asm volatile("ld.relaxed.sys.global.u32 %0,
//       [%1];"
//                     : "=r"(flag_data)
//                     : "l"(__cvta_generic_to_global(intra_node_remote_flags))
//                     : "memory");
...
// This kernel will update expected_rdma_flag_value and
// expected_intra_node_flag_value in local device memory by increasing the
// expected_rdma_flag_value by 1 and expected_intra_node_flag_value by
// num_of_ranks_per_node.
template <int NUM_LSA_TEAMS>
__launch_bounds__(1, 1) __global__ void update_expected_value_kernel(
    uint64_t* expected_rdma_flag_value,
    uint32_t* expected_intra_node_flag_value,
    const int num_of_ranks_per_node) {
  if constexpr (NUM_LSA_TEAMS != 1) {
...
template <
    ...
    int LSA_TEAM_SIZE>
__launch_bounds__(
    INTER_NODE_GROUP::size() + INTRA_NODE_G2S_GROUP::size() +
        INTRA_NODE_S2G_GROUP::size(),
    1) __global__
    void dispatch_kernel(
        const __grid_constant__
            dispatch_kernel_param_t<TOKEN_DATA_TYPE, LSA_TEAM_SIZE> param) {
  if constexpr (NUM_LSA_TEAMS != 1) {
    static_assert(
  ...
  static_assert(
      NUM_OF_STAGES % NUM_PIPELINES == 0,
      "NUM_OF_STAGES must be divisible by NUM_PIPELINES.");
  constexpr int STAGES_PER_PIPELINE = NUM_OF_STAGES / NUM_PIPELINES;
    ...
    for (int p = 0; p < NUM_PIPELINES; p++) {
      for (int s = 0; s < STAGES_PER_PIPELINE; s++) {
        ...
        cuda::ptx::mbarrier_init(
            smem_buffer_ptr->intra_node_mbarrier_buffer + 2 * abs_stage, 1);
        cuda::ptx::mbarrier_init(
            smem_buffer_ptr->intra_node_mbarrier_buffer + 2 * abs_stage + 1, 1);
      }
  ...
  if (threadIdx_x_int < INTER_NODE_GROUP::size()) {
    ...
    G2S_warp_group_device_function<
      ...
      threadIdx_x_int < INTER_NODE_GROUP::size() +
          INTRA_NODE_G2S_GROUP::size() + INTRA_NODE_S2G_GROUP::size()) {
  ...
  // All threads in this block must finish G2S/S2G/N2N work before we signal
  // completion.
  __syncthreads();

  if (threadIdx.x == 0) {
    // Grid barrier: all blocks in this kernel must arrive before inter-rank
    // signaling. TMA S2G writes are already committed by
    // cp_async_bulk_wait_group in S2G warp group.
...
__global__ void combine_kernel(const __grid_constant__ combine_kernel_param_t<LSA_TEAM_SIZE> param)
{
  ...
  // Currently, we use TMA to copy prob data, which need at least 16B size and
  // alignment(which requires expert per node to be multiple of 4). We need to
  // add padding or not using TMA for prob, if we want to support other
  // scenario.
  // assert((param.experts_per_rank * param.num_of_ranks_per_node *
  // sizeof(float)) % 16 == 0);
  ...
  if (threadIdx.x == 0) {
    for (int i = 0; i < NUM_OF_STAGES_G2S; i++) {
      ...
      if constexpr (NUM_LSA_TEAMS != 1) {
        cuda::ptx::mbarrier_init(
            smem_buffer_ptr->intra_node_mbarrier_G2S_buffer + 2 * i, 1);
        cuda::ptx::mbarrier_init(
            smem_buffer_ptr->intra_node_mbarrier_G2S_buffer + 2 * i + 1, 1);
      }
      cuda::ptx::mbarrier_init(
          smem_buffer_ptr->inter_node_mbarrier_G2S_buffer + 2 * i, 1);
      cuda::ptx::mbarrier_init(
          smem_buffer_ptr->inter_node_mbarrier_G2S_buffer + 2 * i + 1, 1);
    }
    if constexpr (NUM_LSA_TEAMS != 1) {
      // Initialize mbarrier
      for (int i = 0; i < NUM_LSA_TEAMS - 1; i++) {
        for (int j = 0; j < MAX_NUM_OF_CHUNKS_PER_RANK; j++) {
          cuda::ptx::mbarrier_init(
              smem_buffer_ptr->intra_node_to_rdma_mbarrier_buffer +
                  i * MAX_NUM_OF_CHUNKS_PER_RANK + j,
              1);
        }
      }
    }
    // Initialize streaming overlap fields (cumulative counter, never reset
    // between chunks)
    if constexpr (NUM_LSA_TEAMS != 1) {
  ...
  // Make sure all the warps wait for mbarriers to be initialized before
  ...
  if (threadIdx_x_int < INTRA_NODE_RED_GROUP::size()) {
    ...
    inter_node_red_warp_group_device_function<
      ...
      threadIdx_x_int < INTRA_NODE_RED_GROUP::size() +
          INTER_NODE_RED_GROUP::size() + INTRA_NODE_G2S_GROUP::size()) {
...
static __device__ __forceinline__ bool bitmap_range_has_set_bit(
  ...
  if (first_byte == last_byte) {
    ...
    return (bitmap_row[first_byte] &
            static_cast<uint8_t>(left_mask & right_mask)) != 0;
  }
...
__launch_bounds__(NUM_THREADS_PER_BLOCK, 1) __global__ void scan(
  ...
  const int num_of_tokens_per_block =
      num_of_tokens_per_warp * NUM_OF_WARPS_PER_BLOCK;
  // The rdma_to_attn_map need to be paded to multiple of
  // rdma_to_attn_map_load_t per node. The largest size of
  // rdma_to_attn_map_load_t allowed in all Hybrid-EP kernels are 16B(16 bools),
  // so need to be paded to 16B per node. That means the size of
  ...
  // Any experts_per_rank is supported; last byte may have unused high bits
  // (always 0).
  const int experts_per_node = experts_per_rank * num_of_ranks_per_node;
  const int experts_per_node_packed =
      (experts_per_node + 7) / 8; // bytes per node section (ceil)
  const int packed_row_bytes =
      experts_per_node_packed * NUM_LSA_TEAMS; // full bitmap row width

  // For each token, calculate how many bytes need to be store to
  // sparse_to_dense_map. Use maximum value for compile-time type selection
  constexpr int MAX_NUM_OF_BYTES_TO_STORE_FOR_EACH_TOKEN =
      sizeof(int32_t) * LSA_TEAM_SIZE;
  using write_t = Copy_t<MAX_NUM_OF_BYTES_TO_STORE_FOR_EACH_TOKEN>;
  const int NUM_OF_BYTES_TO_STORE_FOR_EACH_TOKEN =
      sizeof(int32_t) * num_of_ranks_per_node;
  const int S2D_MAP_STORE_ITER =
      NUM_OF_BYTES_TO_STORE_FOR_EACH_TOKEN / sizeof(write_t);

  // How to convert per-rank routing info to per-node routing info.
  // Current HT constraint is max 8 ranks per node, so one lane can represent
  // one rank.
  static_assert(
      LSA_TEAM_SIZE <= WARP_SIZE,
  ...
  int32_t* previous_block_sum = reinterpret_cast<int32_t*>(
      smem_bytes +
      NUM_OF_WARPS_PER_BLOCK * num_of_ranks_per_node * sizeof(int32_t));
  ...
  if constexpr (ENABLE_PER_EXPERT_COUNTS) {
    block_expert_token_counts = reinterpret_cast<int32_t*>(
        smem_bytes +
        (NUM_OF_WARPS_PER_BLOCK * num_of_ranks_per_node +
         num_of_ranks_per_node) *
            sizeof(int32_t));
    for (int e = threadIdx.x; e < experts_per_rank;
         e += NUM_THREADS_PER_BLOCK) {
      block_expert_token_counts[e] = 0;
    }
    __syncthreads();
  }

  // We assign contiguous tokens called chunk to each CUDA block, each CUDA
  // block get the same size of chunk.
  int block_starting_token = blockIdx.x * num_of_tokens_per_block;
  // warp id and lane id.
  int warp_id = threadIdx.x / WARP_SIZE;
  int lane_id = threadIdx.x % WARP_SIZE;
  // We assign contiguous tokens called sub-chunk to each warp within a CUDA
  // block, each warp within a CUDA block get the same size of sub-chunk.
  int warp_starting_token =
      block_starting_token + warp_id * num_of_tokens_per_warp;
  // Within a sub-chunk, we assign tokens to thread in a interleave pattern. So
  // each thread process a token each time and each warp sum a tile of 32 tokens
  // each time.
  int thread_starting_token = warp_starting_token + lane_id;

  // Step 0: Each warp sum the sub-chunk assigned to them and store the sum back
  // to shared memory. All warps within all CTA attend this step. Also, some
  // tokens need per-node info which store to rdma_to_attn_map, also processed
  // here.

  // Sum of per-rank token routing map within a thread.
  // Use MAX size for compile-time array allocation, actual size determined by
  // runtime num_of_ranks_per_node
...
#pragma unroll
  ...
  for (int i = 0; i < num_of_tokens_per_thread; i++) {
    ...
    // If the current token is out-of-bound, then just end summing tokens
    ...
    // We need to calculate the per-node routing info and save back to
    // rdma_to_attn_map.
    bool per_node_routing_info = (current_token_local_rank == local_rank);
    int current_token_rdma_to_attn_map_id =
        current_token_node_rank * rdma_to_attn_map_size_per_node +
    ...
    uint8_t rank_mask = bitmap_row_to_rank_mask<LSA_TEAM_SIZE>(
...
#pragma unroll
  for (int rank = 0; rank < num_of_ranks_per_node; rank++) {
    ...
    if (lane_id == rank) {
      warp_token_routing_map_sum[warp_id * num_of_ranks_per_node + rank] =
          rank_sum;
    }
  }

  // Sync within a CUDA block to make sure all warps have produced the per-rank
  // sum data to the shared memory before any thread can consume them to produce
  // CUDA block level's sum data.
  __syncthreads();

  // Step 1: Communication between CUDA blocks. Each CUDA block's threads need
  // to produce and store the current block's per-rank sum data to global
  // memory, and load and accumulate previous blocks' per-rank sum data and save
  // the result to shared memory.

  // Each thread within a CUDA block calculate the CUDA block level sum for a
  // single rank at a time.
  for (int i = threadIdx.x; i < num_of_ranks_per_node;
...
#pragma unroll
    ...
    // Store the sum of current rank within this CUDA block to global memory for
    // later scan opeartions. Strong(atomic) store is needed to be visible to
    // strong(atomic) load from other blocks.
  ...
  // Each thread within a CUDA block load previous blocks' block level sum for a
  // single rank at a time.
  for (int i = threadIdx.x; i < num_of_ranks_per_node;
       i += NUM_THREADS_PER_BLOCK) {
    ...
    for (int j = 0; j < blockIdx.x; j++) {
      ...
      do {
        ...
        // Strong(atomic) load is needed to view strong(atomic) store from other
        // blocks.
        uint64_t data = 0;
  ...
  // Sync within a CUDA block to make sure all previous blocks' per-rank sum
  // have been produced to the shared memory before any thread can consume them
  // in scan operation.
  __syncthreads();

  // Step 2: Each warp scan the sub-chunk assigned to them(the same sub-chunk as
  // step 0) and produce sparse_to_dense_map, local_expert_routing_map and
  // num_of_tokens_for_experts. Use MAX size for compile-time array allocation
  ...
  if (lane_id < num_of_ranks_per_node) {
    ...
    for (int j = 0; j < warp_id; j++) {
      lane_rank_prefix +=
          warp_token_routing_map_sum[j * num_of_ranks_per_node + lane_id];
    }
  }

// Broadcast accumulated per-rank prefix sums from source lanes [0,
// num_of_ranks_per_node).
#pragma unroll
  for (int rank = 0; rank < num_of_ranks_per_node; rank++) {
  ...
  for (int i = 0; i < num_of_tokens_per_thread; i++) {
    ...
    // If the whole tiles are out-of-bound, the warp just finish and exit the
    // scan loop together.
    ...
    // then produce the per-rank final exclusive scan within the warp for this
    // tile. Use MAX size for compile-time array allocation
...
#pragma unroll
      ...
      // The inclusive scan from last lane is the sum of this rank of this tile.
      // Need to accumulate that for later tiles.
      ...
      // Calculate the final exclusive scan for current token. -1 represent that
      // the current rank does not need the current token.
      final_ex_scan[j] =
          token_needed_by_this_rank ? previous_token_sum[j] + temp_scan : -1;
    ...
    // Each thread saves local routing map for this token of the local rank if
    // needed. Optional per-expert counting is fused here to avoid a second pass
    // over local_expert_routing_map.
    if (local_rank_seen) {
      bool lane_participates =
          (token_out_of_bound == 0) && token_needed_by_local_rank;
      ...
      if (lane_participates) {
        local_rank_bitmap_row = input_routing_map +
            current_token_id * packed_row_bytes +
            node_rank * experts_per_node_packed;
        local_expert_routing_map_store_base_addr = local_expert_routing_map +
            (final_ex_scan[local_rank] * experts_per_rank);
      }
      ...
      for (int k = 0; k < experts_per_rank; k++) {
        ...
        if (lane_participates) {
          routed_to_expert =
              ((local_rank_bitmap_row[expert_bit / 8] >> (expert_bit % 8)) &
               1u) != 0;
    ...
    // The thread that processing the global last token save the final sum for
    // current rank to num_of_tokens_for_experts.
    if (current_token_id == num_of_total_attn_tokens - 1 && local_rank_seen) {
      *num_of_tokens_for_experts = local_rank_prefix_after_scan;
    }

    // Save final exclusive scan of this token back to sparse_to_dense_map if
    // current token is not out-of-bound and is needed.
    if (token_out_of_bound == 0 && current_token_local_rank == local_rank) {
      ...
      write_t* sparse_to_dense_map_store_base_addr = reinterpret_cast<write_t*>(
          sparse_to_dense_map +
          (current_token_node_rank * num_of_tokens_per_rank +
           current_token_local_id) *
              num_of_ranks_per_node);
#pragma unroll
  ...
  // Since each token(row) is fully independent, each token(row) is assigned to
  // each threads in a interleave pattern.
  if constexpr (NUM_LSA_TEAMS != 1) {
    const int num_of_total_token_rows =
        (NUM_LSA_TEAMS - 1) * num_of_tokens_per_rank;
    ...
    for (int i = 0; i < num_of_token_rows_per_thread; i++) {
      ...
      // If the current token is out-of-bound, then just end processing token
      // rows assigned to this thread.
      if (current_token_id >= num_of_total_token_rows) {
        break;
      }
      int current_token_attn_to_rdma_map_node_id =
          current_token_id % (NUM_LSA_TEAMS - 1);
      int current_token_node_id =
          current_token_attn_to_rdma_map_node_id < node_rank
          ? current_token_attn_to_rdma_map_node_id
          : current_token_attn_to_rdma_map_node_id + 1;
      int current_token_local_id = current_token_id / (NUM_LSA_TEAMS - 1);

      // Load bitmap for this token's remote-node section
      // (current_token_node_id, not node_rank).
      const uint8_t* bitmap_row = input_routing_map +
          ((node_rank * num_of_ranks_per_node + local_rank) *
               num_of_tokens_per_rank +
           current_token_local_id) *
              packed_row_bytes +
          current_token_node_id * experts_per_node_packed;

      bool* attn_to_rdma_map_base_addr = attn_to_rdma_map +
          (current_token_local_id * (NUM_LSA_TEAMS - 1) +
           current_token_attn_to_rdma_map_node_id);

      // Any bit set in this remote-node section means this token must be sent
      // via RDMA.
      bool token_needed_by_this_node =
          bitmap_range_has_set_bit(bitmap_row, 0, experts_per_node);
...
template <
    ...
    // ncclTeamLsa(comm).nRanks and dispatched via
    // HYBRIDEP_SWITCH_LSA_TEAM_SIZE.
    int LSA_TEAM_SIZE>
class hybrid_ep {
 public:
  // Processing metadata. Calculate routing info needed by dispatch and combine
  // operations. input_routing_map: IO: input, dtype: bool, shape:
  // [NUM_OF_TOKENS_PER_RANK * NUM_OF_RANKS_PER_NODE * NUM_LSA_TEAMS,
  // NUM_OF_EXPERTS_PER_RANK * NUM_OF_RANKS_PER_NODE * NUM_LSA_TEAMS]. Routing
  // map which contain global routing info from all tokens to all expert.
  // Allgather is needed before passing the routing map to this API.
  // preprocessing_tmp: IO: output/input, dtype: tmp_state_t, shape:
  // [NUM_OF_BLOCKS for preprocessing kernel, NUM_OF_RANKS_PER_NODE]. The temp
  // buffer needed by the preprocessing kernel. sparse_to_dense_map: IO: output,
  // dtype: int32_t, shape: [NUM_OF_TOKENS_PER_RANK * NUM_LSA_TEAMS,
  // NUM_OF_RANKS_PER_NODE]. The routing info needed by NVL warps(i.e.
  // intra-node communication warps) during both dispatch and combine operation.
  // Remains the same in a trainning iteration(FW+BP). rdma_to_attn_map: IO:
  // output, dtype: bool, shape: [NUM_OF_TOKENS_PER_RANK padded to 16 *
  // NUM_LSA_TEAMS] The routing info mainly needed by RDMA warps during the
  // combine operation. Remains the same in a trainning iteration(FW+BP).
  // attn_to_rdma_map: IO: output, dtype: bool, shape: [NUM_OF_TOKENS_PER_RANK,
  // NUM_LSA_TEAMS - 1]. The routing info mainly needed by RDMA warps during the
  // dispatch operation. Remains the same in a trainning iteration(FW+BP).
  // num_of_tokens_for_experts: IO: output, dtype: int32_t, shape: [1].
  // The total size of expert buffer on this rank(in number of tokens),
  // according to the global routing map. If there are multiple expert on this
  // rank, each token will only appear once. Remains the same in a trainning
  // iteration(FW+BP). local_expert_routing_map: IO: output, dtype: bool, shape:
  // [at least num_of_tokens_for_experts, NUM_OF_EXPERTS_PER_RANK]. The
  // per-expert routing info for all tokens within the expert buffer of this
  // rank. It is used by later layer to routing the tokens to different experts
  // on this rank. Remains the same in a trainning iteration(FW+BP).
                                     ...
                                     cudaStream_t stream)
  {
    ...
    const size_t preprocessing_tmp_sz =
        NUM_OF_BLOCKS * num_of_ranks_per_node * sizeof(tmp_state_t);
    CUDA_CHECK(
        cudaMemsetAsync(preprocessing_tmp, 0, preprocessing_tmp_sz, stream));

    // Launch the preprocessing kernel to process the global routing map.
    // CRITICAL: Must use cudaLaunchKernelEx (via LAUNCH_KERNEL macro) instead
    // of <<<>>> syntax. The <<<>>> syntax fails at runtime for heavily
    // templated kernels instantiated through nested macros in separate device
    // compilation mode. cudaLaunchKernelEx uses direct function pointers,
    // bypassing CUDA's fragile static kernel registration tables.
    ...
    const size_t scan_base_smem_size =
        (NUM_OF_WARPS_PER_BLOCK_SCAN * num_of_ranks_per_node *
         sizeof(int32_t)) +
        (num_of_ranks_per_node * sizeof(int32_t));
    if (per_expert_token_counts != nullptr) {
      cfg.dynamicSmemBytes =
          scan_base_smem_size + (experts_per_rank * sizeof(int32_t));
      auto scan_kernel_ptr = scan<
  ...
  static void dispatch(dispatch_kernel_param_t<TOKEN_DATA_TYPE, LSA_TEAM_SIZE> param, cudaStream_t stream)
  {
    ...
    constexpr int INTER_NODE_GROUP_WARPS =
        multinode_layout ? HYBRIDEP_DISPATCH_N2N_WARPS : 0;
    ...
    constexpr int INTRA_NODE_S2G_GROUP_START =
        multinode_layout ? (2 + NUM_PIPELINES) : NUM_PIPELINES;
    using INTER_NODE_GROUP =
        warp_group<INTER_NODE_GROUP_WARPS, INTER_NODE_GROUP_START>;
    using INTRA_NODE_G2S_GROUP =
        warp_group<INTRA_NODE_G2S_GROUP_WARPS, INTRA_NODE_G2S_GROUP_START>;
    using INTRA_NODE_S2G_GROUP =
        warp_group<INTRA_NODE_S2G_GROUP_WARPS, INTRA_NODE_S2G_GROUP_START>;

    const auto dispatch_kernel_ptr = dispatch_kernel<
        TOKEN_DATA_TYPE,
        INTER_NODE_GROUP,
        INTRA_NODE_G2S_GROUP,
        INTRA_NODE_S2G_GROUP,
    ...
    constexpr int BLOCK_DIM = INTER_NODE_GROUP::size() +
        INTRA_NODE_G2S_GROUP::size() + INTRA_NODE_S2G_GROUP::size();
#ifdef HYBRIDEP_ENABLE_WARP_TIMING
    using warp_timing_entry_t =
        typename dispatch_kernel_param_t<TOKEN_DATA_TYPE, LSA_TEAM_SIZE>::
            warp_timing_entry_t;
    ...
    CUDA_CHECK(cudaMemsetAsync(
        d_wt, 0, WT_TOTAL * sizeof(warp_timing_entry_t), stream));
    ...
    CUDA_CHECK(cudaMemcpy(
        h_wt,
        d_wt,
        WT_TOTAL * sizeof(warp_timing_entry_t),
        cudaMemcpyDeviceToHost));
    int _wt_clock_khz;
    CUDA_CHECK(cudaDeviceGetAttribute(&_wt_clock_khz, cudaDevAttrClockRate, 0));
    auto _wt_us = [&](long long cycles) {
      return (double)cycles * 1000.0 / _wt_clock_khz;
    };
    auto _wt_print_group = [&](const char* name,
                               int warp_start,
                               int warp_count) {
      if (warp_count == 0)
        return;
    ...
    printf(
        "[DISPATCH WARP TIMING] (%d blocks, %d warps/block, %d pipelines, clock=%d kHz)\n",
        NUM_OF_BLOCKS,
        WT_WARPS_PER_BLOCK,
        NUM_PIPELINES,
        _wt_clock_khz);
    ...
    _wt_print_group(
        "S2G",
        INTER_NODE_GROUP_WARPS + INTRA_NODE_G2S_GROUP_WARPS,
  ...
  {
    // The warp groups data type for combine kernel, must match the warp groups
    // layout required by the combine kernel.
    constexpr bool multinode_layout = (NUM_LSA_TEAMS != 1);
    ...
    using INTRA_NODE_RED_GROUP =
        warp_group<INTRA_NODE_RED_GROUP_WARPS, INTRA_NODE_RED_GROUP_START>;
    using INTER_NODE_RED_GROUP =
        warp_group<INTER_NODE_RED_GROUP_WARPS, INTER_NODE_RED_GROUP_START>;
    using INTRA_NODE_G2S_GROUP =
        warp_group<INTRA_NODE_G2S_GROUP_WARPS, INTRA_NODE_G2S_GROUP_START>;
    using INTER_NODE_G2S_GROUP =
        warp_group<INTER_NODE_G2S_GROUP_WARPS, INTER_NODE_G2S_GROUP_START>;
    using INTER_NODE_RDMA_GROUP =
        warp_group<INTER_NODE_RDMA_GROUP_WARPS, INTER_NODE_RDMA_GROUP_START>;
    constexpr int NUM_OF_DATA_PIPELINE_PER_BLOCK = multinode_layout ? 1 : 2;
    static_assert(
        INTER_NODE_G2S_GROUP::warp_size() == NUM_OF_DATA_PIPELINE_PER_BLOCK,
        "Inter-node G2S warp group pipeline and inter-node red warp group pipeline mismatch.");

    // The shared memory needed by the combine kernel.
    // The combine kernel to be launched.
    const auto combine_kernel_ptr = combine_kernel<
        INTRA_NODE_RED_GROUP,
        INTER_NODE_RED_GROUP,
        INTRA_NODE_G2S_GROUP,
        INTER_NODE_G2S_GROUP,
    ...
    model.num_of_ranks_per_node = param.num_of_ranks_per_node;
    model.num_of_nodes = NUM_LSA_TEAMS;
    const int SMEM_SIZE = calculate_combine_smem_layout_size<
        NUM_OF_STAGES_G2S,
        NUM_OF_STAGES_S2G,
        NUM_OF_TOKENS_PER_CHUNK,
        MAX_NUM_OF_TOKENS_PER_RANK,
    ...
    static int configured_smem = 0;
    if (SMEM_SIZE > configured_smem) {
      // If the dynamic shared memory requested is too large, we may need to
      // modify the carveout.
      // CUDA_CHECK(cudaFuncSetAttribute(combine_kernel_ptr,
    ...
    constexpr int BLOCK_DIM = INTRA_NODE_RED_GROUP::size() +
        INTER_NODE_RED_GROUP::size() + INTRA_NODE_G2S_GROUP::size() +
        INTER_NODE_G2S_GROUP::size() + INTER_NODE_RDMA_GROUP::size();
