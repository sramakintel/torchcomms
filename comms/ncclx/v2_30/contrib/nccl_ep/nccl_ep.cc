...
static cudaDataType_t ncclDataTypeToCudaDataType(ncclDataType_t nccl_type) {
  switch (nccl_type) {
    ...
    default:
      assert(
          false &&
          "Unsupported ncclDataType_t for conversion to cudaDataType_t");
...
// This operates on a single in-place host buffer, unlike
// batchAllGatherIpcHandles which batches multiple IPC handles via a packed
// device buffer. Each rank contributes element_size bytes at offset rank *
// element_size.
static void ncclAllGatherHost(
  ...
  CUDA_CHECK(
      cudaMemcpy(host_buffer, d_buffer, total_size, cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaFree(d_buffer));
}

// NCCL barrier using AllReduce.
// If workspace is provided, it is used directly (must be at least sizeof(int)
// device bytes). Otherwise a temporary cudaMalloc/cudaFree pair is used as
// fallback.
static ncclResult_t
ncclBarrier(ncclComm_t comm, cudaStream_t stream, void* workspace = nullptr) {
  ...
  if (workspace) {
    ...
    CUDA_CHECK(
        cudaMalloc(reinterpret_cast<void**>(&nccl_barrier_var), sizeof(int)));
    owns_memory = true;
  }
  CUDA_CHECK(cudaMemset(nccl_barrier_var, 0, sizeof(int)));
  NCCL_CHECK_RESULT(ncclAllReduce(
      nccl_barrier_var, nccl_barrier_var, 1, ncclInt, ncclSum, comm, stream));
...
struct ncclEpGroup {
  ...
  struct {
    // Split communicator: groups ranks with same local_rank across nodes
    // (nNodes ranks each)
    ...
    ncclWindow_t nccl_window =
        {}; // Single registered window handle (pointer-sized)
    ...
    size_t rdma_inter_node_group_packed_offset =
        0; // Packed receive buffer (token+prob+sf)

    unsigned signals_tail_base =
        0; // Base signal ID for tail tracking (sender -> receiver)
  ...
  int max_recv_tokens; // Worst-case receive capacity: nRanks *
                       // max_tokens_per_rank
  ...
  // Physical node properties (CUDA device assignment, IPC between co-located
  // GPUs)
  ...
  int lsa_rank_count; // Ranks per node passed to HT kernels as
                      // num_ranks_per_node
  ...
  struct {
    ...
    // Grid barrier counter for fused device_sync in dispatch tail (per-rank,
    // not IPC-shared)
    uint32_t* dispatch_grid_barrier_counter;
    // Host-side expected flag counters (replaces device-side
    ...
    // Pre-registered dispatch buffers (group-level, allocated during Group
    // Create) These are pre-registered with GIN to avoid ~60ms registration
    // overhead during dispatch
    void* token_staging_buffer; // Pre-registered staging buffer for user tokens
    float* dense_prob_buffer; // Pre-registered buffer for sparse→dense prob
                              // conversion
    float* scaling_factor_staging_buffer; // Pre-registered staging buffer for
                                          // FP8 scaling factors
    ...
    void** peer_ipc_base_ptrs =
        nullptr; // Opened IPC base pointers per peer (for cleanup)
    void* host_ptr_block =
        nullptr; // Single cudaHostAlloc for all pointer arrays
...
// local_handles: array of num_handles local IPC handle pointers (each
// CUDA_IPC_HANDLE_SIZE bytes) all_handles:   array of num_handles output
// buffers (each CUDA_IPC_HANDLE_SIZE * nRanks bytes)
static void batchAllGatherIpcHandles(
    ...
    ncclComm_t comm,
    cudaStream_t stream) {
  const size_t per_rank_size =
      static_cast<size_t>(num_handles) * CUDA_IPC_HANDLE_SIZE;
  ...
  for (int i = 0; i < num_handles; i++) {
    memcpy(
        pack_buf.get() + i * CUDA_IPC_HANDLE_SIZE,
        local_handles[i],
  ...
  CUDA_CHECK(
      cudaMemcpy(host_buf.get(), d_buffer, total_size, cudaMemcpyDeviceToHost));
  CUDA_CHECK(cudaFree(d_buffer));

  // Deinterleave: for each handle i, extract rank r's copy from the packed
  // layout
  for (int i = 0; i < num_handles; i++) {
    uint8_t* out = static_cast<uint8_t*>(all_handles[i]);
    for (int r = 0; r < nRanks; r++) {
      memcpy(
          out + r * CUDA_IPC_HANDLE_SIZE,
...
static ncclResult_t init_hybridep_intranode(
  ...
  // Node semantics: IPC, buffer sizing, and indexing use per-node device
  // ordinals. Note: for MNNVL this only covers same-node peers; cross-node
  // MNNVL HT is not supported.
  ...
  // cudaDeviceCanAccessPeer/cudaDeviceEnablePeerAccess operate on per-node
  // device ordinals (rank_in_node, gpus_per_node) — these are node concepts,
  // not LSA concepts. Cross-node MNNVL P2P is handled by the NVLink fabric
  // through IPC handles; it does not need cudaDeviceEnablePeerAccess and cannot
  // be probed with cudaDeviceCanAccessPeer since remote node device ordinals
  // are not valid on the local node.
  // TODO: replace this loop with a NCCL API that queries P2P capability (e.g.
  ...
  for (int i = 0; i < gpus_per_node; i++) {
    ...
    if (can_p2p) {
      ...
      if (err != cudaSuccess && err != cudaErrorPeerAccessAlreadyEnabled) {
        fprintf(
            stderr,
            "HT: Failed to enable P2P from GPU %d to GPU %d\n",
            rank_in_node,
            i);
      }
    }
  ...
  // Consolidated IPC mega-buffer: single cudaMalloc for all 4 IPC-shared
  // buffers. Expert-prob buffers sized by gpus_per_node (node semantics).
  auto align_ipc = [](size_t s) -> size_t { return (s + 255) & ~size_t(255); };

  size_t expert_output_token_sz =
      static_cast<size_t>(max_recv_tokens) * hidden * sizeof(uint16_t);
  size_t expert_output_prob_sz = static_cast<size_t>(max_recv_tokens) *
      num_local_experts * gpus_per_node * sizeof(float);
  size_t expert_input_token_sz =
      static_cast<size_t>(max_recv_tokens) * hidden * sizeof(uint16_t);
  size_t expert_input_prob_sz = static_cast<size_t>(max_recv_tokens) *
      num_local_experts * gpus_per_node * sizeof(float);
  ...
  uint8_t* mega_base =
      static_cast<uint8_t*>(ep_group->ht_buffers.ipc_mega_buffer);
  ...
  ep_group->ht_buffers.expert_output_prob =
      reinterpret_cast<float*>(mega_base + dispatch_token_aligned);
  ...
  ep_group->ht_buffers.ipc_combine_prob_offset =
      dispatch_token_aligned + dispatch_prob_aligned + combine_token_aligned;
  ep_group->ht_buffers.expert_input_prob = reinterpret_cast<float*>(
      mega_base + dispatch_token_aligned + dispatch_prob_aligned +
      combine_token_aligned);

  // Host pointer arrays indexed by per-node position (one entry per node peer).
  size_t host_block_sz = sizeof(void*) * gpus_per_node +
      sizeof(float*) * gpus_per_node + sizeof(uint16_t*) * gpus_per_node +
      sizeof(float*) * gpus_per_node;
  CUDA_CHECK(cudaHostAlloc(
      &ep_group->ht_buffers.host_ptr_block,
      host_block_sz,
      cudaHostAllocMapped));

  uint8_t* hptr = static_cast<uint8_t*>(ep_group->ht_buffers.host_ptr_block);
  ep_group->ht_buffers.dispatch_expert_output_token_buffer_ptrs =
      reinterpret_cast<void**>(hptr);
  hptr += sizeof(void*) * gpus_per_node;
  ep_group->ht_buffers.dispatch_expert_output_prob_buffer_ptrs =
      reinterpret_cast<float**>(hptr);
  hptr += sizeof(float*) * gpus_per_node;
  ep_group->ht_buffers.combine_expert_input_token_buffer_ptrs =
      reinterpret_cast<uint16_t**>(hptr);
  hptr += sizeof(uint16_t*) * gpus_per_node;
  ep_group->ht_buffers.combine_expert_input_prob_buffer_ptrs =
      reinterpret_cast<float**>(hptr);
  ...
  if (rank_in_node == 0) {
    ...
    CUDA_CHECK(cudaMalloc(
        reinterpret_cast<void**>(&completion_flags_base),
        2 * sizeof(uint32_t)));
    CUDA_CHECK(cudaMemsetAsync(
        completion_flags_base, 0, 2 * sizeof(uint32_t), stream));
    ep_group->ht_buffers.intra_node_write_completion_flags =
        completion_flags_base;
    ep_group->ht_buffers.combine_intra_node_write_completion_flags =
        completion_flags_base + 1;
  }

  // Dispatch grid barrier counter (local to each rank, NOT IPC-shared)
  {
    uint32_t* grid_barrier_base;
    CUDA_CHECK(ep_group->alloc_fn(
        reinterpret_cast<void**>(&grid_barrier_base), sizeof(uint32_t)));
  ...
  // Consolidated: [0] = mega buffer (token+prob+combine), [1] = completion
  // flags
  // =========================================================================
  ...
  CUDA_CHECK(cudaIpcGetMemHandle(
      &local_handles[0], ep_group->ht_buffers.ipc_mega_buffer));
  if (rank_in_node == 0) {
    CUDA_CHECK(cudaIpcGetMemHandle(
        &local_handles[1],
        ep_group->ht_buffers.intra_node_write_completion_flags));
  ...
  // Indexed by rank_in_node; node_id identifies which physical node this rank
  // belongs to.
  // =========================================================================

  // Track opened IPC base pointers for cleanup (one per node peer)
  CUDA_CHECK(cudaHostAlloc(
      &ep_group->ht_buffers.peer_ipc_base_ptrs,
      sizeof(void*) * gpus_per_node,
      cudaHostAllocMapped));
  memset(
      ep_group->ht_buffers.peer_ipc_base_ptrs,
      0,
  ...
  for (int i = 0; i < gpus_per_node; i++) {
    if (i == rank_in_node) {
      ...
      memcpy(
          &peer_handle,
          all_handles_buf[0].get() + peer_global * CUDA_IPC_HANDLE_SIZE,
          CUDA_IPC_HANDLE_SIZE);
      void* peer_base;
      CUDA_CHECK(cudaIpcOpenMemHandle(
          &peer_base, peer_handle, cudaIpcMemLazyEnablePeerAccess));
      ...
      ep_group->ht_buffers.dispatch_expert_output_prob_buffer_ptrs[i] =
          reinterpret_cast<float*>(
              pb + ep_group->ht_buffers.ipc_dispatch_prob_offset);
      ep_group->ht_buffers.combine_expert_input_token_buffer_ptrs[i] =
          reinterpret_cast<uint16_t*>(
              pb + ep_group->ht_buffers.ipc_combine_token_offset);
      ep_group->ht_buffers.combine_expert_input_prob_buffer_ptrs[i] =
          reinterpret_cast<float*>(
              pb + ep_group->ht_buffers.ipc_combine_prob_offset);
    }
  }

  // Merged completion flags: non-rank_in_node-0 ranks IPC-map from node-local
  // rank 0.
  if (rank_in_node != 0) {
    ...
    memcpy(
        &rank0_handle,
        all_handles_buf[1].get() + node_rank0_global * CUDA_IPC_HANDLE_SIZE,
        CUDA_IPC_HANDLE_SIZE);
    void* ptr;
    CUDA_CHECK(cudaIpcOpenMemHandle(
        &ptr, rank0_handle, cudaIpcMemLazyEnablePeerAccess));
    ep_group->ht_buffers.intra_node_write_completion_flags =
...
static ncclResult_t destroy_hybridep_intranode(ncclEpGroup_t ep_group) {
  if (!ep_group->ht_buffers.initialized)
    return ncclSuccess;
  ...
  // Close IPC handles: 1 mega-buffer handle per node peer + 1 completion flag
  // handle
  for (int i = 0; i < gpus_per_node; i++) {
  ...
  // Free merged completion flags (rank_in_node 0 only; base pointer covers both
  ...
  if (ep_group->ht_buffers.dispatch_expert_output_scaling_factor_buffer_ptrs) {
    cudaFreeHost(
        ep_group->ht_buffers.dispatch_expert_output_scaling_factor_buffer_ptrs);
  }
...
// Undef any existing definition from internal NCCL headers to avoid using
// ncclDebugLog
#ifdef NCCLCHECK
#undef NCCLCHECK
#endif
...
static ncclResult_t init_hybridep_internode(
  ...
  // HT internode always uses physical node semantics: RDMA groups = nNodes, IPC
  // = gpus_per_node. MNNVL (multiple nodes with NVLink fabric) is treated like
  // regular multi-node RDMA for HT.
  int gpus_per_node = ep_group->gpus_per_node;
  int rank_in_node = ep_group->rank_in_node;
  int node_id = ep_group->node_id;
  ...
  // These buffers are accessed with stride MAX_SUPPORTED_TOKENS_PER_RANK
  // (compile-time constant used as rdma_remote_node_id *
  // MAX_SUPPORTED_TOKENS_PER_RANK + token_offset in the kernel). They must be
  // sized for that stride regardless of the runtime max_tokens_per_rank.
  size_t rdma_intra_node_red_token_sz = align_size(
      static_cast<size_t>(MAX_SUPPORTED_TOKENS_PER_RANK * (nNodes - 1)) *
          ep_group->hidden * sizeof(uint16_t),
      GIN_ALIGNMENT);
  size_t combine_rdma_inter_node_group_token_sz = rdma_intra_node_red_token_sz;
  size_t rdma_intra_node_red_prob_sz = align_size(
      static_cast<size_t>(MAX_SUPPORTED_TOKENS_PER_RANK * (nNodes - 1)) *
          (ep_group->num_local_experts * n_ranks_per_node) * sizeof(float),
      GIN_ALIGNMENT);
  size_t combine_rdma_inter_node_group_prob_sz = rdma_intra_node_red_prob_sz;
  size_t flags_sz =
      align_size(static_cast<size_t>(nNodes) * sizeof(uint64_t), GIN_ALIGNMENT);
  ...
  size_t scaling_factor_staging_sz = align_size(
      static_cast<size_t>(ep_group->config.max_tokens_per_rank) * sizeof(float),
      GIN_ALIGNMENT);

  size_t bytes_per_token_entry = ep_group->hidden * sizeof(uint16_t);
  size_t bytes_per_prob_entry =
      (ep_group->num_local_experts * n_ranks_per_node) * sizeof(float);
  size_t bytes_per_sf_entry = (ep_group->hidden / 128) * sizeof(float);
  size_t bytes_per_entry =
      bytes_per_token_entry + bytes_per_prob_entry + bytes_per_sf_entry;
  size_t rdma_send_staging_sz = align_size(
      static_cast<size_t>(nNodes - 1) * ep_group->config.max_tokens_per_rank *
          bytes_per_entry,
      GIN_ALIGNMENT);
  ...
  NCCLCHECK(
      ncclMemAlloc(&ep_group->gin_config.gin_base_ptr, total_gin_buffer_size));

  // Partition the buffer into individual regions
  uint8_t* ptr = reinterpret_cast<uint8_t*>(ep_group->gin_config.gin_base_ptr);
  size_t offset = 0;

  ep_group->ht_buffers.rdma_intra_node_red_token =
      reinterpret_cast<uint16_t*>(ptr + offset);
  offset += rdma_intra_node_red_token_sz;

  ep_group->ht_buffers.combine_rdma_inter_node_group_token =
      reinterpret_cast<uint16_t*>(ptr + offset);
  offset += combine_rdma_inter_node_group_token_sz;

  ep_group->ht_buffers.rdma_intra_node_red_prob =
      reinterpret_cast<float*>(ptr + offset);
  offset += rdma_intra_node_red_prob_sz;

  ep_group->ht_buffers.combine_rdma_inter_node_group_prob =
      reinterpret_cast<float*>(ptr + offset);
  offset += combine_rdma_inter_node_group_prob_sz;

  ep_group->ht_buffers.rdma_inter_node_group_flags =
      reinterpret_cast<uint64_t*>(ptr + offset);
  CUDACHECK_RET(cudaMemset(
      ep_group->ht_buffers.rdma_inter_node_group_flags, 0, flags_sz));
  offset += flags_sz;

  ep_group->ht_buffers.combine_rdma_inter_node_group_flags =
      reinterpret_cast<uint64_t*>(ptr + offset);
  CUDACHECK_RET(cudaMemset(
      ep_group->ht_buffers.combine_rdma_inter_node_group_flags, 0, flags_sz));
  offset += flags_sz;

  ep_group->ht_buffers.token_staging_buffer =
      reinterpret_cast<void*>(ptr + offset);
  offset += token_staging_sz;

  ep_group->ht_buffers.dense_prob_buffer =
      reinterpret_cast<float*>(ptr + offset);
  offset += dense_prob_sz;

  ep_group->ht_buffers.scaling_factor_staging_buffer =
      reinterpret_cast<float*>(ptr + offset);
  ...
  // =========================================================================
  // Node concept: color = rank's position within its physical node
  // (rank_in_node). Groups same-node-position ranks across nodes into one
  // split_comm for GIN/RDMA ops. key = node_id orders ranks within each
  // split_comm by their node index.
  int color = rank_in_node;
  int key = node_id;
  NCCLCHECK(ncclCommSplit(
      ep_group->comm, color, key, &ep_group->gin_config.split_comm, nullptr));

  int qps_per_rank = ep_group->config.num_qp_per_rank;
  int min_required_ctx =
      HYBRIDEP_DISPATCH_NUM_OF_BLOCKS * HYBRIDEP_DISPATCH_N2N_WARPS;
  if (qps_per_rank == 0)
  ...
  int max_chunks_per_rank =
      (MAX_SUPPORTED_TOKENS_PER_RANK + HT_OF_NUM_TOKENS_PER_CHUNK - 1) /
  ...
  int streaming_tail_signals =
      nNodes * nNodes * n_ranks_per_node * max_chunks_per_rank;
...

  {
    ...
    NCCL_CHECK_RESULT(
        ncclCommQueryProperties(ep_group->gin_config.split_comm, &props));
    if (props.ginType == NCCL_GIN_TYPE_NONE) {
      fprintf(
          stderr,
          "[HT GIN] Error: NCCL EP internode requires GIN, but GIN is not supported\n");
      return ncclInvalidUsage;
    }
  }

  {
    ...
    NCCLCHECK(ncclDevCommCreate(
        ep_group->gin_config.split_comm,
        &reqs,
        &ep_group->gin_config.dcomms[0]));
  }
...
static ncclResult_t destroy_hybridep_internode(ncclEpGroup_t ep_group) {
  if (!ep_group->ht_buffers.internode_initialized)
    return ncclSuccess;
  ...
  if (ep_group->gin_config.dcomms != nullptr) {
    if (ep_group->gin_config.split_comm) {
      ncclResult_t res = ncclDevCommDestroy(
          ep_group->gin_config.split_comm, &ep_group->gin_config.dcomms[0]);
      if (res != ncclSuccess) {
        fprintf(
            stderr,
            "[HT GIN] Warning: Failed to destroy device comm: %s\n",
            ncclGetErrorString(res));
      }
    }
  ...
  if (ep_group->gin_config.gin_base_ptr != nullptr) {
    ncclCommWindowDeregister(
        ep_group->gin_config.split_comm, ep_group->gin_config.nccl_window);
    ep_group->gin_config.nccl_window = {};
  }
  ...
  if (ep_group->gin_config.gin_base_ptr != nullptr) {
    ...
    if (res != ncclSuccess) {
      fprintf(
          stderr,
          "[HT GIN] Warning: Failed to free GIN memory: %s\n",
          ncclGetErrorString(res));
    }
...
ncclResult_t ncclEpCreateGroup(
  ...
  int nRanks;
  assert(
      comm != nullptr && ncclCommCount(comm, &nRanks) == ncclSuccess &&
      nRanks > 0);
  assert(
      in_config->version == 1 &&
      "ncclEpCreateGroup: invalid config version (expected 1)");
  assert(
      (in_config->algorithm == NCCL_EP_ALGO_LOW_LATENCY ||
  ...
  assert(
      in_config->num_experts > 0 &&
      "ncclEpCreateGroup: num_experts must be greater than 0");
  assert(
      in_config->token_size_bytes > 0 &&
      "ncclEpCreateGroup: token_size_bytes must be greater than 0");
  assert(
      !(in_config->algorithm == NCCL_EP_ALGO_LOW_LATENCY &&
        in_config->max_tokens_per_rank == 0) &&
  ...
  // Query LSA team size: number of ranks reachable via NVLink/LSA from this
  // rank.
  ...
  *out_ep_group = new (raw_memory) ncclEpGroup();
  ...
  // C-style cast required: cudaMalloc/cudaFree are overloaded and
  // reinterpret_cast cannot disambiguate overloaded functions.
  ...
  gethostname(
      all_hostnames.data() + ep_group->rank * HOSTNAME_LEN, HOSTNAME_LEN);
  ncclAllGatherHost(
      all_hostnames.data(),
      HOSTNAME_LEN,
      ep_group->rank,
  ...
  ep_group->max_recv_tokens =
      ep_group->nRanks * static_cast<int>(ep_group->config.max_tokens_per_rank);
  ep_group->hidden =
      ep_group->config.token_size_bytes / ncclTypeSize(ncclBfloat16);
  ...
  if (ep_group->config.rdma_buffer_size == NCCL_EP_AUTO &&
      ep_group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ep_group->config.rdma_buffer_size = nccl_ep::get_low_latency_rdma_size_hint(
        ep_group->config.max_tokens_per_rank,
        ep_group->hidden,
        ep_group->nRanks,
  ...
  CUDA_CHECK(
      cudaMemsetAsync(ep_group->ep_workspace, 0, NUM_WORKSPACE_BYTES, stream));
  ...
  if (!props.deviceApiSupport) {
    fprintf(
        stderr,
        "Error: NCCL EP requires NCCL Device API support, but Device API is not supported\n");
    return ncclInvalidUsage;
  }
  ...
  if (ep_group->config.rdma_buffer_size > 0 && low_latency_mode) {
    ...
    NCCL_CHECK_RESULT(ncclMemAlloc(
        &ep_group->rdma_buffer, ep_group->config.rdma_buffer_size));

    // Clean buffer (mainly for low-latency mode)
    CUDA_CHECK(cudaMemset(
        ep_group->rdma_buffer, 0, ep_group->config.rdma_buffer_size));
    ...
    ep_group->num_dispatch_signals =
        ep_group->num_local_experts * ep_group->nRanks;
    ...
    if (props.nLsaTeams > 1 && props.ginType == NCCL_GIN_TYPE_NONE) {
      fprintf(
          stderr,
          "[LL] Error: NCCL EP requires GIN, but GIN is not supported\n");
      return ncclInvalidUsage;
    }

    ncclDevCommRequirements reqs = NCCL_DEV_COMM_REQUIREMENTS_INITIALIZER;
    if (props.nLsaTeams > 1) {
      reqs.ginContextCount =
          ep_group->config.num_qp_per_rank; // all contexts in single comm
      // Signal layout: combine uses [0, num_total_signals), dispatch uses
      // [num_total_signals, 2*num_total_signals)
      reqs.ginSignalCount = 2 * num_total_signals;
      reqs.ginForceEnable = true;
      reqs.ginConnectionType = NCCL_GIN_CONNECTION_FULL;
    }
    NCCL_CHECK_RESULT(
        ncclDevCommCreate(ep_group->comm, &reqs, &nccl_dev_comms_host[0]));

    CUDA_CHECK(cudaMalloc(
        reinterpret_cast<void**>(&ep_group->nccl_dev_comms),
    ...
    NCCL_CHECK_RESULT(ncclCommWindowRegister(
        ep_group->comm,
        ep_group->rdma_buffer,
        ep_group->config.rdma_buffer_size,
        &nccl_wins_host[0],
        0));

    CUDA_CHECK(cudaMalloc(
        reinterpret_cast<void**>(&ep_group->nccl_wins), sizeof(ncclWindow_t)));
    CUDA_CHECK(cudaMemcpy(
        ep_group->nccl_wins,
        nccl_wins_host,
        sizeof(ncclWindow_t),
...
ncclResult_t ncclEpGroupDestroy(ncclEpGroup_t ep_group, cudaStream_t stream) {
  ...
  // Clean up HT internode resources (GIN deregistration must happen before
  // ncclCommDestroy)
  if (ep_group->config.algorithm == NCCL_EP_ALGO_HIGH_THROUGHPUT &&
  ...
  // Clean up RDMA resources (single-comm path: 1 window, 1 devcomm on
  // ep_group->comm)
  if (ep_group->config.rdma_buffer_size > 0 &&
      NCCL_EP_ALGO_LOW_LATENCY == ep_group->config.algorithm) {
    CUDA_CHECK(cudaDeviceSynchronize());
    NCCL_CHECK_RESULT(ncclBarrier(ep_group->comm, stream));

    // Deregister single NCCL window (copy back from device, deregister on
    // ep_group->comm)
    ...
    // Destroy single NCCL device communicator (copy back from device, destroy
    // on ep_group->comm)
...
ncclResult_t ncclEpTensorCreate(
  ...
  if (data != nullptr) {
    ...
    CUDA_CHECK(
        ep_group->alloc_fn(&t->data, total_size * ncclTypeSize(datatype)));
    t->owns_data = true;
  }
  *tensor = t;
  return ncclSuccess;
...
struct ncclEpHandle {
  ...
  union {
    ...
    struct {
      ...
      // (bitmap format). Contains complete routing information needed for
      // preprocessing. dtype: uint8_t (bitmap: 1 bit per expert, 8 experts per
      // byte) layout: [total_tokens, ceil(num_experts / 8)] lifetime: valid
      // after ncclAllGather in ncclEpCreateHandle
      uint8_t* global_routing_map;

      // =================================================================================
      // PREPROCESSING OUTPUTS - Computed once per iteration, used by dispatch &
      // combine
      // =================================================================================

      // Sparse-to-dense map: maps each (token, source_rank) pair to its
      // position in the destination rank's expert buffer. Used by NVLink
      // (intra-node) warps. Value of -1 indicates token is not routed to that
      // rank. dtype: int32_t layout: [num_nodes * max_tokens_per_rank,
      // num_ranks_per_node] usage: dispatch S2G warp group, combine G2S warp
      // group lifetime: valid after metadata_preprocessing, constant within
      // iteration vs NCCL HT: no direct equivalent.
      //   - HT: uses is_token_in_rank (local tokens only) + atomics to compute
      //   positions
      //   - HT: precomputes positions for ALL nodes' tokens, -1 sentinel for
      //   not routed
      int32_t* sparse_to_dense_map;

      // RDMA-to-attention map: boolean mask indicating which tokens this node
      // needs to RECEIVE from RDMA (inter-node). Indexed by [node_id,
      // token_id]. Primarily used during combine to know which remote tokens to
      // wait for. dtype: bool layout: [num_nodes,
      // max_tokens_per_rank_padded_to_16]
      //        (padding to 16 required for TMA alignment)
      // usage: dispatch G2S warp (polling), combine inter-node G2S/reduction
      // warps lifetime: valid after metadata_preprocessing, constant within
      // iteration vs NCCL HT: inverse perspective of is_token_in_rank.
      //   - is_token_in_rank: outbound - "where do MY tokens go?" [my_token,
      //   dest_rank]
      //   - rdma_to_attn_map: inbound - "which remote tokens do I receive?"
      //   [src_node, token]
      bool* rdma_to_attn_map;

      // Attention-to-RDMA map: boolean mask indicating which local tokens need
      // to be SENT via RDMA (inter-node) to each remote node. Only allocated
      // when num_nodes > 1. dtype: bool layout: [max_tokens_per_rank, num_nodes
      // - 1] usage: dispatch N2N (RDMA) warp group lifetime: valid after
      // metadata_preprocessing, constant within iteration vs NCCL HT: closest
      // equivalent to is_token_in_rank for inter-node RDMA.
      //   - is_token_in_rank: per-rank granularity [num_tokens, num_ranks]
      //   - attn_to_rdma_map: per-node granularity [num_tokens, num_nodes-1]
      //   (RDMA only)
      ...
      // Local expert routing map: per-expert routing for tokens in this rank's
      // buffer. Used by subsequent expert MLP layers to route tokens to correct
      // experts. dtype: bool layout: [max_recv_tokens, experts_per_rank]
      //        where max_recv_tokens = num_ranks * max_tokens_per_rank
      // usage: passed to expert computation layers (not used by
      // dispatch/combine directly) lifetime: valid after
      // metadata_preprocessing, constant within iteration vs NCCL HT: similar
      // purpose to num_tokens_per_expert but more detailed
      bool* local_expert_routing_map;

      // Number of tokens routed to local experts (total across all local
      // experts). Each token counted once even if routed to multiple local
      // experts. dtype: int32_t layout: [1] usage: buffer sizing, iteration
      // control lifetime: valid after metadata_preprocessing
      ...
      // =================================================================================
      // CONVERSION BUFFERS - Pre-allocated to avoid dispatch/combine-time
      // malloc
      // =================================================================================
      ...
      // Token staging buffer: pre-registered buffer to avoid GIN registration
      // during dispatch User tokens are copied here during dispatch, then this
      // buffer is used for RDMA dtype: uint16_t (bf16) or uint8_t (fp8) layout:
      // [max_tokens_per_rank, hidden] usage: copy user tokens → use for
      // inter-node RDMA lifetime: group-owned (allocated in Group Create, freed
      // in Group Destroy)
      void* token_staging_buffer; // Pointer to group-level buffer (not
                                  // handle-owned)

      // Scaling factor staging buffer: pre-registered buffer for FP8 scaling
      // factors User scaling factors are copied here during dispatch, then this
      // buffer is used for RDMA dtype: float layout: [max_tokens_per_rank]
      // usage: copy user scaling factors → use for inter-node RDMA
      // lifetime: group-owned (allocated in Group Create, freed in Group
      // Destroy)
      float* scaling_factor_staging_buffer; // Pointer to group-level buffer
                                            // (not handle-owned)

      // RDMA inter-node group flags: atomic completion flags for each remote
      // node. Remote ranks increment via RDMA atomic fetch-add to signal chunk
      // completion. Only allocated when num_nodes > 1. dtype: uint64_t layout:
      // [num_nodes - 1] usage: dispatch N2N warp (signaling), dispatch G2S warp
      // (polling) lifetime: reset to 0 at init, incremented by remote RDMA
      // atomics
      // uint64_t* rdma_inter_node_group_flags;

      // Per-handle preprocessing block (single allocation for all preprocessing
      // buffers)
...
// static const ncclNDTensor_t* find_tensor_by_tag(const ncclNDTensor_t* const*
// tensors, int num_tensors, ncclEpTensorTag_t tag) {
...
static ncclNDTensor_t find_tensor_by_tag(
    const ncclNDTensor_t* tensors,
    int num_tensors,
    ncclEpTensorTag_t tag) {
  for (int i = 0; i < num_tensors; i++) {
    if (tensors[i]->tag == tag)
...
static bool is_internode_available(ncclEpGroup_t ep_group) {
  // True when there are multiple physical nodes. HT always uses node-based RDMA
  // across nodes, including MNNVL (which has NVLink fabric but is treated as
  // regular multi-node for HT).
...
ncclResult_t ncclEpCreateHandle(
  ...
  *out_handle = new ncclEpHandle();
  ...
  if (ep_group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ...
    assert(
        num_local_tensors == 0 &&
        "LL mode does not accept local tensors in ncclEpCreateHandle");

    // Allocate packed tensors
    // packed_recv_x is the input tensor in the dispatch
    ncclEpTensorCreate(
        ep_group,
        &handle->ll.expert_recv_source_indices,
        2,
        ncclInt32,
        NCCL_EP_TENSOR_TAG_NONE,
        nullptr,
        static_cast<unsigned int>(handle->group->num_local_experts),
        static_cast<unsigned int>(
            ep_group->nRanks * ep_group->config.max_tokens_per_rank));
    ncclEpTensorCreate(
        ep_group,
        &handle->ll.expert_dispatch_layout,
        2,
        ncclInt64,
        NCCL_EP_TENSOR_TAG_NONE,
        nullptr,
    ...
    assert(
        (ep_group->config.max_tokens_per_rank *
         handle->group->num_local_experts) %
                4 ==
            0 and
        "TMA requires the number of tokens to be multiple of 4");

    handle->ll.layout = nccl_ep::LowLatencyLayout(
        handle->group->rdma_buffer,
        handle->group->config.max_tokens_per_rank,
        handle->group->hidden,
        handle->group->nRanks,
        handle->group->config.num_experts);

    assert(
        handle->ll.layout.total_bytes <=
        handle->group->config.rdma_buffer_size);
  } else { // HT
    assert(
        ep_group->config.max_tokens_per_rank > 0 &&
        "HT requires max_tokens_per_rank > 0");
    assert(
        handle->num_tokens <=
    ...
    if (num_local_tensors > 0) {
      recv_expert_counter = find_tensor_by_tag(
          local_tensors,
          num_local_tensors,
    ...
    // Allocate preprocessing buffers per-handle (single block allocation)
    {
      ...
      size_t sz_routing =
          align256(static_cast<size_t>(total_send_tokens) * num_experts_packed);
      size_t sz_r2a = align256(
          static_cast<size_t>(nNodes) * padded_max_tokens * sizeof(bool));
      size_t sz_a2r = (nNodes > 1)
          ? align256(
                static_cast<size_t>(max_tokens) * (nNodes - 1) * sizeof(bool))
          : 0;
      size_t sz_ler = align256(
          static_cast<size_t>(ep_group->max_recv_tokens) * experts_per_rank *
          sizeof(bool));
      size_t sz_ntfe = align256(sizeof(int32_t));
      size_t zero_region = sz_routing + sz_r2a + sz_a2r + sz_ler + sz_ntfe;

      size_t sz_s2d = align256(
          static_cast<size_t>(nNodes) * max_tokens * n_ranks_per_node *
          sizeof(int32_t));
      size_t sz_rank_mask = align256(
          static_cast<size_t>(nNodes) * max_tokens * n_ranks_per_node *
          sizeof(uint8_t));

      size_t sz_scan_tmp = align256(
          nccl_ep::hybridep::get_preprocessing_scan_tmp_size(n_ranks_per_node));
      size_t sz_prob = !is_internode_available(ep_group)
          ? align256(
                static_cast<size_t>(max_tokens) * num_experts * sizeof(float))
          : 0;
      size_t sz_per_expert_counts_tmp =
          align256(static_cast<size_t>(experts_per_rank) * sizeof(int32_t));
      size_t no_memset_region =
          sz_rank_mask + sz_scan_tmp + sz_prob + sz_per_expert_counts_tmp;

      size_t total_size = zero_region + sz_s2d + no_memset_region;

      CUDA_CHECK(ep_group->alloc_fn(
          &handle->hybridep.preprocessing_block, total_size));
      ...
      handle->hybridep.attn_to_rdma_map =
          (nNodes > 1) ? reinterpret_cast<bool*>(ptr + offset) : nullptr;
      offset += sz_a2r;

      handle->hybridep.local_expert_routing_map =
          reinterpret_cast<bool*>(ptr + offset);
      offset += sz_ler;

      handle->hybridep.num_tokens_for_experts =
          reinterpret_cast<int32_t*>(ptr + offset);
      offset += sz_ntfe;
      ...
      if (!is_internode_available(ep_group)) {
        ...
        handle->hybridep.dense_prob_buffer =
            reinterpret_cast<float*>(ptr + offset);
        offset += sz_prob;
      } else {
    ...
    // For multi-node: dense_prob_buffer is the group-level GIN-registered
    // buffer
    if (is_internode_available(ep_group)) {
      handle->hybridep.dense_prob_buffer =
          ep_group->ht_buffers.dense_prob_buffer;
    ...
    *handle->hybridep.num_tokens_for_experts_host = -1;
    ...
    const int padded_num_tokens =
        ((handle->num_tokens + 15) / 16) * 16; // rdma_to_attn_map uint4 loads
    const size_t routing_send_bytes =
        static_cast<size_t>(handle->num_tokens) * num_experts_packed;
    const size_t rdma_to_attn_bytes =
        static_cast<size_t>(nNodes) * padded_num_tokens * sizeof(bool);
    const size_t attn_to_rdma_bytes = (nNodes > 1)
        ? static_cast<size_t>(handle->num_tokens) * (nNodes - 1) * sizeof(bool)
        : 0;
    const size_t local_expert_routing_bytes =
        static_cast<size_t>(ep_group->max_recv_tokens) * experts_per_rank *
        sizeof(bool);
    const size_t sparse_to_dense_bytes = static_cast<size_t>(nNodes) *
        handle->num_tokens * n_ranks_per_node * sizeof(int32_t);

    // convert_topk_to_routing_map uses bitwise-OR into this buffer, so local
    // send rows must be pre-zeroed.
    uint8_t* local_routing_send_ptr = handle->hybridep.global_routing_map +
        (max_tokens * num_experts_packed) * ep_group->rank;
    if (routing_send_bytes > 0) {
      CUDA_CHECK(cudaMemsetAsync(
          local_routing_send_ptr, 0, routing_send_bytes, stream));
    }
    if (rdma_to_attn_bytes > 0) {
      CUDA_CHECK(cudaMemsetAsync(
          handle->hybridep.rdma_to_attn_map, 0, rdma_to_attn_bytes, stream));
    }
    if (attn_to_rdma_bytes > 0) {
      CUDA_CHECK(cudaMemsetAsync(
          handle->hybridep.attn_to_rdma_map, 0, attn_to_rdma_bytes, stream));
    }
    // Keep full clear for local_expert_routing_map to preserve current dispatch
    // output behavior when callers allocate outputs at max receive capacity.
    CUDA_CHECK(cudaMemsetAsync(
        handle->hybridep.local_expert_routing_map,
        0,
        local_expert_routing_bytes,
        stream));
    CUDA_CHECK(cudaMemsetAsync(
        handle->hybridep.num_tokens_for_experts,
        0,
        sizeof(*handle->hybridep.num_tokens_for_experts),
        stream));
    if (sparse_to_dense_bytes > 0) {
      CUDA_CHECK(cudaMemsetAsync(
          handle->hybridep.sparse_to_dense_map,
          0xFF,
          sparse_to_dense_bytes,
          stream));
    }
    ...
    if (recv_expert_counter != nullptr) {
      if (recv_expert_counter != nullptr) {
        assert(
            recv_expert_counter->ndim == 1 && "recv_expert_counter must be 1D");
        assert(
            recv_expert_counter->datatype == ncclInt32 &&
            "recv_expert_counter must be ncclInt32");
        assert(
            recv_expert_counter->sizes[0] >=
                static_cast<unsigned int>(ep_group->num_local_experts) &&
            "recv_expert_counter size must be >= num_local_experts");
        assert(
            recv_expert_counter->data != nullptr &&
            "recv_expert_counter data must not be null");
      }

      if (recv_expert_counter->tag ==
          NCCL_EP_TENSOR_TAG_RECV_EXPERT_COUNTER_HOST) {
        // Fused preprocessing uses atomics; avoid atomics directly into mapped
        // host memory.
        ...
        per_expert_counts_device =
            static_cast<int32_t*>(recv_expert_counter->data);
      }
    }
    ...
    // Mirror the actual recv-token count to host once; getters can read this
    // cached value.
    CUDA_CHECK(cudaMemcpyAsync(
...
ncclResult_t ncclEpDispatch(
  ...
  if (group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ncclNDTensor_t x =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_TOKENS);
    assert(x != nullptr);
    assert(x->ndim == 2);
    ...
    assert(
        x->sizes[1] * ncclTypeSize(x->datatype) ==
        group->config.token_size_bytes);

    // Find and validate output tensors
    ncclNDTensor_t recv_x =
        find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOKENS);
    ncclNDTensor_t scales =
        find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_SCALES);
    assert(recv_x != nullptr);
    assert(recv_x->ndim == 3);
    assert(tensor_is_contiguous(recv_x));
    assert(recv_x->sizes[0] == group->num_local_experts);
    assert(
        recv_x->sizes[1] == group->nRanks * group->config.max_tokens_per_rank);
    ...
    if (scales != nullptr) {
      ...
      assert(
          scales->sizes[1] ==
          group->config.max_tokens_per_rank * group->nRanks);
    ...
    auto dispatch_fn = [=](int phases) {
      ...
      auto* expert_recv_source_indices_data =
          static_cast<int*>(handle->ll.expert_recv_source_indices->data);
      auto* expert_dispatch_layout_data =
          static_cast<int64_t*>(handle->ll.expert_dispatch_layout->data);
      ...
      nccl_ep::internode_ll::dispatch(
          ...
          (reinterpret_cast<uint64_t>(buffer.dispatch_rdma_send_buffer) -
           reinterpret_cast<uint64_t>(group->rdma_buffer)),
          (reinterpret_cast<uint64_t>(buffer.dispatch_rdma_recv_data_buffer) -
           reinterpret_cast<uint64_t>(group->rdma_buffer)),
          (reinterpret_cast<uint64_t>(buffer.dispatch_rdma_recv_count_buffer) -
           reinterpret_cast<uint64_t>(group->rdma_buffer)),
    ...
    assert(
        num_local_tensors == 0 && "HT dispatch does not accept local_tensors");

    ncclNDTensor_t x =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_TOKENS);
    ncclNDTensor_t topk_idx =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_TOPK_IDX);
    ncclNDTensor_t topk_weights =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_TOPK_WEIGHTS);
    ncclNDTensor_t scales =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_SCALES);
    ...
    if (!is_single_node && handle->hybridep.token_staging_buffer != nullptr) {
      // Copy user tokens to pre-registered staging buffer (D2D copy is ~0.1ms
      // vs ~30ms GIN registration)
    ...
    bool use_fp8 =
        (x->datatype == ncclFloat8e4m3 || x->datatype == ncclFloat8e5m2);

    // For FP8: copy user scaling factors to pre-registered staging buffer
    float* scales_ptr = use_fp8 ? static_cast<float*>(scales->data)
                                : nullptr; // Default: use user buffer directly
    if (use_fp8 && !is_single_node &&
        handle->hybridep.scaling_factor_staging_buffer != nullptr) {
      // Copy user scaling factors to pre-registered staging buffer (D2D copy is
      // ~0.1ms vs ~30ms GIN registration)
    ...
    const int experts_per_node =
        group->num_local_experts * group->lsa_rank_count;
    assert(
    ...
    if (use_fp8) {
      ...
      assert(
          (((group->hidden / 128) * static_cast<int>(sizeof(float))) % 16) ==
              0 &&
          "HT dispatch FP8 requires scaling-factor bytes per token to be 16B aligned");
    }

    // Output tensors
    ncclNDTensor_t recv_topk_weights = find_tensor_by_tag(
        outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOPK_WEIGHTS);
    ncclNDTensor_t recv_topk_idx =
        find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOPK_IDX);
    ...
    if (forward_dispatch) {
      ...
      assert(
          topk_idx->ndim == 2 && tensor_is_contiguous(topk_idx) &&
          topk_idx->datatype == ncclInt64);
      assert(
          topk_weights->ndim == 2 && tensor_is_contiguous(topk_weights) &&
          topk_weights->datatype == ncclFloat32);
     ...
     * ===== */
    // NCCL HT uses sparse format: topk_idx[token][k] = expert_id,
    // topk_weights[token][k] = weight HT uses dense format: prob[token][expert]
    // = weight (0 if not routed)
    float* dense_prob = handle->hybridep.dense_prob_buffer;
    if (forward_dispatch) {
      size_t dense_prob_size = static_cast<size_t>(handle->num_tokens) *
          group->config.num_experts * sizeof(float);
    ...
    // DispatchParams encapsulates all buffers and metadata needed by HT
    // dispatch kernel:
    //   - Input buffers: attn_input_token, attn_input_prob,
    //   attn_input_scaling_factor
    //   - IPC output buffers: expert_output_token_ptrs, expert_output_prob_ptrs
    //   (per-rank pointers)
    ...
    params.attn_input_scaling_factor =
        use_fp8 ? static_cast<const float*>(scales_ptr) : nullptr;
    // Use HOST pointer arrays - these get copied into the kernel param struct
    // for fast __grid_constant__ access
    params.expert_output_token_ptrs =
        group->ht_buffers.dispatch_expert_output_token_buffer_ptrs;
    params.expert_output_prob_ptrs =
        group->ht_buffers.dispatch_expert_output_prob_buffer_ptrs;
    params.expert_output_scaling_factor_ptrs = use_fp8
    ...
    params.expected_rdma_flag_value =
        is_single_node ? 0 : group->ht_buffers.host_dispatch_expected_rdma;
    params.rdma_inter_node_group_flags = is_single_node
        ? nullptr
        : group->ht_buffers.rdma_inter_node_group_flags;
    params.expected_intra_node_flag_value =
        group->ht_buffers.host_dispatch_expected_intra;
    params.intra_node_write_completion_flags =
        group->ht_buffers.intra_node_write_completion_flags;
    params.dispatch_grid_barrier_counter =
        group->ht_buffers.dispatch_grid_barrier_counter;
    // Pass device communicators and windows
    params.dcomms = is_single_node ? nullptr : group->gin_config.d_dcomms;
    params.nccl_window =
        is_single_node ? ncclWindow_t{} : group->gin_config.nccl_window;
    params.num_gin_comms = is_single_node ? 0 : group->gin_config.num_comms;
    params.num_ctx_per_comm =
        is_single_node ? 0 : group->gin_config.num_ctx_per_comm;
    params.gin_base_ptr =
        is_single_node ? nullptr : group->gin_config.gin_base_ptr;
    ...
    size_t bytes_per_token_entry =
        group->hidden * sizeof(uint16_t); // token data
    size_t bytes_per_prob_entry =
        (group->num_local_experts * group->lsa_rank_count) *
        sizeof(float); // prob data
    size_t bytes_per_sf_entry =
        (group->hidden / 128) * sizeof(float); // scaling factor (FP8)
    size_t bytes_per_entry =
        bytes_per_token_entry + bytes_per_prob_entry + bytes_per_sf_entry;

    params.mr_info = {
        .attn_input_token_offset =
            is_single_node ? 0 : group->gin_config.token_staging_offset,
        .attn_input_prob_offset =
            is_single_node ? 0 : group->gin_config.dense_prob_offset,
        .attn_input_scaling_factor_offset = is_single_node
            ? 0
            : group->gin_config.scaling_factor_staging_offset,
        // Batched staging parameters (packed layout)
        .rdma_send_staging_offset =
            is_single_node ? 0 : group->gin_config.rdma_send_staging_offset,
        .rdma_inter_node_group_packed_offset = is_single_node
            ? 0
            : group->gin_config.rdma_inter_node_group_packed_offset,
        .bytes_per_entry = bytes_per_entry,
        .max_tokens_per_dest =
            static_cast<size_t>(group->config.max_tokens_per_rank),
        // Streaming signal parameters
        .signals_tail_base = is_single_node
            ? 0
            : static_cast<unsigned>(group->gin_config.signals_tail_base),
        .num_max_rdma_chunked_send_tokens = is_single_node
            ? 0
            : group->gin_config.num_max_rdma_chunked_send_tokens,
    };
    ...
    // HT kernel writes to IPC-mapped buffers
    // (dispatch_expert_output_*_buffer_ptrs) Copy results to user-provided
    // output tensors
    ncclNDTensor_t recv_x =
        find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOKENS);
    if (recv_x != nullptr) {
      assert(recv_x->ndim == 2 && tensor_is_contiguous(recv_x));
      size_t copy_size = static_cast<size_t>(recv_x->sizes[0]) *
          recv_x->sizes[1] * ncclTypeSize(recv_x->datatype);

      CUDA_CHECK(cudaMemcpyAsync(
          recv_x->data,
          group->ht_buffers
              .dispatch_expert_output_token_buffer_ptrs[group->rank_in_node],
          copy_size,
          cudaMemcpyDeviceToDevice,
          stream));
    }
    ...
    if (forward_dispatch) {
      // Convert outputs - both recv_topk_weights and recv_topk_idx must be
      // provided together
      if (recv_topk_weights != nullptr && recv_topk_idx != nullptr) {
        // Validate formats for both outputs
        assert(
            recv_topk_weights->ndim == 2 &&
            tensor_is_contiguous(recv_topk_weights));
        ...
        nccl_ep::hybridep::dense_to_sparse_prob(
            group->ht_buffers
                .dispatch_expert_output_prob_buffer_ptrs[group->rank_in_node],
            handle->hybridep.local_expert_routing_map,
    ...
    if (use_fp8) {
      ncclNDTensor_t recv_scales =
          find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_SCALES);
      if (recv_scales != nullptr) {
        assert(recv_scales->ndim == 2 && tensor_is_contiguous(recv_scales));
        size_t copy_size = static_cast<size_t>(recv_scales->sizes[0]) *
...
ncclResult_t ncclEpCombine(
  ...
  if (handle->group->config.algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ...
    const int num_max_dispatch_tokens_per_rank =
        handle->group->config.max_tokens_per_rank;
    ...
    unsigned signal_base =
        handle->ll.buffer_idx * (handle->group->num_dispatch_signals / 2);
    auto combine_fn = [=](int phases) {
      ...
      nccl_ep::internode_ll::combine(
          ...
          (reinterpret_cast<uint64_t>(buffer.combine_rdma_send_buffer) -
           reinterpret_cast<uint64_t>(handle->group->rdma_buffer)),
          (reinterpret_cast<uint64_t>(buffer.combine_rdma_recv_data_buffer) -
           reinterpret_cast<uint64_t>(handle->group->rdma_buffer)),
          (reinterpret_cast<uint64_t>(buffer.combine_rdma_recv_flag_buffer) -
           reinterpret_cast<uint64_t>(handle->group->rdma_buffer)),
          next_clean_meta.first,
          next_clean_meta.second,
    ...
    ncclNDTensor_t topk_weights =
        find_tensor_by_tag(inputs, num_inputs, NCCL_EP_TENSOR_TAG_TOPK_WEIGHTS);
    ncclNDTensor_t combined_topk_weights = find_tensor_by_tag(
        outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOPK_WEIGHTS);

    // Determine if this is backward mode (topk_weights provided = backward
    // combine)
    ...
    if (backward_combine) {
      ...
      assert(
          combined_topk_weights->ndim == 2 &&
          tensor_is_contiguous(combined_topk_weights));
    ...
    // HT combine doesn't use local tensors, so hybridep should not use local
    // tensors either
    assert(num_local_tensors == 0);

    /* ===== Output tensors ===== */
    ncclNDTensor_t combined_x =
        find_tensor_by_tag(outputs, num_outputs, NCCL_EP_TENSOR_TAG_TOKENS);
    assert(combined_x != nullptr);
    assert(combined_x->ndim == 2 && tensor_is_contiguous(combined_x));
    assert(
        combined_x->sizes[0] ==
        num_combined_tokens); // Output should match original token count
    assert(
        combined_x->sizes[1] == hidden); // Should match input hidden dimension

    /* ===== Copy input to IPC staging buffers ===== */
    // Expert MLP output needs to be in IPC buffer so other ranks can read it
    size_t token_copy_size = static_cast<size_t>(num_tokens) * hidden *
        sizeof(uint16_t); // BF16 = uint16_t
    CUDA_CHECK(cudaMemcpyAsync(
     ...
     * ===== */
    // For backward combine, convert sparse input weights to dense format for HT
    // kernel
    if (backward_combine) {
      int experts_per_node = group->num_local_experts * group->lsa_rank_count;
      size_t dense_prob_size =
          static_cast<size_t>(num_tokens) * experts_per_node * sizeof(float);

      // Zero-initialize the dense prob buffer before scattering
      CUDA_CHECK(cudaMemsetAsync(
          group->ht_buffers
              .combine_expert_input_prob_buffer_ptrs[group->rank_in_node],
          0,
          dense_prob_size,
          stream));

      // Convert sparse [num_tokens, topk] to dense [num_tokens,
      // experts_per_node] Uses local_expert_routing_map to determine expert
      // positions (matches dispatch output order)
    ...
    if (backward_combine) {
      size_t dense_output_prob_size = static_cast<size_t>(num_combined_tokens) *
          group->config.num_experts * sizeof(float);
      CUDA_CHECK(cudaMemsetAsync(
          dense_output_prob, 0, dense_output_prob_size, stream));
    }

    /* ===== Build CombineParams ===== */
    // CombineParams encapsulates all buffers and metadata needed by HT combine
    // kernel:
    //   - IPC input buffers: expert_input_token_ptrs, expert_input_prob_ptrs
    //   (per-rank pointers)
    //   - Output buffers: attn_output_token, attn_output_prob (user-provided)
    //   - RDMA buffers: rdma_intra_node_red_*, combine_rdma_inter_node_group_*
    //   (for multi-node)
    //   - Metadata: sparse_to_dense_map, rdma_to_attn_map, attn_to_rdma_map,
    //   local_expert_routing_map
    //   - Sync flags: combine_expected_*_flag_value,
    //   combine_intra_node_write_completion_flags
    ...
    // Use HOST pointer arrays - these get copied into the kernel param struct
    // for fast __grid_constant__ access
    params.expert_input_token_ptrs =
        group->ht_buffers.combine_expert_input_token_buffer_ptrs;
    params.expert_input_prob_ptrs = backward_combine
        ? group->ht_buffers.combine_expert_input_prob_buffer_ptrs
        : nullptr;
    params.attn_output_token = combined_x->data;
    params.attn_output_prob = backward_combine ? dense_output_prob : nullptr;
    params.rdma_intra_node_red_token =
        is_single_node ? nullptr : group->ht_buffers.rdma_intra_node_red_token;
    params.rdma_intra_node_red_prob = (!is_single_node && backward_combine)
        ? group->ht_buffers.rdma_intra_node_red_prob
        : nullptr;
    params.combine_rdma_inter_node_group_token = is_single_node
        ? nullptr
        : group->ht_buffers.combine_rdma_inter_node_group_token;
    params.combine_rdma_inter_node_group_prob =
        (!is_single_node && backward_combine)
        ? group->ht_buffers.combine_rdma_inter_node_group_prob
        : nullptr;
    params.sparse_to_dense_map = handle->hybridep.sparse_to_dense_map;
    params.rdma_to_attn_map = handle->hybridep.rdma_to_attn_map;
    params.attn_to_rdma_map = handle->hybridep.attn_to_rdma_map;
    ...
    params.combine_expected_rdma_flag_value =
        is_single_node ? 0 : group->ht_buffers.host_combine_expected_rdma;
    params.combine_rdma_inter_node_group_flags = is_single_node
        ? nullptr
        : group->ht_buffers.combine_rdma_inter_node_group_flags;
    params.combine_expected_intra_node_flag_value =
        group->ht_buffers.host_combine_expected_intra;
    params.combine_intra_node_write_completion_flags =
        group->ht_buffers.combine_intra_node_write_completion_flags;
    // Pass device communicators and windows
    params.dcomms = is_single_node ? nullptr : group->gin_config.d_dcomms;
    params.nccl_window =
        is_single_node ? ncclWindow_t{} : group->gin_config.nccl_window;
    ...
    // Use offsets relative to gin_base_ptr
    params.mr_info = {
        .rdma_intra_node_red_token_offset = is_single_node
            ? 0
            : group->gin_config.rdma_intra_node_red_token_offset,
        .combine_rdma_inter_node_group_token_offset = is_single_node
            ? 0
            : group->gin_config.combine_rdma_inter_node_group_token_offset,
        .rdma_intra_node_red_prob_offset = is_single_node
            ? 0
            : group->gin_config.rdma_intra_node_red_prob_offset,
        .combine_rdma_inter_node_group_prob_offset = is_single_node
            ? 0
            : group->gin_config.combine_rdma_inter_node_group_prob_offset,
    };
    params.local_rank = group->rank_in_node;
    params.node_rank = group->node_id;
    ...
    // HT outputs dense [num_tokens, num_experts], NCCL expects sparse
    // [num_tokens, topk]
    if (backward_combine && combined_topk_weights != nullptr) {
      nccl_ep::hybridep::dense_to_sparse_prob_combine(
          dense_output_prob,
          handle->hybridep.global_routing_map +
              (num_combined_tokens * ((group->config.num_experts + 7) / 8)) *
                  group->rank,
