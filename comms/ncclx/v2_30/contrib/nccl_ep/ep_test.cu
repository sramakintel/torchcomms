...
void printUsage(const char* programName, int myRank) {
  if (myRank == 0) {
    ...
    printf(
        "                               ll:  Low latency mode (NCCL_EP_ALGO_LOW_LATENCY)\n");
    printf(
        "                               ht:  High throughput mode (NCCL_EP_ALGO_HIGH_THROUGHPUT)\n");
    printf(
        "  -m                           Disable max_tokens_per_rank (only supported with HT mode)\n");
    printf(
        "  -s <none|dispatch|combine|both>  Set send_only mode (default: none)\n");
    printf(
        "                               none:     send_only=false for both dispatch and combine\n");
    printf(
        "                               dispatch: send_only=true for dispatch only\n");
    printf(
        "                               combine:  send_only=true for combine only\n");
    printf(
        "                               both:     send_only=true for both dispatch and combine\n");
    printf(
        "  -c                           Enable cached mode (default: disabled, only supported for HT)\n");
    printf(
        "  -r                           Enable random mode (random topk_idx, skip correctness checks)\n");
    printf(
        "  -t <num>                     Set number of tokens (default: 50)\n");
    printf(
        "  -d <num>                     Set hidden dimension size (default: 7168)\n");
...
int main(int argc, char* argv[]) {
  ...
  ncclEpAlgorithm_t algorithm =
      NCCL_EP_ALGO_LOW_LATENCY; // Default to 'll' (low latency)
    ...
    switch (opt) {
      ...
      case 't':
        ...
        if (num_tokens == 0) {
          if (myRank == 0) {
            printf(
                "Error: Invalid num_tokens '%s'. Must be a positive integer.\n",
                optarg);
            printUsage(argv[0], myRank);
          }
          MPI_Finalize();
      ...
      case 'd':
        ...
        if (hidden == 0) {
          if (myRank == 0) {
            printf(
                "Error: Invalid hidden size '%s'. Must be a positive integer.\n",
                optarg);
            printUsage(argv[0], myRank);
          }
          MPI_Finalize();
  ...
  // Not yet supported in the current release; code paths are kept for future
  // use.
  ...
  unsigned int top_k = std::min(
      8, nRanks); // DeepSeek v3 has 8, how many experts each token goes to
  ...
  unsigned int local_experts_end =
      local_experts_start + num_local_experts; // exclusive
  ...
  if (top_k > num_local_experts) {
    if (myRank == 0)
      printf(
          "Error: top_k (%u) must be less than or equal to num_local_experts (%u)\n",
          top_k,
          num_local_experts);
    MPI_Finalize();
    exit(EXIT_FAILURE);
  ...
  config.rdma_buffer_size =
      NCCL_EP_AUTO; // NCCL_EP_AUTO for auto configuration, internally uses the
                    // hint
  config.num_qp_per_rank =
      NCCL_EP_AUTO; // Default is 24, see internode_ll.cu:181 for the minimum
  config.num_channels = NCCL_EP_AUTO; // Number of communication channels

  const char* algorithm_name = (algorithm == NCCL_EP_ALGO_LOW_LATENCY)
      ? "LOW_LATENCY"
      : "HIGH_THROUGHPUT";
  printf(
  ...
  NCCLCHECK(
      ncclEpCreateGroup(&ep_group, comm, &config, s, torchMalloc, torchFree));
  ...
  if (random_mode) {
    // Random mode: first expert is random, rest are deterministic to avoid
    // repetitions
    ...
    for (int i = 0; i < num_tokens; i++) {
      ...
      // Remaining experts are deterministic based on first choice (no
      // repetitions)
    ...
    // Deterministic mode: send each token to top_k number of semi-random
    // experts, equal distribution
    for (int i = 0; i < num_tokens; i++) {
  ...
  // Create recv_expert_counter host tensor for ncclEpCreateHandle (only when
  // disable_max_tokens is true)
  ...
  // Array sizes depend on algorithm: LL uses 1 input/output, HT use 3 inputs/3
  // outputs
  ...
  if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    // LL mode: outputs[0] is [num_local_experts, max_recv_tokens * nRanks,
    // hidden]
    NCCLCHECK(ncclEpTensorCreate(
        ep_group,
        &outputs[0],
        ...
        config.max_tokens_per_rank * nRanks,
        static_cast<unsigned int>(hidden)));
  } else {
  ...
  // In HT mode, outputs[1] (recv_topk_weights) and outputs[2] (recv_topk_idx)
  // are 2D tensors [num_recv_tokens, top_k]
  if (algorithm != NCCL_EP_ALGO_LOW_LATENCY) {
    NCCLCHECK(ncclEpTensorCreate(
        ep_group,
        &outputs[1],
  ...
  // Fill the first ELEMENTS_TESTED_PER_TOKEN elements of each token with a
  // special value based on the current rank
  ...
  // Host buffer for first phase dispatch output (kept for cached mode
  // comparison in HT mode)
  uint16_t* first_dispatch_output0_host = nullptr;

  printf(
      "Rank %d: Testing ncclEpDispatch (send_only=%s)\n",
      myRank,
  ...
  // HT mode with disable_max_tokens: points to handle_local_tensors[0] (already
  // host memory) HT mode without disable_max_tokens: nullptr (no validation
  // available)
  ...
  if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    ...
    NCCLCHECK(ncclEpTensorGetData(
        handle_local_tensors[0], &handle_local_tensor0_data));
    recv_count_host = static_cast<int*>(handle_local_tensor0_data);
  }

  unsigned int recv_from_expert_start =
      (local_experts_start + num_experts - num_local_experts) % num_experts;
  ...
  if (!random_mode && algorithm == NCCL_EP_ALGO_LOW_LATENCY &&
      recv_count_host != nullptr) {
    // LL mode: outputs[0] is [num_local_experts, config.max_tokens_per_rank *
    // nRanks, hidden]
    uint16_t* output_host = new uint16_t
        [num_local_experts * config.max_tokens_per_rank * nRanks * hidden]();
    void* output0_data;
    ...
    for (unsigned int e = 0; e < num_local_experts; e++) {
      int expected_count = num_tokens; // Each expert receives num_tokens tokens
                                       // (one from each rank)
      ...
      for (int t = 0; t < recv_count_host[e] &&
           t < static_cast<int>(config.max_tokens_per_rank * nRanks);
           t++) {
        size_t token_offset =
            (e * config.max_tokens_per_rank * nRanks + t) * hidden;
        for (int j = 0; j < ELEMENTS_TESTED_PER_TOKEN; ++j) {
          uint16_t expected = static_cast<uint16_t>(0x1000 + recv_rank);
          uint16_t actual = output_host[token_offset + j];
          if (actual != expected) {
            printf(
                "Dispatch data check failed! Rank %d, expert %d, token %d, element %d: expected %d, got %d\n",
                myRank,
                e,
                t,
                j,
    ...
    if (recv_count_host != nullptr) {
      for (unsigned int e = 0; e < num_local_experts; e++) {
        int expected_count = num_tokens; // Each expert receives num_tokens
                                         // tokens (one from each rank)
        if (recv_count_host[e] != expected_count) {
          printf(
              "Recv_count check failed! Rank %d, expert %d: expected %d, got %d\n",
              myRank,
  ...
  // In HT mode, copy and verify outputs[1] (recv_topk_weights) and outputs[2]
  // (recv_topk_idx)
  if (!random_mode && algorithm != NCCL_EP_ALGO_LOW_LATENCY) {
    ...
    CUDACHECK(cudaMemcpy(
        recv_topk_weights_host,
        output1_data,
        num_recv_tokens * top_k * sizeof(float),
        cudaMemcpyDeviceToHost));
    ...
    CUDACHECK(cudaMemcpy(
        recv_topk_idx_host,
        output2_data,
        num_recv_tokens * top_k * sizeof(int64_t),
        cudaMemcpyDeviceToHost));
    ...
    if (weight_errors > 0) {
      printf(
          "Rank %d: recv_topk_weights verification failed with %d errors\n",
          myRank,
          weight_errors);
    }
    if (idx_errors > 0) {
      printf(
          "Rank %d: recv_topk_idx verification failed with %d errors\n",
          myRank,
          idx_errors);
    }
    ...
    if (ht_outputs_valid) {
      printf(
          "Rank %d: %s mode recv_topk_weights and recv_topk_idx verification passed\n",
          myRank,
          algorithm_name);
    } else {
      dispatch_check_passed = false;
  ...
  if (random_mode) {
    printf(
        "Rank %d: %s Dispatch flow completed (random mode, checks skipped)\n",
        myRank,
        algorithm_name);
  } else if (dispatch_check_passed) {
    printf(
        "Rank %d: %s Dispatch flow passed successfully\n",
        myRank,
        algorithm_name);
  } else {
  ...
  if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
    // LL mode: expert_outputs is 3D [num_local_experts,
    // config.max_tokens_per_rank * nRanks, hidden]
    NCCLCHECK(ncclEpTensorCreate(
        ep_group,
        &expert_outputs,
        3,
        ...
        nullptr,
        static_cast<unsigned int>(num_local_experts),
        static_cast<unsigned int>(config.max_tokens_per_rank * nRanks),
        static_cast<unsigned int>(hidden)));

    // Fill expert outputs with multiple test values per token
    // The ith element will be (i+1)*2 for all experts
    uint16_t* expert_outputs_host =
        new uint16_t[config.max_tokens_per_rank * hidden]();

    for (unsigned int t = 0; t < config.max_tokens_per_rank; ++t) {
      for (int j = 0; j < ELEMENTS_TESTED_PER_TOKEN; ++j) {
        // Set the ith element to (i+1)*2, stored as bf16
        expert_outputs_host[t * hidden + j] =
            float_to_bf16(static_cast<float>((j + 1) * 2));
      }
    }
    ...
    for (int e = 0; e < num_local_experts; ++e) {
      size_t offset_bytes = static_cast<size_t>(e) *
          config.max_tokens_per_rank * hidden * nRanks * 2;
      CUDACHECK(cudaMemcpy(
          static_cast<uint8_t*>(expert_outputs_data) + offset_bytes,
          expert_outputs_host,
  ...
  printf(
      "Rank %d: Testing ncclEpCombine (send_only=%s)\n",
      myRank,
  ...
  NCCLCHECK(ncclEpCombine(
      ...
      combine_outputs,
      1,
      combine_local_tensors,
      algorithm == NCCL_EP_ALGO_LOW_LATENCY ? 1 : 0,
      combine_send_only /* send_only */,
  ...
  if (random_mode) {
    printf(
        "Rank %d: Combine flow completed (random mode, checks skipped)\n",
        myRank);
    ...
    printf(
        "Rank %d: Combine verification FAILED with %d errors\n",
        myRank,
        combine_errors);
  ...
  // Cached mode test: repeat dispatch and combine calls with the same inputs
  // but new outputs Compare results between first and second phase to verify
  // cached mode correctness
  if (cached_mode) {
    // Cached mode is only supported in HT modes (not LL)
    if (algorithm == NCCL_EP_ALGO_LOW_LATENCY) {
      printf(
          "Rank %d: Error - cached mode is only supported in HT modes (not LL)\n",
          myRank);
      exit(EXIT_FAILURE);
    }
    ...
    // Save first phase dispatch outputs to host for comparison (only if not in
    // random mode)
    ...
    if (!random_mode) {
      ...
      CUDACHECK(cudaMemcpy(
          ...
          out0_data,
          num_recv_tokens * hidden * sizeof(uint16_t),
          cudaMemcpyDeviceToHost));
      void* out1_data;
    ...
    // Setup combine inputs with per-received-token topk_weights from dispatch
    // output. HT backward combine expects COMBINE_INPUT_TOPK_WEIGHTS to align
    // with COMBINE_INPUT_TOKENS.

    // Create a new tensor with the correct COMBINE_INPUT tag and copy data from
    // dispatch output.
    ...
    NCCLCHECK(ncclEpTensorGetData(
        cached_combine_topk_weights_input, &cached_ctwi_data));
    ...
    printf(
        "Rank %d: Testing cached mode - second ncclEpComplete (dispatch)\n",
        myRank);
    NCCLCHECK(ncclEpComplete(ep_handle, nullptr, s));
    CUDACHECK(cudaStreamSynchronize(s));
    ...
    NCCLCHECK(ncclEpCombine(
        ep_handle,
        cached_combine_inputs,
        2,
        cached_combine_outputs,
        2,
        nullptr,
        0, /* no local_tensors for HT combine */
        combine_send_only /* send_only */,
        nullptr /* config reserved */,
        s));

    printf(
        "Rank %d: Testing cached mode - second ncclEpComplete (combine)\n",
        myRank);
    NCCLCHECK(ncclEpComplete(ep_handle, nullptr, s));
    CUDACHECK(cudaStreamSynchronize(s));

    // Copy second phase outputs to host for comparison (only if not in random
    // mode)
    int cached_dispatch_errors = 0;
    int cached_combine_errors = 0;

    if (!random_mode) {
      uint16_t* second_dispatch_output0 =
          new uint16_t[num_recv_tokens * hidden];
      ...
      NCCLCHECK(
          ncclEpTensorGetData(cached_combined_output, &cached_comb_out_data));
      CUDACHECK(cudaMemcpy(
      ...
      NCCLCHECK(ncclEpTensorGetData(
          cached_combined_topk_weights, &cached_comb_tw_data));
      CUDACHECK(cudaMemcpy(
      ...
      for (unsigned int i = 0; i < num_recv_tokens * hidden; ++i) {
        if (first_dispatch_output0[i] != second_dispatch_output0[i]) {
          if (cached_dispatch_errors < 5) {
            printf(
                "Rank %d: Cached dispatch output0 mismatch at %u: first=%u, second=%u\n",
                myRank,
                i,
      ...
      // Verify combined topk_weights output (should match input
      // topk_weights: 1.0f / top_k)
      float expected_weight = 1.0f / top_k;
    ...
    if (random_mode) {
      printf(
          "Rank %d: Cached mode completed (random mode, checks skipped)\n",
          myRank);
    } else if (cached_dispatch_errors == 0 && cached_combine_errors == 0) {
      printf(
          "Rank %d: Cached mode verification PASSED - dispatch and combine outputs match\n",
          myRank);
    } else {
      printf(
    ...
    printf(
        "Rank %d: Cached mode - second dispatch and combine calls completed successfully\n",
        myRank);
  }

  // Clean up first phase host buffer (kept for cached mode comparison in HT
  // mode)
  if (first_dispatch_output0_host)
    delete[] first_dispatch_output0_host;
  ...
  // Free recv_expert_counter host tensor (uses cudaFreeHost, not cudaFree) only
  // if it was allocated
